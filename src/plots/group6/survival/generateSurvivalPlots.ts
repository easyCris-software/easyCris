...
export function buildCoxForestPlot(result: TestResult): PlotBuilderOutput | null {
  ...
  const coefficients = predictorCoeffs.map((param) => ({
    ...
    estimate: toNumber(param.hazard_ratio) ?? 1,
    stdError: toNumber(param.std_error) ?? 0,
    pValue: toNumber(param.p_value) ?? 1,
    confidenceInterval:
      toNumber(param.hr_ci_lower_95) !== null && toNumber(param.hr_ci_upper_95) !== null
        ? ([toNumber(param.hr_ci_lower_95), toNumber(param.hr_ci_upper_95)] as [number, number])
        : undefined,
  }))
  ...
  const axisFont = buildAxisFont()
  const hrValues = coefficients.map((c) => c.estimate).filter((v) => Number.isFinite(v))
