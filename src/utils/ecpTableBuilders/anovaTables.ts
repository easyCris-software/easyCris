...
function buildAssumptionChecksTable(
  ...
  // Normality check row
  if (result.all_groups_normal !== undefined) {
    rows.push({
      cells: [
        { value: 'Normality Check', align: 'left' },
        { value: '', align: 'right' },
        { value: '', align: 'right' },
        { value: result.all_groups_normal ? 'All groups normal' : 'Some groups non-normal', align: 'left' },
      ],
    });
  }
