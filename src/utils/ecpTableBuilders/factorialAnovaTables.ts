...
interface ResidualNormalityTestRow {
  test_name?: string
  statistic?: number
  p_value?: number
...
const RESIDUAL_NORMALITY_TEST_NAME_MAP: Record<string, string> = {
...
const RESIDUAL_NORMALITY_STAT_KEY_MAP: Record<string, string> = {
...
const RESIDUAL_NORMALITY_P_KEY_MAP: Record<string, string> = {
...
export function buildFactorialAnovaTables(
  ...
  const residualNormality = buildResidualNormalityTable(result, options)
  if (residualNormality) {
    tables.push(residualNormality)
  }
  ...
  const pairwise = Array.isArray(result.pairwise_comparisons)
    ? (result.pairwise_comparisons as Record<string, unknown>[])
    : []
...
function collectMultifactorEffects(result: Record<string, unknown>): EffectRow[] {
  const effects: EffectRow[] = []
  const mainEffects = result.main_effects as Record<string, unknown>[] | undefined
  const interactions = result.interactions as Record<string, unknown>[] | undefined
...
function buildFactorialEffectSizesTable(
  ...
  if (twoWay) {
    ...
    const mainEffects = result.main_effects as Record<string, unknown>[]
    for (const effect of mainEffects) {
      effectRows.push({
...
function buildFactorialCellMeansTable(
  ...
  const emmeans = Array.isArray(result.cell_emmeans)
    ? (result.cell_emmeans as Record<string, unknown>[])
    : null
  const summaries =
    (result.cell_summaries as Record<string, unknown>[] | undefined) ??
    (result.cell_means as Record<string, unknown>[] | undefined)
...
function buildFactorialSimpleEffectsTables(
  ...
  if (simpleEffects.length === 0 && marginalEffects.length === 0) {
    ...
    if (simpleEffectsCount > 0) {
      const flatSimpleEffects: Record<string, unknown>[] = []
...
function buildFactorialAssumptionsTable(
  ...
  const assumptions = result.assumptions as Record<string, unknown> | undefined
  const homogeneity =
    assumptions?.homogeneity_of_variance as Record<string, unknown> | undefined ??
    result.levene_test as Record<string, unknown> | undefined
  if (!homogeneity) {
    return null
  }
  ...
  if (homogeneity) {
    ...
    rows.push({
      cells: [
        { value: typeof homogeneity.test === 'string' ? homogeneity.test : "Levene's Test", align: 'left' },
...
function buildResidualNormalityTable(
  ...
  const rows: ECPRow[] = [
    {
      cells: [
        { value: 'Scope', isHeader: true, align: 'left' },
        { value: 'Test', isHeader: true, align: 'left' },
        { value: 'Statistic', isHeader: true, align: 'right' },
        { value: 'N', isHeader: true, align: 'right' },
  ...
  const residualN = parseInteger(normality?.n)
  for (const [testIndex, test] of tests.entries()) {
    const testName = test.test_name || ''
    const statKey = RESIDUAL_NORMALITY_STAT_KEY_MAP[testName]
    const pKey = RESIDUAL_NORMALITY_P_KEY_MAP[testName]
    const stat = parseNumber(test.statistic)
    const pValue = parseNumber(test.p_value)
    const pDisplay = formatMaybePValue(pValue, options)
    const isNormal =
      typeof test.is_normal === 'boolean'
        ? test.is_normal
        : pValue === undefined
        ? null
    ...
    rows.push({
      cells: [
        { value: 'Residuals', align: 'left' },
        { value: RESIDUAL_NORMALITY_TEST_NAME_MAP[testName] ?? testName, align: 'left' },
        {
          value: formatMaybeStatistic(stat, d),
          align: 'right',
          attrs: statKey && stat !== undefined
            ? { 'data-stat': `normality_residuals_${statKey}`, 'data-value': String(stat) }
            : undefined,
        },
        {
          value: residualN ?? '.',
          format: residualN !== undefined ? 'integer' : undefined,
          align: 'right',
          attrs: residualN !== undefined && testIndex === 0
            ? { 'data-stat': 'normality_residuals_n', 'data-value': String(residualN) }
            : undefined,
        },
        {
          value: pDisplay.text,
          align: 'right',
          isSignificant: pDisplay.isSignificant,
          attrs: pKey && pValue !== undefined
            ? { 'data-stat': `normality_residuals_${pKey}`, 'data-value': String(pValue) }
            : undefined,
        },
        {
  ...
  const footnotes = [
    `Note: p > ${options.alpha} indicates residuals are consistent with normality for that diagnostic.`,
  ]
  if (typeof normality?.warning === 'string' && normality.warning.trim()) {
    footnotes.push(normality.warning)
  }

  return {
    title: 'Residual Normality Tests',
    columns: [
      { key: 'scope', header: 'Scope', align: 'left', width: 14 },
    ...
    rows,
    footnotes,
    testName: 'factorial_anova_residual_normality',
  }
}
