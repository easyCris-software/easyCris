/**
 * Jarque-Bera Normality Test Module
 *
 ...
 * Uses skewness and kurtosis to detect deviations from normality.
 *
 * Requirements:
 ...
 */
export const jarqueBerraModule: ITestModule = {
  moduleId: 'normality_jb',

  /**
   * Validate column selection for Jarque-Bera Test
   *
   * Validation Rules:
  ...
  validateSelection(
    ...
    const result = TestValidator.combineResults([
      TestValidator.checkColumnCount(columns, 1, 'Jarque-Bera Test'),
      TestValidator.checkAllNumeric(columns, 'Jarque-Bera Test'),
    ])
  ...
  /**
   * Build Python payload for Jarque-Bera Test
   *
  ...
  buildPayload(
    ...
    try {
      if (columns.length !== 1 || selectedColumnIndices.length !== 1) {
        return {
          success: false,
          error: 'Jarque-Bera Test requires exactly 1 column',
        }
      }
      ...
      if (data.length < 4) {
        return {
          success: false,
          error: `Insufficient sample size: ${data.length} observations. Jarque-Bera requires at least 4 observations.`,
        }
      }
  ...
  /**
   * Default parameters for Jarque-Bera Test
   *
