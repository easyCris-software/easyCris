/**
 * Cramer-von Mises Normality Test Module
 *
 ...
 * Goodness-of-fit test measuring discrepancy between empirical and theoretical distributions.
 *
 * Requirements:
 ...
 */
export const cramerVonMisesModule: ITestModule = {
  moduleId: 'normality_cvm',

  /**
   * Validate column selection for Cramer-von Mises Test
   *
   * Validation Rules:
  ...
  validateSelection(
    ...
    const result = TestValidator.combineResults([
      TestValidator.checkColumnCount(columns, 1, 'Cramer-von Mises Test'),
      TestValidator.checkAllNumeric(columns, 'Cramer-von Mises Test'),
    ])
  ...
  /**
   * Build Python payload for Cramer-von Mises Test
   *
  ...
  buildPayload(
    ...
    try {
      if (columns.length !== 1 || selectedColumnIndices.length !== 1) {
        return {
          success: false,
          error: 'Cramer-von Mises Test requires exactly 1 column',
        }
      }
      ...
      if (data.length < 3) {
        return {
          success: false,
          error: `Insufficient sample size: ${data.length} observations. Cramer-von Mises requires at least 3 observations.`,
        }
      }
  ...
  /**
   * Default parameters for Cramer-von Mises Test
   *
