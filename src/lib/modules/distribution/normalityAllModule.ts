 ...
 * Normality (All Tests) Module
 *
 * Runs all normality tests at once for comprehensive assessment:
 * - Shapiro-Wilk Test
 ...
 *
 * Interpretation:
 * - Provides comprehensive normality assessment across multiple methods
 * - Each test may emphasize different aspects of deviation from normality
 */
export const normalityAllModule: ITestModule = {
  moduleId: 'normality_all',

  /**
   * Validate column selection for Combined Normality Tests
   *
   * Validation Rules:
  ...
  validateSelection(
    ...
    const result = TestValidator.combineResults([
      TestValidator.checkColumnCount(columns, 1, 'Normality (All Tests)'),
      TestValidator.checkAllNumeric(columns, 'Normality (All Tests)'),
    ])
  ...
  /**
   * Build Python payload for Combined Normality Tests
   *
  ...
  buildPayload(
    ...
    try {
      if (columns.length !== 1 || selectedColumnIndices.length !== 1) {
        return {
          success: false,
          error: 'Normality (All Tests) requires exactly 1 column',
        }
      }
      ...
      if (data.length < 8) {
        return {
          success: false,
          error: `Insufficient sample size: ${data.length} observations. Combined normality tests require at least 8 observations.`,
        }
      }
  ...
  /**
   * Default parameters for Combined Normality Tests
   *
