/**
 * Kendall Tau Correlation Module
 *
 ...
 * Kendall Tau Correlation Module
 *
 * Non-parametric measure of ordinal association between two variables.
 * Produces correlation coefficient (tau/τ) ranging from -1 to +1.
 *
 * Requirements:
 ...
 * - Better for small samples (more accurate p-values)
 * - More robust to tied ranks
 * - Interpretable as probability of concordance
 * - More conservative estimates (values typically lower than Spearman)
 *
 * Advantages over Pearson:
 * - No normality assumption (distribution-free)
 * - Robust to outliers (uses rank comparisons)
 * - Ideal for ordinal data (Likert scales)
 * - Works well with skewed distributions
 *
 * Interpretation:
 * - τ > 0: Positive association (high x → high y)
 * - τ < 0: Negative association (high x → low y)
 * - τ = 0: No association
 * - Values tend to be lower than Pearson/Spearman for the same data
 *
 * Use Case: Small samples with many tied values, ordinal scales
 */
export const kendallModule: ITestModule = {
  moduleId: 'correlation_kendall',

  /**
   * Validate column selection for Kendall Tau Correlation
   *
  ...
  validateSelection(
    ...
    const result = TestValidator.combineResults([
      TestValidator.checkColumnCountRange(columns, 2, 20, "Kendall's Tau Correlation"),
      TestValidator.checkAllNumeric(columns, "Kendall's Tau Correlation", { allowBinary: true }),
      TestValidator.checkOrdinalTieWarning(columns, "Kendall's Tau Correlation", {
        suggestKendall: false,
      }),
    ])
    ...
    if (result.isValid && hasOrdinal) {
      return {
        ...result,
        suggestions: [
          "Kendall's Tau is ideal for ordinal data (Likert scales) and small samples with tied ranks. It's more robust than Spearman for tied values.",
        ],
      }
    }
  ...
  /**
   * Build Python payload for Kendall Tau Correlation
   *
  ...
  buildPayload(
    ...
    try {
      if (columns.length < 2 || selectedColumnIndices.length < 2) {
        return {
          success: false,
          error: "Kendall's Tau Correlation requires at least 2 columns",
        }
      }
  ...
  /**
   * Default parameters for Kendall Tau Correlation
   *
