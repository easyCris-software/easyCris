/**
 * Contract tests for kaplanMeierModule
 *
...
describe('kaplanMeierModule', () => {
  describe('validateSelection', () => {
    describe('Column count validation', () => {
      it('should fail when < 2 columns selected', () => {
        ...
        const result = kaplanMeierModule.validateSelection(columns)

        expect(result.isValid).toBe(false)
        expect(result.errors).toContain(
          'Kaplan-Meier requires at least 2 columns (time + event). Selected: 1.'
        )
      ...
      it('should pass with exactly 3 columns (time + event + group)', () => {
        ...
        expect(result.errors).toHaveLength(0)
        expect(result.suggestions).toContain(
          'Kaplan-Meier with 3 groups: Survival curves will be compared using log-rank test.'
        )
      })
      ...
      it('should fail when > 3 columns selected', () => {
        ...
        expect(result.isValid).toBe(false)
        expect(result.errors).toContain(
          'Kaplan-Meier accepts at most 3 columns (time + event + group). Selected: 4.'
        )
        expect(result.suggestions).toContain(
    ...
    describe('Time column validation', () => {
      it('should fail when time column is not numeric', () => {
        ...
        expect(result.suggestions).toContain(
          'Time-to-event should be a positive numeric value (e.g., days, months, years).'
        )
      })
    ...
    describe('Group column validation', () => {
      ...
      it('should fail when group column has > 10 levels', () => {
        ...
        expect(result.isValid).toBe(false)
        expect(result.errors).toContain(
          "Group column 'Site' has too many levels (15). Maximum 10 groups allowed for Kaplan-Meier."
        )
        expect(result.suggestions).toContain(
  ...
  describe('buildPayload', () => {
    describe('Single-group survival analysis', () => {
      it('should build payload without group column', () => {
        ...
        const result = kaplanMeierModule.buildPayload(columns, indices, rows, parameters)

        expect(result.success).toBe(true)
        expect(result.payload).toBeDefined()
        expect(result.payload?.test).toBe('kaplan_meier')
    ...
    describe('Multi-group survival analysis', () => {
    ...
    describe('Error handling', () => {
      it('should fail when < 2 columns provided', () => {
        ...
        expect(result.error).toContain(
          'Kaplan-Meier requires at least 2 columns (time + event)'
        )
      })
      ...
      it('should fail when insufficient sample size (< 10 observations)', () => {
        ...
        expect(result.error).toContain('Kaplan-Meier requires at least 10 observations')
      })
  ...
  describe('defaultParameters', () => {
    it('should return default alpha of 0.05', () => {
      const params = kaplanMeierModule.defaultParameters()
