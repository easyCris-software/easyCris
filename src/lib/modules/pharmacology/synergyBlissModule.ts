/**
 * Bliss Independence Synergy Module
 *
 * Calculates drug synergy using Bliss independence model.
 *
 * Bliss expected effect: E_AB = E_A + E_B - (E_A * E_B)
 ...
 *
 * Note: Bliss Independence does not require dose concentrations -
 ...
 * Bliss Independence Synergy Module
 */
export const synergyBlissModule: ITestModule = {
  moduleId: 'synergy_bliss',

  /**
   * Validate column selection for Bliss Synergy
   */
  validateSelection(
    columns: ColumnClassification[],
    _options?: ValidateOptions
  ): TestValidationResult {
    return validateSynergySelection(columns, 'Bliss Independence Synergy')
  },

  /**
   * Build Python payload for Bliss Synergy
   *
   * Bliss only needs response values - no dose concentrations required.
   */
  buildPayload(
    columns: ColumnClassification[],
    ...
    try {
      if (columns.length < 3 || selectedColumnIndices.length < 3) {
        return {
          success: false,
          error:
            'Bliss Synergy requires at least 3 columns: Drug A Dose, Drug B Dose, Combined Response.',
        }
      }
      ...
      if (!pivotResult.responses_a || !pivotResult.responses_b || !pivotResult.combo_matrix) {
        return {
          success: false,
          error: 'Failed to build Bliss synergy payload: missing pivoted response data.',
        }
      }
      ...
      if (hasExplicitSingleAgents) {
        payload.metadata.warnings.push(
          'Note: Bliss Independence uses raw response values without curve fitting. ' +
  ...
  /**
   * Default parameters for Bliss Synergy
   */
