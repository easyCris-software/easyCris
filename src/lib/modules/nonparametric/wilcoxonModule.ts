 ...
 * - TestValidator.cs::ValidateForTest() - uses same validation as Independent T-Test (line 803)
 *
 ...
 * Wilcoxon Signed-Rank Test Module
 *
 * Non-parametric alternative to Paired T-Test for matched samples.
 * Tests the median of differences between paired observations.
 *
 * Supported formats:
 ...
 * - Small sample sizes
 * - Paired observations (before/after, matched pairs)
 */
export const wilcoxonModule: ITestModule = {
  moduleId: 'wilcoxon',

  /**
 * Validate column selection for Wilcoxon Signed-Rank Test
 *
 * Supports long and wide formats.
  ...
  validateSelection(
    ...
    if (columns.length !== 2) {
      return {
        isValid: false,
        errors: [`Wilcoxon Signed-Rank Test requires exactly 2 columns. Selected: ${columns.length}.`],
        warnings: [],
        suggestions: [
    ...
    if (!longFormat && !isWideFormat) {
      ...
      return {
        isValid: false,
        errors: [
          `Wilcoxon Signed-Rank Test requires either (1 numeric + 1 categorical) or (2 numeric) columns. Found: ${numericCount} numeric, ${categoricalCount} categorical.`,
        ],
        warnings: [],
        suggestions: [
    ...
    if (isWideFormat) {
      ...
      suggestions.push(
        '✓ Ideal for ordinal data: Wilcoxon Signed-Rank Test uses ranks and does not assume normality.'
      )
    ...
    if (!longFormat) {
      return {
        isValid: false,
        errors: ['Wilcoxon Signed-Rank Test requires long or wide format inputs.'],
        warnings: [],
        suggestions: [],
      }
    }
    ...
    // Check exactly 2 groups (paired test requirement)
    if (longFormat.groupCount !== 2) {
      return {
        isValid: false,
        errors: [
          `Wilcoxon Signed-Rank Test requires exactly 2 time points. Categorical column has ${longFormat.groupCount} unique values.`,
        ],
        warnings: [],
        suggestions: [
          longFormat.groupCount < 2
            ? 'The time/condition column must have exactly 2 distinct values (e.g., Pre/Post, Before/After).'
            : `For ${longFormat.groupCount} time points, use Friedman Test instead.`,
        ],
      }
    }
    ...
    // Wilcoxon-specific: Add positive suggestion for ordinal data
  ...
  /**
   * Build Python payload for Wilcoxon Signed-Rank Test
   *
   * Supports long and wide formats.
   *
  ...
  buildPayload(
    ...
    try {
      if (columns.length !== 2 || selectedColumnIndices.length !== 2) {
        return {
          success: false,
          error: 'Wilcoxon Signed-Rank Test requires exactly 2 columns',
        }
      }
      ...
      const mapping = parameters.wilcoxon_mapping as
      ...
      if (isWideFormat) {
        ...
        if (col1Index === null || col2Index === null || !col1 || !col2) {
          return {
            success: false,
            error: 'Wilcoxon Signed-Rank Test requires two numeric columns in wide format.',
          }
        }
      ...
      if (mapping?.group && mapping?.outcome) {
        ...
        if (!longFormat) {
          return {
            success: false,
            error: 'Wilcoxon Signed-Rank Test requires 1 numeric column and 1 categorical column.',
          }
        }
      ...
      if (result.groups.length !== 2) {
        return {
          success: false,
          error: `Wilcoxon Signed-Rank Test requires exactly 2 time points. Found ${result.groups.length} groups: ${result.groupNames.join(', ')}`,
        }
      }
      ...
      // CRITICAL: Wilcoxon (like paired t-test) requires EQUAL sample sizes
      if (group1Data.length !== group2Data.length) {
        return {
          success: false,
          error: `Wilcoxon Signed-Rank Test requires equal sample sizes. Found: ${group1Name} (n=${group1Data.length}), ${group2Name} (n=${group2Data.length}). Ensure each subject has data for both time points.`,
        }
      }
      ...
      // Build Python payload (backend expects group1/group2)
  ...
  /**
   * Default parameters for Wilcoxon Signed-Rank Test
   *
