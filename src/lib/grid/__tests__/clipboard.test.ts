...
describe('parseClipboardText', () => {
  ...
  describe('Edge cases', () => {
    ...
    it('removes trailing tab-only blank tail row', () => {
      const result = parseClipboardText('a1\tb1\n\t\t\n')
      expect(result).toEqual([
        ['a1', 'b1'],
      ])
    ...
    it('preserves trailing empty columns in non-empty rows', () => {
      const result = parseClipboardText('a1\tb1\t\na2\tb2\t')
      expect(result).toEqual([
        ['a1', 'b1', ''],
        ['a2', 'b2', ''],
  ...
  describe('Quoted cells', () => {
    it('preserves tabs inside quoted cells', () => {
      const result = parseClipboardText('"a\tb"\tc')
      expect(result).toEqual([['a\tb', 'c']])
    })
    ...
    it('normalizes Windows newlines inside quoted cells', () => {
      const result = parseClipboardText('"line1\r\nline2"\tc')
      expect(result).toEqual([['line1\nline2', 'c']])
    })

    it('unescapes doubled quotes inside quoted cells', () => {
      const result = parseClipboardText('"a ""quote"""\tc')
      expect(result).toEqual([['a "quote"', 'c']])
    })

    it('handles empty quoted cells in multi-cell rows', () => {
      const result = parseClipboardText('a\t""\tb')
      expect(result).toEqual([['a', '', 'b']])
    })

    it('tolerates an unclosed quoted cell at end of input', () => {
      const result = parseClipboardText('"unclosed')
    ...
    it('trims trailing blank rows after parsing', () => {
      const result = parseClipboardText('a\tb\n\n')
      expect(result).toEqual([['a', 'b']])
    })
  })
  ...
  describe('Numeric and special values', () => {
    ...
    it('preserves special characters', () => {
      const result = parseClipboardText('hello, world\tplain "quoted"\t$100')
      expect(result).toEqual([['hello, world', 'plain "quoted"', '$100']])
    })
  })
})
