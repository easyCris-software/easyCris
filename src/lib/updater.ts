...
export async function runUpdaterFlow(
  ...
  try {
    ...
    const proceedToInstall = await confirm(
      isWindows
        ? 'easyCris will close to launch the installer.\n\nSave your work now and continue?'
    ...
    emit({
      ...
      message: shouldRestart
        ...
        : 'Update installed. Restart later to finish.',
      version: update.version,
    })
    snapshot = {
      ...appendUpdaterEvent(snapshot, {
