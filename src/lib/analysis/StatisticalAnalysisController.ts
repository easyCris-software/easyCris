...
export class StatisticalAnalysisController {
  ...
  private _currentViewScope: GridViewScope | null = null
  ...
  public async runAnalysisWithTests(
    ...
    try {
      ...
      this._currentViewScope = viewScope && viewScope.datasetId === dataset.id ? viewScope : null
  ...
  private async executeTest(testName: string): Promise<void> {
    ...
    try {
      ...
      if (isLargeDataset) {
        if (this._currentViewScope?.source === 'view-filter') {
          toast.error(
            'Toolbar view filters cannot scope large-dataset analysis yet. Use the Advanced Filter dropdown option under Data to create a permanent filtered dataset.',
            { id: toastId }
          )
          return
        }
        ...
        const scopedRows =
          this._currentViewScope?.source === 'view-filter' && this._currentViewScope.dataModelRows !== null
            ? this._currentViewScope.dataModelRows.filter((rowIdx) => rowIdx >= 0 && rowIdx < rowCount)
            : null

        if (scopedRows !== null && scopedRows.length === 0) {
          toast.error('Filtered view contains no data rows for analysis', { id: toastId })
          return
        }

        const rowIndices = scopedRows ?? Array.from({ length: rowCount }, (_row, rowIdx) => rowIdx)

        // Build rowsArray as row-major matrix [rowIndex][localColumnIndex]
        rowsArray = rowIndices.map((rowIdx) =>
      ...
      if (this._currentViewScope?.source === 'view-filter' && !isLargeDataset) {
        parameters.analysis_scope = 'view-filter'
        parameters.analysis_scope_row_count = rowsArray.length
        parameters.analysis_scope_total_rows = this._currentViewScope.totalDataRowCount
      }
      ...
      if (actualTestName.toLowerCase().includes('anova')) {
        const anovaPayloadLog = {
          ...
          continuous_effects_config:
            (payloadResult.payload?.parameters as Record<string, unknown> | undefined)?.[
              'continuous_effects_config'
            ],
        }
      ...
      const primaryColumnName = resolvePrimaryColumnName(
