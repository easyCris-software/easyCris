...
interface DataState {
  ...
  replaceAllDatasetsWith: (dataset: Dataset) => void
  ...
  /** Insert `count` empty rows at a specific model-row index */
  insertRowsAtDataset: (datasetId: string, index: number, count: number) => void
  ...
  updateCellsBatch: (
    datasetId: string,
    updates: Array<{ rowIdx: number; columnId: string; value: unknown }>
  ) => void
  ...
  devtools(
    (set, get) => ({
      ...
      replaceAllDatasetsWith: (dataset) =>
        set(
          {
            datasets: [ensureDatasetAutoColumnCounter(dataset)],
            currentDataset: ensureDatasetAutoColumnCounter(dataset),
            gridSelection: {
              selectedRows: [],
              selectedColumns: [],
              selectedCells: undefined,
      ...
      insertRowsAtDataset: (datasetId, index, count) =>
        set(
          state => {
            ...
            const apply = (d: Dataset): Dataset => {
              ...
              return {
                ...d,
                rowCount: d.rowCount + safeCount,
                dataRowCount: Math.max(dataRows + safeCount, at + safeCount),
                modifiedAt: new Date(),
              }
            }
      ...
      initializeBlankDataset: async (
        ...
        try {
          ...
          logRuntimeDebug('app', 'data_store_initialize_blank_start', {
          ...
          logRuntimeDebug('app', 'data_store_initialize_blank_project_start', {
            datasetId: dataset.id,
          })
          await ensureProjectId()
          logRuntimeDebug('app', 'data_store_initialize_blank_project_done', {
            datasetId: dataset.id,
          })
          ...
          const backendStartedAt = typeof performance !== 'undefined' ? performance.now() : Date.now()
          const backendWatchdog =
            typeof window !== 'undefined'
              ? window.setTimeout(() => {
                  if (!backendFinished) {
                    logRuntimeDebug('app', 'data_store_initialize_blank_backend_slow', {
                      datasetId: dataset.id,
                      elapsedMs: Math.round((typeof performance !== 'undefined' ? performance.now() : Date.now()) - backendStartedAt),
                      columnCount: columnPayload.length,
                    })
                  }
                }, 5000)
              : null
          logRuntimeDebug('app', 'data_store_initialize_blank_backend_start', {
            datasetId: dataset.id,
            columnCount: columnPayload.length,
          })
          await cacheService.createEmptyDuckDB(dataset.id, columnPayload)
          backendFinished = true
          if (backendWatchdog !== null) {
            window.clearTimeout(backendWatchdog)
          }
          logRuntimeDebug('app', 'data_store_initialize_blank_backend_done', {
            datasetId: dataset.id,
            elapsedMs: Math.round((typeof performance !== 'undefined' ? performance.now() : Date.now()) - backendStartedAt),
          })
          logRuntimeDebug('app', 'data_store_initialize_blank_backend_total_done', {
            datasetId: dataset.id,
            elapsedMs: Math.round((typeof performance !== 'undefined' ? performance.now() : Date.now()) - startedAt),
          })
        } catch (error) {
          logRuntimeDebug('app', 'data_store_initialize_blank_failed', {
            datasetId: dataset.id,
        ...
        set(
          state => ({
            ...
            currentDataset: shouldActivate ? dataset : state.currentDataset,
            // Clear invalidation state when switching to new dataset
            invalidatedColumnIds: shouldActivate ? new Set<string>() : state.invalidatedColumnIds,
            columnClassificationCache: shouldActivate
              ? new Map<string, CachedClassification>()
              : state.columnClassificationCache,
            selectionStats: shouldActivate ? null : state.selectionStats,
          }),
          undefined,
          'initializeBlankDataset'
        )
        logRuntimeDebug('app', 'data_store_initialize_blank_store_done', {
          datasetId: dataset.id,
          activated: shouldActivate,
          datasetCount: get().datasets.length,
          currentDatasetId: get().currentDataset?.id ?? null,
        })
        ...
        logRuntimeDebug('app', 'data_store_initialize_blank_cache_done', {
          datasetId: dataset.id,
        ...
        set(
          state => {
            ...
            for (const update of updates) {
              if (!updatedCache[update.rowIdx]) {
                updatedCache[update.rowIdx] = {}
              }
              updatedCache[update.rowIdx]![update.columnId] = update.value
            }
