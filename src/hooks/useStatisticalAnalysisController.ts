...
export function useStatisticalAnalysisController(): UseStatisticalAnalysisControllerResult {
  ...
  const runAnalysisWithTests = useCallback(
    async (testNames: string[], columns: ColumnClassification[], dataset: any, familyId?: string | null) => {
      try {
        ...
        await controllerRef.current.runAnalysisWithTests(testNames, columns, dataset, familyId)
      } catch (error) {
