...
import type { ValidationResult } from '@/store/analysis-store'
...
/**
 * Statistical test execution request
 */
...
export interface ExportOptions {
  format: 'png' | 'svg' | 'pdf' | 'html' | 'csv' | 'json'
  width?: number
  height?: number
  dpi?: number
  includeData?: boolean
...
export const tauriApi = {
  ...
  async getColumnStatistics(
    ...
    try {
      return await invoke('get_column_statistics', { datasetId, columnId })
    } catch (error) {
      throw createTauriError(error, 'Failed to get column statistics')
    }
  },
  ...
  /**
   * Validate test parameters (5-layer validation)
   */
  async validateTest(
    testId: string,
    parameters: Record<string, unknown>,
    ...
    try {
      return await invoke<ValidationResult>('validate_test', {
        testId,
        parameters,
        datasetId,
      })
    } catch (error) {
      throw createTauriError(error, 'Validation failed')
    }
  },
  ...
  /**
   * Execute statistical test via Python backend
   */
  async executeTest(request: TestExecutionRequest): Promise<TestResult> {
    try {
      return await invoke<TestResult>('execute_test', { request })
    } catch (error) {
      throw createTauriError(error, 'Test execution failed')
    }
  },
  ...
  /**
   * Export plot to image (PNG, SVG, PDF)
   * Uses Kaleido via Python for Plotly charts
   */
  async exportPlot(
    plotId: string,
    plotlyJson: unknown,
    ...
    try {
      return await invoke<Uint8Array>('export_plot', {
        plotId,
        plotlyJson,
        options,
      })
    } catch (error) {
      throw createTauriError(error, 'Failed to export plot')
    }
  },
   ...
   *
   * Windows-only function using pywin32 for OLE/COM clipboard operations.
   *
   * @param ecpPath - Absolute path to .ecp project file (must exist)
   * @param pngBytes - PNG image data as Uint8Array
   * @returns Success message
   * @throws TauriError if project file not found or clipboard operation fails
   */
  async copyPlotOle(ecpPath: string, pngBytes: Uint8Array): Promise<string> {
    console.log('[tauriApi] copyPlotOle called:', {
      ecpPath,
      pngBytesLength: pngBytes.length,
      pngBytesType: typeof pngBytes,
    })

    try {
      // Convert Uint8Array to regular array for Tauri IPC
      const pngArray = Array.from(pngBytes)
      console.log('[tauriApi] Converted to array:', {
        arrayLength: pngArray.length,
        arrayType: typeof pngArray,
        isArray: Array.isArray(pngArray),
        firstBytes: pngArray.slice(0, 10),
      })

      console.log('[tauriApi] Invoking Rust command copy_plot_ole...')
      const result = await invoke<string>('copy_plot_ole', {
        ecpPath,
        pngBytes: pngArray,
      })

      console.log('[tauriApi] Rust command result:', result)
      return result
    } catch (error) {
      console.error('[tauriApi] copyPlotOle error:', error)
      ...
      throw createTauriError(error, 'Failed to copy plot to clipboard')
    }
  },
   ...
   */
  async checkPythonHealth(): Promise<{
    available: boolean
    version: string
    modules: Array<{ name: string; version: string }>
  }> {
    try {
      return await invoke('check_python_health')
    } catch (error) {
      throw createTauriError(error, 'Python health check failed')
    }
  },
