/**
 * plotBackendService.ts - TypeScript interface for plot_backend.py
 *
...
/**
 * Call the plot_backend.py with a JSON request
 */
async function callPlotBackend<T>(request: Record<string, unknown>): Promise<T> {
  const input = JSON.stringify(request)

  try {
    const result = await invoke<string>('run_plot_backend', { input })
