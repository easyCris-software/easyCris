...
const appStoreApi = create<AppState>((set, get) => ({
  ...
  createFamily: vi.fn(async (name?: string) => {
    const newFamily = {
      id: 'statistics-2',
      name: name ?? 'Statistics #2',
      datasetId: datasetB.id,
    }
    dataStoreApi.setState(state => ({
      datasets: [...state.datasets, datasetB],
    }) as any)
    set(state => ({
...
vi.mock('@/utils/e2eMode', () => ({ isE2EEnabled: () => true }))
...
vi.mock('@/store/analysis-store', () => ({
  useAnalysisStore: Object.assign(
    ...
    { getState: () => ({ execution: { status: 'idle' }, history: [], maxHistorySize: 50, clearHistory: vi.fn(), setExecutionProgress: vi.fn() }) }
  ),
}))
...
describe('AppShell family create activation contract', () => {
  ...
  it('publishes the committed display dataset id to the E2E shim during pending activation and updates it after promotion', async () => {
    ...
    expect(
      ...
      committedDatasetId: 'dataset-a',
      pendingDatasetId: null,
      pendingSurfaceStatus: 'committed',
    })

    await act(async () => {
      await useAppStore.getState().createFamily('Statistics #2')
    })

    await waitFor(() => {
      expect(useAppStore.getState().activeFamilyId).toBe('statistics-2')
      expect(screen.getByTestId('view-state-probe').textContent).toContain('"datasetId":"dataset-a"')
      ...
      expect(
        ...
        committedDatasetId: 'dataset-a',
        pendingDatasetId: 'blank-dataset-b',
        pendingSurfaceStatus: 'staging',
        token: 1,
      })
    })
    ...
    await waitFor(() => {
      ...
      expect(
        ...
        committedDatasetId: 'blank-dataset-b',
        pendingDatasetId: null,
        pendingSurfaceStatus: 'committed',
        token: null,
      })
