...
import { FolderDown, Play, Trash2, Copy, ClipboardPaste, Undo2, Download, TableOfContents, Settings2, Type, Palette, Axis3d, Scissors, Bug, Lightbulb, BookOpen, BookText, FileText, Wrench, RefreshCw, Plus } from 'lucide-react'
import { SortAscendingIcon, WarningCircle } from '@phosphor-icons/react'
...
const LEGAL_RESOURCES: LegalResource[] = [
  ...
  {
    id: 'eula',
    label: 'Open-Source Terms',
    description: 'AGPL terms and commercial licensing contact',
    file: 'legal/OSS_TERMS.txt',
    Icon: FileText,
  },
]
  ...
  const helpFeedbackItems = [
    {
      ...
      icon: Bug,
      onSelect: () => {
        void openUrl('https://github.com/easyCris-software/easyCris/issues/new?labels=bug&template=bug_report.md')
        setShowReportDialog(false)
      },
    },
    {
      ...
      icon: Lightbulb,
      onSelect: () => {
        void openUrl('https://github.com/easyCris-software/easyCris/issues/new?labels=enhancement&template=feature_request.md')
        setShowReportDialog(false)
      },
    },
  ]
                  ...
                  {helpFeedbackItems.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={item.onSelect}
                        className="w-full flex items-center gap-3 px-3 py-2 hover:bg-accent rounded-md text-left transition-colors"
                      >
                        <Icon className="h-4 w-4 text-muted-foreground shrink-0" />
                        <div>
                          <p className="text-sm font-medium">{item.title}</p>
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Outline Button (Visual Grouping) */}
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                variant="ghost"
            ...
            <TooltipContent>
              <p>Create expandable outline view grouped by column</p>
            </TooltipContent>
          </Tooltip>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                className="flex items-center gap-2"
              >
                <SortAscendingIcon className="h-4 w-4" weight="regular" />
                <span>Sort</span>
              </Button>
            </TooltipTrigger>
            <TooltipContent>
              <p>Sort data by column</p>
            </TooltipContent>
          </Tooltip>
        </div>
