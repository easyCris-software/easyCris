...
import { OleIntegrationDialog } from '@/components/dialogs/OleIntegrationDialog'
...
type LegalDocHashes = Record<LegalDocKey, string>
type LegalDocTexts = Record<LegalDocKey, string>
...
const LEGAL_DOC_LABELS: Record<LegalDocKey, string> = {
  eula: 'Open-Source Terms',
  privacy: 'Privacy Policy',
  thirdParty: 'Third-Party Licenses',
}
const ALL_LEGAL_DOC_KEYS: LegalDocKey[] = ['eula', 'privacy', 'thirdParty']
...
const LEGAL_POLICY_VERSION = '2026-02-11'
...
const computeCurrentLegalBundle = async (): Promise<LegalDocsBundle> => {
  const [eula, privacy, thirdParty] = await Promise.all([
    readBundledLegalText(LEGAL_DOC_PATHS.eula),
    readBundledLegalText(LEGAL_DOC_PATHS.privacy),
    readBundledLegalText(LEGAL_DOC_PATHS.thirdParty),
  ])

  const [eulaHash, privacyHash, thirdPartyHash] = await Promise.all([
    sha256Hex(eula),
    sha256Hex(privacy),
    sha256Hex(thirdParty),
  ])
...
const parseStoredLegalAcceptance = (raw: string | null): StoredLegalAcceptance | null => {
  ...
  try {
    ...
    const hashes = parsed.hashes as Partial<LegalDocHashes>
    if (
      typeof hashes.eula !== 'string' ||
      typeof hashes.privacy !== 'string' ||
      typeof hashes.thirdParty !== 'string'
    ) {
      return null
    }
    return {
...
const getChangedLegalDocs = (
  stored: StoredLegalAcceptance,
  currentHashes: LegalDocHashes
): LegalDocKey[] =>
  ALL_LEGAL_DOC_KEYS.filter((key) => stored.hashes[key] !== currentHashes[key])
...
export function AppShell({ className }: AppShellProps) {
  ...
  // Preferences (needed for OLE prompt state)
  ...
  const [legalDocsToReview, setLegalDocsToReview] = useState<LegalDocKey[]>([
    ...ALL_LEGAL_DOC_KEYS,
  ])
  ...
  const [selectedLegalDoc, setSelectedLegalDoc] = useState<LegalDocKey>('eula')
  ...
  const showBlockingAppBusyOverlay =
    appOperationLock.active && appOperationLock.owner === 'rnaseq'
  ...
  const runLegalGateCheck = useCallback(async () => {
    ...
    try {
      ...
      if (!stored) {
        // Normal install path: installer/open-source terms acceptance is primary.
        ...
        try {
          ...
          setLegalDocsToReview([...ALL_LEGAL_DOC_KEYS])
          setLegalGateOpen(false)
        } catch (persistError) {
        ...
        const docsToReview: LegalDocKey[] = requiresReconsent
  ...
  const statisticsGridKey = useMemo(() => {
    const familyKey = activeFamilyId ?? 'statistics-1'
    const datasetKey = activeDataset?.id ?? 'none'
    const projectKey = projectId ?? 'project-unknown'
    return `stats-grid:${projectKey}:${familyKey}:${datasetKey}`
  }, [activeDataset?.id, activeFamilyId, projectId])
  ...
  useEffect(() => {
    ...
    if (!family?.datasetId) {
      if (e2eEnabled && wantsBootstrap) {
        console.log('[E2E] Suppressed family blank-dataset bootstrap')
      }
      const shouldBootstrapBlankDataset =
        !e2eEnabled && wantsBootstrap
      if (shouldBootstrapBlankDataset && family) {
        let cancelled = false
        const init = async () => {
          try {
      ...
      } else if (!fallbackDataset && currentDataset) {
        setCurrentDataset(null)
      }
      return
    }
    ...
    // Use O(1) Map lookup instead of O(n) find()
    const dataset = datasetsById.get(family.datasetId) ?? null
    if (dataset) {
      if (currentDataset?.id !== dataset.id) {
        setCurrentDataset(dataset)
      }
    } else {
      if (fallbackDataset && currentDataset?.id !== fallbackDataset.id) {
        setCurrentDataset(fallbackDataset)
      } else if (!fallbackDataset && currentDataset) {
        setCurrentDataset(null)
      }
  ...
  // Check OLE integration on first launch (Windows only)
  // DISABLED: OLE activation is too buggy with Office security restrictions
  // To re-enable: uncomment this useEffect block
  /*
  useEffect(() => {
    const checkOleIntegration = async () => {
      // Skip if not Windows
      ...
      if (preferences?.ole_integration_prompted) {
        return
      }

      try {
        // Check if OLE is already registered
        const isRegistered = await invoke<boolean>('check_ole_registration')

        if (isRegistered) {
          // Already registered, just update preferences
          const basePrefs = preferences ?? { theme: 'light' }
          savePreferences({
            ...basePrefs,
            theme: basePrefs.theme ?? 'light',
            ole_integration_prompted: true,
            ole_integration_enabled: true,
          })
        } else {
          // Not registered, show the prompt
          setOleIntegrationDialogOpen(true)
        }
      } catch (error) {
        // Command not available (non-Windows or old build), ignore
        console.log('OLE registration check not available:', error)
      }
    }

    // Delay check slightly to avoid blocking initial render
    const timer = setTimeout(checkOleIntegration, 1000)
  ...
  */
  ...
  const oleExperimentalEnabled = import.meta.env.DEV || import.meta.env.VITE_EXPERIMENTAL_OLE === 'true'

  // OLE integration dialog state (Windows only, first-launch prompt)
  const [oleIntegrationDialogOpen, setOleIntegrationDialogOpen] = useState(false)
  ...
  const handleTestExecution = useCallback(async (selectedColumns: SelectedColumnInfo[]) => {
    ...
    if (module && classifications) {
      try {
        ...
        await runAnalysisWithTests([selectedTest.id], classifications, currentDataset, activeFamilyId)
  ...
  // Refs to store Sort/Group dialog openers from SpreadsheetView
  ...
  }, [initializeBlankDataset, legalGateBlocking, e2eEnabled])
  ...
  const confirmPreflight = (_preflight: PreflightResult, type: string, rowCount: number): Promise<boolean> => {
    return new Promise<boolean>((resolve) => {
      ...
      setPreflightConfirm({
        message: `This dataset has ${rowCount.toLocaleString()} rows. ${OPERATION_LABELS[type] ?? type} on large datasets may take a moment.`,
        rowCount,
        operationLabel: OPERATION_LABELS[type] ?? type,
      })
    })
  }
  ...
  const getColumnsWithData = async (
    ...
    try {
      ...
      const withDataIds = new Set(
        stats
          .filter((stat) => stat.nonNullCount > 0)
          .map((stat) => stat.columnId)
      )
      const filtered = columns.filter((column) => withDataIds.has(column.id))
      if (filtered.length === 0) {
        console.info(`[Transform] ${contextLabel}: no non-empty columns detected, using full schema list.`)
        return columns
      }
      if (filtered.length < columns.length) {
        console.info(`[Transform] ${contextLabel}: filtered columns by data presence.`, {
      ...
      console.warn(`[Transform] ${contextLabel}: failed to fetch column stats; using full schema list.`, error)
      return columns
    }
  }
  ...
  const isTransformBlocked = async (dataset: Dataset): Promise<boolean> => {
    ...
    if (dataRowCount === 0) {
      toast.error('Dataset has no rows to transform')
      return true
    }
  ...
  const importDatasetFromPath = async (filePath: string) => {
    ...
    try {
      ...
      toast.loading('Importing dataset…', { id: toastId })
  ...
  const handleImportData = async () => {
    if (blockIfAppLocked('Import')) return

    // OPTIMIZATION: Open dialog first, then set loading state
    // Keeps the click → dialog path minimal for instant feedback
  ...
  const handleOpenRNAseqConfigureFromGuide = () => {
    ...
    const projectToOpen =
      ...
      rnaseqStore.createProject('New RNA-seq Project')

    rnaseqStore.setActiveProject(projectToOpen.id)
    rnaseqStore.setActiveTab(projectToOpen.id, 'counts')
  }
  ...
  const handleImportRNAseqSample = async () => {
    ...
    try {
      ...
      const project = rnaseqStore.createProject(projectName)
      rnaseqStore.setCountsDataset(project.id, countsDataset.id)
      rnaseqStore.setMetadataDataset(project.id, metadataDataset.id)
  ...
  const handleApplyAdvancedFilter = async (config: FilterConfig) => {
    if (blockIfAppLocked('Advanced filter')) return
    if (!transformDataset) return
    if (await isTransformBlocked(transformDataset)) return
  ...
  const applyPendingTransform = async (mode: TransformMode) => {
    ...
    try {
      ...
      if (transform.type === 'pivot_wider') {
        ...
        if (collisionKeys.length > 0) {
          const preview = collisionKeys
            ...
            .join(', ')
          const suffix = collisionKeys.length > 6 ? '…' : ''
          toast.warning(
      ...
      if (mode === 'new-family') {
        ...
        useAppStore.getState().setProjectDirty(true)
        toast.success(`${actionLabel} applied → new family "${newFamily.name}"`)
      } else {
  ...
  const saveProjectToPath = useCallback(async (filePath: string) => {
    ...
    try {
      ...
      // ✅ PHASE 3: Bundle DuckDB files with project (portable storage)
  ...
  const handleSaveProject = useCallback(async () => {
    ...
    if (!filePath) {
      // First save → behave like Save As
      filePath = await save({
        title: 'Save Project',
  ...
  const clearAllDataResultsAndPlots = useCallback(() => {
    clearAllDatasets()
  ...
  const handleNewProject = useCallback(async () => {
    ...
    restoreFamilies([newFamily], newFamily.id)
    ...
    try {
      ...
      setActiveFamilyDataset(newFamily.id, blankDataset.id, false)
    } catch (error) {
    ...
    if (wasRNAseqActive) {
      useRNAseqStore.getState().createProject('RNA-seq 1')
    }
  }, [
    activeRNAseqProject,
    blockIfAppLocked,
  ...
  const loadProjectFromPath = useCallback(async (
    ...
    try {
      ...
      // ✅ PHASE 5: Configure backend to use project-adjacent storage
      const projectDir = await dirname(filePath)
      const projectBaseName = await basename(filePath, '.ecp')
      const dataDir = await join(projectDir, `${projectBaseName}_data`)
      ...
      // - If user cancels or zero datasets loadable → return early, state unchanged
      ...
      for (const dataset of restoredDatasets) {
        ...
        // ✅ PRIORITY 1: Use existing DuckDB file (has edits)
        if (dataset.duckdbPath) {
            const projectDir = await dirname(filePath)
            const isDuckDBAbsolute = await isAbsolute(dataset.duckdbPath)
            const absoluteDuckDBPath = isDuckDBAbsolute
          ...
          // ✅ PRIORITY 2: Re-import from source file (fallback)
        ...
        // ✅ PRIORITY 3: Skip if both DuckDB and source failed
        if (!importSucceeded) {
  ...
  const handleClearData = async () => {
    ...
    if (datasetId) {
      const usedByOtherFamilies = families.some(
        (family) => family.datasetId === datasetId && family.id !== activeFamilyId
      )
      const usedByRNAseq = rnaseqDatasetIds.has(datasetId)

      if (!usedByOtherFamilies && !usedByRNAseq) {
        removeDataset(datasetId)
        // REMOVED: Frontend Zustand cache - SpreadsheetView now uses streaming row provider
        // setCacheData(`dataset:${datasetId}`, [])
        cacheService.removeDataset(datasetId).catch(error => {
          console.error(`Failed to remove dataset ${datasetId} from cache`, error)
        })
      }
    }
    ...
    if (activeFamily) {
      ...
      useAppStore.getState().updateFamilyDataFlag(activeFamily.id, false)
    }
  ...
  // Handle OLE integration dialog choices
  const handleOleIntegrationChoice = async (choice: 'enable' | 'later' | 'never') => {
    setOleIntegrationDialogOpen(false)

    if (choice === 'enable') {
      if (!oleExperimentalEnabled) {
        toast.info('PowerPoint OLE integration is disabled in this build', {
          description: 'Set VITE_EXPERIMENTAL_OLE=true to test the experimental OLE flow.',
        })
        return
      }

      try {
        // Call Rust command to enable OLE registration (triggers UAC prompt)
        await invoke<string>('enable_ole_registration')

        // Update preferences to mark as prompted and enabled
        const basePrefs = preferences ?? { theme: 'light' }
        savePreferences({
          ...basePrefs,
          theme: basePrefs.theme ?? 'light',
        ...
        toast.success('PowerPoint integration enabled', {
          description: 'You can now double-click plots pasted into PowerPoint.',
        })
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error)
        toast.error('Failed to enable PowerPoint integration', {
          description: message,
        })

        // Still mark as prompted even if it failed
        const basePrefs = preferences ?? { theme: 'light' }
        savePreferences({
          ...basePrefs,
          theme: basePrefs.theme ?? 'light',
      ...
      // User wants to be reminded later, don't mark as prompted
      // (Dialog will show again on next launch)
      console.log('OLE integration deferred to later')
    } else if (choice === 'never') {
      // User doesn't want this feature, mark as prompted but not enabled
      const basePrefs = preferences ?? { theme: 'light' }
      savePreferences({
        ...basePrefs,
        theme: basePrefs.theme ?? 'light',
      ...
      toast.info('PowerPoint integration disabled', {
        description: 'You can still copy plots, but double-click activation will not work.',
      })
    }
  }

  if (legalGateEnabled && !legalGateReady) {
    return (
      <>
        <Toaster position="bottom-right" />
        <div className={cn('flex h-[var(--app-height)] w-[var(--app-width)] items-center justify-center bg-background', className)}>
          <div className="rounded-lg border bg-card px-6 py-4 text-center shadow-sm">
            <p className="text-sm font-medium">Preparing legal documents…</p>
            <p className="mt-1 text-xs text-muted-foreground">Please wait before using easyCris.</p>
          </div>
        </div>
      </>
    )
  ...
  return (
    <>
            ...
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {legalGateRequiresReconsent
                  ...
                  : 'This version requires legal acceptance before you continue. Review the Open-Source Terms, Privacy Policy, and Third-Party Licenses below.'}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <div className="space-y-3 text-xs text-muted-foreground">
              ...
              {legalGateHashes ? (
                <div className="space-y-1 font-mono">
                  <p>Open-Source Terms hash: {shortHash(legalGateHashes.eula)}</p>
                  <p>Privacy hash: {shortHash(legalGateHashes.privacy)}</p>
                  <p>Third-party hash: {shortHash(legalGateHashes.thirdParty)}</p>
                </div>
              ) : (
                <p>{legalGateLoading ? 'Computing legal document hashes…' : 'Legal hashes unavailable.'}</p>
              )}
              <div className="flex flex-wrap gap-2">
                {visibleLegalDocs.map((docKey) => (
                  ...
                  >
                    {LEGAL_DOC_LABELS[docKey].replace(' Policy', '').replace(' Licenses', '')}
                  </button>
                ))}
              </div>
              <div className="rounded-md border bg-muted/20 p-2">
                ...
                {selectedLegalText ? (
                  ...
                  <p className="text-[11px]">{legalGateLoading ? 'Loading document…' : 'Document unavailable.'}</p>
                )}
              </div>
      ...
      <Toolbar
        ...
        onImportData={handleImportData}
        onSort={() => openSortDialogRef.current?.()}
            ...
            {activeRNAseqProject ? (
              ...
              <div className="h-full flex flex-col bg-background min-w-0">
                {/* Action Toolbar - inside workspace for proper alignment */}
                  ...
                  onSort={() => openSortDialogRef.current?.()}
                  onGroup={() => openGroupDialogRef.current?.()}
      ...
      />
        ...
        {oleExperimentalEnabled && (
          <OleIntegrationDialog
            open={oleIntegrationDialogOpen}
            onChoice={handleOleIntegrationChoice}
          />
        )}
      </div>
    </>
