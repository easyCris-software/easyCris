...
import { ChevronDown, Download, FolderDown, Settings2, TableProperties, Trash2 } from 'lucide-react'
...
export function RNAseqWorkspace({
  ...
  const removeDataset = useDataStore((state) => state.removeDataset)
  ...
  const clearClipboardHandlers = useCallback(() => {
    ...
    onCutRequest?.(() => undefined)
    onPasteRequest?.(() => undefined)
  ...
  const handleClearDataset = useCallback(() => {
    ...
    if (activeTab === 'counts' && countsDataset) {
      const datasetId = countsDataset.id
      const usedByStats = families.some((family) => family.datasetId === datasetId)
      const usedByOtherRNAseq = rnaseqProjects.some(
        (project) =>
          project.id !== activeProject.id &&
          (project.countsDatasetId === datasetId || project.metadataDatasetId === datasetId)
      )
      if (!usedByStats && !usedByOtherRNAseq) {
      ...
      setMetadataDataset(activeProject.id, null)
      setCurrentDataset(null)
  ...
  return (
    <div className={cn('flex flex-col h-full', className)}>
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b bg-background">
        <div className="flex items-center gap-2">
          ...
          <Button
            ...
            disabled={!countsDataset || !metadataDataset || isMutationLocked}
          >
          ...
          {(activeTab === 'counts' || activeTab === 'metadata') && (countsDataset || metadataDataset) && (
            <>
              <Button
                variant="outline"
                size="sm"
                onClick={() => openGroupDialogRef.current?.()}
              ...
              <Button
                variant="outline"
                size="sm"
                onClick={() => openSortDialogRef.current?.()}
                disabled={isMutationLocked}
              >
      ...
      <RNAseqTabBar
        ...
        hasCountsData={!!countsDataset}
        hasMetadataData={!!metadataDataset}
