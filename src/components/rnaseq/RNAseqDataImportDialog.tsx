...
type DatasetLike = Pick<Dataset, 'id' | 'columns' | 'rowCount'>
...
export function RNAseqDataImportDialog({
  ...
  const { setCountsDataset, setMetadataDataset, getProject } = useRNAseqStore()
  ...
  const validateFile = async (
    ...
    try {
      ...
      if (context.targetMode === 'counts') {
        ...
        if (linkedDataset && countValidation.valid) {
        ...
        if (linkedDataset && metadataValidation.valid) {
  ...
  const confirmSampleMismatch = (sampleMatch: SampleMatchResult): boolean => {
    const prompt = `${sampleMatch.message}\n\nContinue import anyway?`
    return window.confirm(prompt)
  }

  const normalizeSampleId = (value: unknown): string => {
    if (value === null || value === undefined) return ''
    return String(value).trim()
  }

  const isPlaceholderColumnName = (name: string): boolean => {
  ...
  const hasNonZeroNumericValue = (values: unknown[]): boolean => {
  ...
  const columnHasData = (column: { name: string }, values: unknown[]): boolean => {
  ...
  const getCountSampleIdsFromSampleColumns = (sampleColumns: Array<{ name: string }>): string[] => {
  ...
  const getColumnsSampledDataSafe = async (
    dataset: DatasetLike,
  ...
  const getCountSampleIdsWithData = async (dataset: DatasetLike): Promise<string[]> => {
  ...
  const finalizeImport = async (
    ...
    try {
      ...
      // Link to RNA-seq project
      if (context.targetMode === 'counts') {
        setCountsDataset(context.targetProjectId, dataset.id)
      } else {
        setMetadataDataset(context.targetProjectId, dataset.id)
      }
