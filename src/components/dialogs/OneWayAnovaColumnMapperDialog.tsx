...
export function OneWayAnovaColumnMapperDialog({
  ...
  const [posthocMethod, setPosthocMethod] = useState<PostHocAdjustmentMethod>('tukey')
  ...
  const resolvedGroupLevels = useMemo(() => {
    ...
    const col = columns.find(
      (candidate) =>
        candidate.columnId === selectedGroup || candidate.columnName === selectedGroup
    )
  ...
  const handleConfirm = useCallback(() => {
    if (!isComplete || hasDuplicates || groupValidationError || dunnettValidationError || posthocQError) {
      return
    }
      ...
      <DialogContent className="max-w-lg" data-testid="one-way-anova-dialog">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1.5 border-t pt-4">
            ...
            <Select value={posthocMethod} onValueChange={(v) => setPosthocMethod(v as PostHocAdjustmentMethod)}>
              <SelectTrigger className="w-full" data-testid="one-way-adjustment-select">
          ...
          {isDunnett && resolvedGroupLevels.length > 0 && (
            <div className="space-y-1.5">
              ...
              <Select value={controlLevel} onValueChange={setControlLevel}>
                <SelectTrigger className="w-full" data-testid="one-way-control-select">
        ...
        <DialogFooter>
          ...
          <Button
            ...
            disabled={!isComplete || hasDuplicates || !!groupValidationError || !!dunnettValidationError || !!posthocQError}
...
function FieldSelector({ field, value, columns, onChange }: FieldSelectorProps) {
  ...
  const selectTestId =
    field.key === 'group' ? 'one-way-group-select' : field.key === 'outcome' ? 'one-way-outcome-select' : undefined
  ...
  return (
    <div className="space-y-1.5">
      ...
      <Select value={value || ''} onValueChange={onChange}>
        ...
        <SelectContent>
          {columns.map((col) => (
            <SelectItem key={col.columnId} value={col.columnId} data-value={col.columnId} data-label={col.columnName}>
              {col.columnName}
