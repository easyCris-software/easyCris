...
function FactorCard({
  ...
  const getEncodingPreview = (): string => {
    if (isBinary) {
      ...
      const orderedLevels = [baseline, ...sortedLevels.filter((l) => l !== baseline)]
    ...
    <div className="rounded-lg border-2 border-gray-200 bg-gray-50 p-4 space-y-3">
      ...
      {isBinary && (
        <div className="space-y-2">
          <Label htmlFor={`${factor.columnName}-baseline`} className="text-sm font-medium">
      ...
      {!isBinary && (
        <div className="text-xs text-gray-600">
          Multi-categorical factors use alphabetical encoding (baseline always = 0)
        </div>
      )}

      {/* Encoding preview */}
      {encodingPreview && (
        <div className="rounded-md bg-blue-50 border border-blue-200 p-2 text-xs text-blue-800 font-mono">
          {encodingPreview}
        </div>
      )}
    </div>
...
export function FactorEncodingDialog({
  ...
  const [baselineSelections, setBaselineSelections] = useState<Map<string, string>>(() => {
  ...
  const handleBaselineChange = useCallback((factorName: string, baseline: string) => {
   ...
   */
  const buildEncodingMappings = useCallback((): Map<string, Map<string, number>> => {
    const mappings = new Map<string, Map<string, number>>()

    factors.forEach((factor) => {
      const factorMapping = new Map<string, number>()
      const baseline = baselineSelections.get(factor.columnName) ?? factor.levels[0] ?? ''
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[85dvh] flex flex-col overflow-hidden">
        <DialogHeader>
          <DialogTitle className="text-xl font-semibold">Configure Regression Categorical Predictors</DialogTitle>
          <DialogDescription className="text-sm text-gray-600">
            Choose reference (baseline) levels for categorical predictors used in linear or logistic regression.
            Baseline = 0, all other levels become dummy variables that compare outcomes to the baseline group.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden overscroll-contain space-y-6">
          ...
          <div className="space-y-4">
            <div className="text-sm font-medium text-gray-700">Categorical Factors:</div>
            {factors.map((factor) => (
              <FactorCard
                key={factor.columnName}
                factor={factor}
                baselineLevel={baselineSelections.get(factor.columnName) ?? factor.levels[0] ?? ''}
                onBaselineChange={(level) => handleBaselineChange(factor.columnName, level)}
              />
            ))}
          </div>
          ...
          <div className="rounded-md bg-blue-50 border border-blue-200 p-3 text-sm text-blue-900">
            <p className="font-medium mb-1">Dummy Variable Encoding (Regression Only)</p>
            <p className="text-xs">
              The selected baseline level is coded as 0 for each predictor. Every other level produces a dummy column
              (1 when the level is present, 0 otherwise). In linear regression, coefficients represent mean differences
              versus baseline; in logistic regression, coefficients represent log-odds differences versus baseline.
            </p>
            <p className="text-xs mt-2">
              ANOVA workflows apply effect coding automatically, so no manual baseline selection is needed in that path.
            </p>
          </div>
        </div>
