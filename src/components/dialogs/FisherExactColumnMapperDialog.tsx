...
const FIELD_DEFINITIONS: FieldDefinition[] = [
  {
    ...
    description: 'The categorical variable defining the groups (rows in contingency table)',
    icon: Users,
  },
  {
    key: 'outcome',
    label: 'Outcome Variable',
    description: 'The categorical variable defining the outcome (columns in contingency table)',
    icon: Target,
  },
]
...
export function FisherExactColumnMapperDialog({
  ...
  const handleFieldChange = useCallback((fieldKey: keyof FisherExactColumnMapping, value: string) => {
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm text-gray-600">
            Map your selected columns to the Fisher's Exact Test inputs. Specify which column
            represents the group (rows) and which represents the outcome (columns).
          </DialogDescription>
        </DialogHeader>

        <div className="rounded-md bg-blue-50 border border-blue-200 p-3 text-sm text-blue-900">
          <p>
            <strong>Note:</strong> Fisher's Exact Test requires a 2×2 contingency table. Each
            column must have exactly 2 categories.
          </p>
        </div>
