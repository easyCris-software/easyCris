...
export function ColumnSelectionDialog({
  ...
  const scopedDataModelRows = useMemo(() => {
    if (
      !currentDataset ||
      viewScope?.datasetId !== currentDataset.id ||
      viewScope.source !== 'view-filter' ||
      viewScope.dataModelRows === null
    ) {
      return null
    }
    const dataRowCount =
    ...
    return viewScope.dataModelRows.filter((row) => row >= 0 && row < dataRowCount)
  }, [currentDataset, viewScope])
  ...
  useEffect(() => {
    ...
    const effectiveRowCount =
      typeof currentDataset.dataRowCount === 'number' && currentDataset.dataRowCount > 0
        ? currentDataset.dataRowCount
        : currentDataset.rowCount
    const activeScopedRows = scopedDataModelRows
    ...
    const loadColumnData = async () => {
      try {
        ...
        if (activeScopedRows) {
          ...
          if (isStale()) return
          const dataMap = new Map<number, Record<string, unknown>>()
          for (const rowIdx of activeScopedRows) {
  ...
  }, [currentDataset, isOpen, scopedDataModelRows])
  ...
  useEffect(() => {
    ...
    for (const col of currentDataset.columns) {
      ...
      const detectedClassification = scopedDataModelRows
        ? classifyColumn(col.id, col.name, rowData)
        : stats
