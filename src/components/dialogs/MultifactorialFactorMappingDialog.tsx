...
export function MultifactorialFactorMappingDialog({
  ...
  const handleAddFacet = useCallback(() => {
    const available = getAvailableForFacets.filter((col) => !facets.includes(col.columnId))
      ...
      <DialogContent className="max-w-lg max-h-[90vh] flex flex-col overflow-hidden">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm text-gray-600">
            Choose which factors should be primary (x-axis), secondary (grouping), and facets
            (panels) for Multifactorial ANOVA analysis and plots.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden overscroll-contain space-y-4">
          ...
          <div className="rounded-md bg-blue-50 border border-blue-200 p-3 text-sm text-blue-900">
            <p>
              <strong>Tip:</strong> Primary factor appears on the x-axis, secondary factor defines
              the groups/colors, and facet factors split the plot into separate panels. This
              assignment affects plot layout and interaction interpretation.
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="primary" className="flex items-center gap-2 font-medium">
                <Grid3x3 className="h-4 w-4 text-gray-500" />
                Primary Factor (X-Axis)
              </Label>
              <p className="text-xs text-gray-500 -mt-1">Main independent variable on x-axis</p>
            ...
            {/* Secondary Factor */}
            <div className="space-y-2">
              <Label htmlFor="secondary" className="flex items-center gap-2 font-medium">
            ...
            <div className="space-y-2">
                ...
                <div key={index} className="flex gap-2 items-center">
                  ...
                  >
                    <SelectTrigger className="flex-1">
                      <SelectValue placeholder={`Select facet ${index + 1}...`} />
                    </SelectTrigger>
                    <SelectContent>
                      {getAvailableForFacets
                        .filter((col) => !facets.includes(col.columnId) || col.columnId === facetId)
                        .map((col) => (
                          <SelectItem key={col.columnId} value={col.columnId}>
                            {col.columnName}
                          </SelectItem>
              ...
              {getAvailableForFacets.filter((col) => !facets.includes(col.columnId)).length > 0 && (
                <Button
                  variant="outline"
                  size="sm"
