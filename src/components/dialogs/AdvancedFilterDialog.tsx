...
export function AdvancedFilterDialog({
  ...
  // Get unique values for each column (memoized)
  ...
  // Compute preview match count
  const previewMatchCount = useMemo(() => {
    if (!data.length) return null

    // Only compute if all conditions are valid
    ...
    try {
      ...
      return DataTransformService.countFilterMatches(data, config)
    } catch {
      return null
    }
  }, [groups, groupOperator, data])
      ...
      <ResizableDialogContent className="flex flex-col p-0">
            ...
            <div key={groupIndex} className="rounded-md border p-4 space-y-3">
              ...
              {group.conditions.map((condition, conditionIndex) => {
                const uniqueInfo = condition.columnId
                  ? columnUniqueValues.get(condition.columnId)
                  : undefined
                ...
                return (
                  <div key={conditionIndex} className="space-y-2">
                    ...
                    {uniqueValues && uniqueValues.length > 0 && needsValue && (
        ...
        <ResizableDialogFooter className="px-6 py-4 border-t">
          <div className="w-full space-y-3">
            ...
            {/* Preview match count */}
            {previewMatchCount !== null && data.length > 0 && (
              <div className="flex items-center justify-center gap-2 text-sm">
                {previewMatchCount === 0 ? (
                  <>
                    <AlertCircle className="h-4 w-4 text-amber-500" />
                    <span className="text-amber-600 dark:text-amber-400">
                      No rows match in preview (0 of {data.length.toLocaleString()} sample rows)
                    </span>
                  </>
                ) : (
                  <span className="text-muted-foreground">
                    Preview (sample): <strong>{previewMatchCount.toLocaleString()}</strong> of{' '}
                    {data.length.toLocaleString()} rows match
                  </span>
                )}
              </div>
            )}
