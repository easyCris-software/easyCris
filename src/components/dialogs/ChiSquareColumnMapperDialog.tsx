...
const FIELD_DEFINITIONS: FieldDefinition[] = [
  {
    ...
    description:
      'Categorical grouping variable (rows in the contingency table)',
    icon: Rows,
  },
  {
    key: 'outcome',
    ...
    description:
      'Categorical outcome variable (columns in the contingency table)',
    icon: Columns,
  },
]
...
export function ChiSquareColumnMapperDialog({
  ...
  const getAvailableColumns = useCallback(
    (fieldKey: keyof ChiSquareColumnMapping) => {
      ...
      return categoricalColumns.filter(
        col => !selectedValues.includes(col.columnId)
      )
    },
  ...
  const handleFieldChange = useCallback(
    (fieldKey: keyof ChiSquareColumnMapping, value: string) => {
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription className={dialogDescriptionClass}>
            Map your selected categorical columns to the row and column
            variables used to build the contingency table.
          </DialogDescription>
        </DialogHeader>

        <div className={dialogInfoPanelClass}>
          <p>
            <strong>Tip:</strong> Group determines the table rows, and Outcome
            determines the table columns. Swap them if you want a different
            orientation.
          </p>
        </div>
