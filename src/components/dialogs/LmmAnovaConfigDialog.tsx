...
export interface LmmTrajectoryRoles {
  /** Column ID of the treatment/group factor (drives line dash and x-axis comparison) */
  treatmentFactorId: string
  /** Column ID of the time/repeated-measures factor (drives x-axis values) */
  timeFactorId: string
  /** Level of the treatment factor that is the reference/control group (dot line) */
  treatmentReferenceLevel?: string
}

export interface LmmPlotFacetRoles {
  /**
   * Column ID of the strata factor used as the panel/facet dimension.
   * Facet controls how the plot is split into sub-panels.
   */
  facetBy?: string
  /**
   * Column ID of the strata factor used as the color/trace grouping dimension.
   * Color controls which strata levels get different trace colors.
   */
  colorBy?: string
  /**
   * Column ID of the single strata factor used as the plot title dimension.
   * It appears in the subplot title and is excluded from the Facet/Color pickers.
   */
  titleBy?: string
}
...
export interface LmmAnovaConfig {
  ...
  /** Explicit trajectory plot role contract — overrides simple_effects[0] for plot building */
  trajectoryRoles?: LmmTrajectoryRoles
  /** Explicit facet / color assignment for compound trajectory plots */
  plotFacetRoles?: LmmPlotFacetRoles
}
...
const GROUPING_PATTERN =
  /\b(strain|sex|gender|race|ethnicity|genotype|group|batch|cohort|site|center|location|tissue|background|lineage|region|ancestry)\b/i
// Columns whose names suggest they are the comparison / treatment axis — excluded from auto-selection
const COMPARISON_PATTERN =
  /\b(treatment|condition|drug|dose|intervention|therapy|control|stimulus|vehicle|placebo|trt|diet|group|arm)\b/i
...
export function isLikelyGroupingFactor(name: string): boolean {
...
export function LmmAnovaConfigDialog({
  ...
  // Trajectory role assignment (explicit contract for plot building)
  const [trajectoryTreatmentFactorId, setTrajectoryTreatmentFactorId] = useState('')
  const [trajectoryTimeFactorId, setTrajectoryTimeFactorId] = useState('')
  const [trajectoryReferenceLevelId, setTrajectoryReferenceLevelId] = useState('')
  // Facet / Color / Title role assignment for compound trajectory plots
  ...
  useEffect(() => {
    ...
    if (columns.length === 0) {
      ...
      setTrajectoryTreatmentFactorId('')
      setTrajectoryTimeFactorId('')
      setTrajectoryReferenceLevelId('')
      setTitleBy('')
      setFacetBy('')
      setColorBy('')
      return
    ...
    // Trajectory roles always start empty — user explicitly chooses treatment and time factors.
    setTrajectoryTreatmentFactorId('')
    setTrajectoryTimeFactorId('')
    setTrajectoryReferenceLevelId('')
    // Facet/Color/Title roles always start empty — user explicitly assigns them when strata >= 2.
    setTitleBy('')
    setFacetBy('')
    setColorBy('')
  ...
  // Trajectory role assignment — only relevant when stratified with ≥2 categorical predictors
  const trajectoryRoleEligible =
    effectiveStratifyIds.length > 0 && withinModelCategoricalPredictors.length >= 2

  // Show a gate hint when user has strata but hasn't met the ≥2 categorical predictor requirement.
  const trajectoryGateHintVisible =
    !trajectoryRoleEligible && effectiveStratifyIds.length > 0

  // Available categorical predictors for role assignment (must not be in stratifyBy)
  const trajectoryRoleCandidates = withinModelCategoricalPredictors

  // Reference levels for the selected treatment factor
  const trajectoryTreatmentColumn = useMemo(
    () => availablePredictors.find(c => getColumnKey(c) === trajectoryTreatmentFactorId),
    [availablePredictors, trajectoryTreatmentFactorId]
  )
  const trajectoryTreatmentLevels: string[] = useMemo(
    () => (trajectoryTreatmentColumn?.uniqueValues ?? []).map(String),
    [trajectoryTreatmentColumn]
  )

  const trajectoryRolesValidationError = useMemo(() => {
    if (!trajectoryRoleEligible) return null
    if (!trajectoryTreatmentFactorId) return 'Choose a treatment factor for the trajectory plot.'
    if (!trajectoryTimeFactorId) return 'Choose a time factor for the trajectory plot.'
    if (trajectoryTreatmentFactorId === trajectoryTimeFactorId) {
      return 'Treatment factor and time factor must be different predictors.'
    }
    return null
  }, [trajectoryRoleEligible, trajectoryTreatmentFactorId, trajectoryTimeFactorId])

  // Facet / Color role eligibility: only shown when 2+ strata selected
  const facetColorEligible = effectiveStratifyIds.length >= 2

  // Assignable strata = selected strata that are NOT the titleBy factor
  const assignableStratifyIds = useMemo(
    () => effectiveStratifyIds.filter(id => id !== titleBy),
    [effectiveStratifyIds, titleBy]
  )

  // Sanitize facet/color/title roles whenever strata selection changes:
  // - Clear stale IDs (removed from strata or now conflicting)
  // - Clear titleBy if it equals facetBy or colorBy
  // - Clear if facetBy === colorBy
  useEffect(() => {
    if (!facetColorEligible) {
      if (titleBy || facetBy || colorBy) {
        setTitleBy('')
        setFacetBy('')
        setColorBy('')
      }
    ...
    const stratifySet = new Set(effectiveStratifyIds)
    // Clear titleBy when stale (removed from strata) or when it conflicts with facetBy/colorBy.
    // Dropdown disabling is first-line prevention; this effect is the authoritative fallback.
    if (titleBy && (!stratifySet.has(titleBy) || titleBy === facetBy || titleBy === colorBy)) {
      setTitleBy('')
    }
    const assignableSet = new Set(assignableStratifyIds)
    const facetStale = !!facetBy && !assignableSet.has(facetBy)
    const colorStale = !!colorBy && !assignableSet.has(colorBy)
    const conflict = !!facetBy && facetBy === colorBy

    if (facetStale) setFacetBy('')
    if (colorStale) setColorBy('')
    if (conflict && !facetStale) setColorBy('')
  }, [assignableStratifyIds, effectiveStratifyIds, facetColorEligible, titleBy, facetBy, colorBy])
  ...
  // Unified trajectory-role normalizer.
  // Fires on any candidate-set change and self-heals four problem classes:
  // Trajectory-role normalizer: heals two problem classes only.
  //   1. Stale ID — predictor removed from model/strata: clear it and fill from remaining candidate.
  //   2. Same-ID conflict — both roles resolve to the same predictor: reassign time to alternate.
  // Does NOT auto-fill when selects are empty — user must explicitly choose their roles.
  useEffect(() => {
    if (trajectoryRoleCandidates.length < 2) return

    const eligibleIds = new Set(trajectoryRoleCandidates.map(getColumnKey))
    const treatmentStale = !!trajectoryTreatmentFactorId && !eligibleIds.has(trajectoryTreatmentFactorId)
    const timeStale      = !!trajectoryTimeFactorId      && !eligibleIds.has(trajectoryTimeFactorId)
    const sameId         = !!trajectoryTreatmentFactorId && trajectoryTreatmentFactorId === trajectoryTimeFactorId

    // Only run when there is an actual problem to fix (stale ID or same-ID conflict).
    // Empty selects are intentional — user picks their own roles; do not auto-fill.
    if (!treatmentStale && !timeStale && !sameId) return

    let nextTreatId = treatmentStale ? '' : trajectoryTreatmentFactorId
    let nextTimeId  = timeStale      ? '' : trajectoryTimeFactorId

    // After clearing a stale slot, fill it from the remaining eligible candidate
    if (!nextTreatId && nextTimeId) {
      nextTreatId = getColumnKey(
        trajectoryRoleCandidates.find(c => getColumnKey(c) !== nextTimeId) ?? trajectoryRoleCandidates[0]!
      )
    } else if (nextTreatId && !nextTimeId) {
      nextTimeId = getColumnKey(
        trajectoryRoleCandidates.find(c => getColumnKey(c) !== nextTreatId)
          ?? trajectoryRoleCandidates[trajectoryRoleCandidates.length - 1]!
      )
    }

    // Repair same-ID conflict: reassign time to the first alternate candidate
    if (nextTreatId && nextTreatId === nextTimeId) {
      const alternate = trajectoryRoleCandidates.find(c => getColumnKey(c) !== nextTreatId)
      nextTimeId = alternate ? getColumnKey(alternate) : ''
    }

    if (nextTreatId !== trajectoryTreatmentFactorId) {
      setTrajectoryTreatmentFactorId(nextTreatId)
      if (treatmentStale) setTrajectoryReferenceLevelId('')
    }
    if (nextTimeId !== trajectoryTimeFactorId) {
      setTrajectoryTimeFactorId(nextTimeId)
    }
  }, [trajectoryRoleCandidates, trajectoryTreatmentFactorId, trajectoryTimeFactorId])
  ...
  const validationMessage = useMemo(() => {
    ...
    if (trajectoryRolesValidationError) return trajectoryRolesValidationError
    return null
  }, [
    ...
    trivialStrataCols,
    trajectoryRolesValidationError,
  ])
    ...
    onConfirm({
      ...
      config: {
        ...
        trajectoryRoles:
          trajectoryRoleEligible && trajectoryTreatmentFactorId && trajectoryTimeFactorId
            ? {
                treatmentFactorId: trajectoryTreatmentFactorId,
                timeFactorId: trajectoryTimeFactorId,
                treatmentReferenceLevel: trajectoryReferenceLevelId || undefined,
              }
            : undefined,
        plotFacetRoles:
          facetColorEligible && (facetBy || colorBy || titleBy)
            ? {
                facetBy: facetBy || undefined,
                colorBy: colorBy || undefined,
                titleBy: titleBy || undefined,
              }
            : undefined,
      },
    })
      ...
      >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto divide-y divide-zinc-100">
          ...
          <section className="px-7 py-5">
            ...
            {/* ── FACET / COLOR ROLE ASSIGNMENT ── shown only when ≥ 2 strata selected */}
            {facetColorEligible && (
              <div className="mt-4 rounded-xl border border-zinc-200 bg-zinc-50/60 p-4 space-y-3" data-testid="lmm-facet-color-block">
                <div>
                  <p className="text-xs font-semibold text-zinc-700 mb-0.5">Compound Plot Layout</p>
                  <p className="text-[11px] text-zinc-400">
                    Facet controls panel split. Color controls trace grouping.
                  </p>
                </div>

                {/* Three-dropdown layout: Title by / Facet by / Color by */}
                <div className="grid grid-cols-3 gap-3">
                  <div className="flex flex-col gap-1">
                    ...
                    <select
                      className="w-full rounded-lg border border-zinc-200 bg-white px-2.5 py-1.5 text-xs text-zinc-800 focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500"
                      data-testid="lmm-title-by-select"
                      value={titleBy}
                      onChange={e => setTitleBy(e.target.value)}
                    >
                      <option value="">None</option>
                      {effectiveStratifyIds.map(id => {
                        const col = stratifyEligiblePredictors.find(c => getColumnKey(c) === id)
                        if (!col) return null
                        return (
                          <option
                            key={id}
                            value={id}
                            disabled={id === facetBy || id === colorBy}
                          >
                            {col.columnName}
                            {id === facetBy ? ' (used as Facet)' : id === colorBy ? ' (used as Color)' : ''}
                          </option>
                        )
                      })}
                    </select>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <select
                      className="w-full rounded-lg border border-zinc-200 bg-white px-2.5 py-1.5 text-xs text-zinc-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                      data-testid="lmm-facet-by-select"
                      value={facetBy}
                      onChange={e => setFacetBy(e.target.value)}
                    >
                      <option value="">Auto</option>
                      {assignableStratifyIds.map(id => {
                        const col = stratifyEligiblePredictors.find(c => getColumnKey(c) === id)
                        if (!col) return null
                        return (
                          <option
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <select
                      className="w-full rounded-lg border border-zinc-200 bg-white px-2.5 py-1.5 text-xs text-zinc-800 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                      data-testid="lmm-color-by-select"
                      value={colorBy}
          ...
          {/* ── TRAJECTORY ROLE GATE HINT ── */}
          {trajectoryGateHintVisible && (
            <section className="px-7 py-4 border-t border-zinc-100">
              {sectionHead('TR', 'Trajectory Plot Roles')}
              <p
                className="text-xs text-zinc-400 italic mt-1"
                data-testid="lmm-trajectory-gate-hint"
              >
                Requires 2 categorical model predictors and at least 1 stratification factor.
              </p>
            </section>
          )}

          {/* ── TRAJECTORY ROLE ASSIGNMENT ── */}
          {trajectoryRoleEligible && (
            <section className="px-7 py-5 border-t border-zinc-100">
              {sectionHead('TR', 'Trajectory Plot Roles')}
              <p className="text-xs text-zinc-500 mb-4">
                Specify which predictor is the treatment axis and which is the time axis for the trajectory plot.
              </p>
              <div className="flex flex-col gap-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1.5">
                    <label className={labelCapCls}>Treatment factor</label>
                    <select
                      className={selectCls}
                      data-testid="lmm-trajectory-treatment-factor"
                      value={trajectoryTreatmentFactorId}
                      onChange={e => {
                        setTrajectoryTreatmentFactorId(e.target.value)
                        setTrajectoryReferenceLevelId('')
                      }}
                    >
                      <option value="">Select treatment factor...</option>
                      {trajectoryRoleCandidates.map(c => (
                        <option
                          key={getColumnKey(c)}
                          value={getColumnKey(c)}
                          disabled={getColumnKey(c) === trajectoryTimeFactorId}
                        >
                          {c.columnName}
                        </option>
                      ))}
                  ...
                  <div className="flex flex-col gap-1.5">
                    <label className={labelCapCls}>Time factor</label>
                    <select
                      className={selectCls}
                      data-testid="lmm-trajectory-time-factor"
                      value={trajectoryTimeFactorId}
                      onChange={e => setTrajectoryTimeFactorId(e.target.value)}
                    >
                      <option value="">Select time factor...</option>
                      {trajectoryRoleCandidates.map(c => (
                        <option
                          key={getColumnKey(c)}
                          value={getColumnKey(c)}
                          disabled={getColumnKey(c) === trajectoryTreatmentFactorId}
                        >
                          {c.columnName}
                        </option>
                      ))}
                ...
                {trajectoryTreatmentLevels.length > 0 && (
                  <div className="flex flex-col gap-1.5">
                    <label className={labelCapCls}>Reference / control level (optional)</label>
                    <select
                      className={selectCls}
                      data-testid="lmm-trajectory-reference-level"
                      value={trajectoryReferenceLevelId}
                      onChange={e => setTrajectoryReferenceLevelId(e.target.value)}
                    >
                      <option value="">Auto-detect from model</option>
                      {trajectoryTreatmentLevels.map(level => (
                        <option key={level} value={level}>{level}</option>
                      ))}
                    </select>
                    <p className="text-[11px] text-zinc-400">
                      The reference level is shown as a dotted line; all others as solid lines.
                    </p>
                  </div>
                )}
              </div>
