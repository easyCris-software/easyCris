/**
 * Independent T-Test Column Mapper Dialog
 *
 ...
 */
export interface IndependentTTestColumnInfo {
  columnName: string
  columnId: string
...
interface IndependentTTestColumnMapperDialogProps {
  open: boolean
  columns: IndependentTTestColumnInfo[]
  testName: string
  onConfirm: (mapping: IndependentTTestColumnMapping) => void
  onCancel: () => void
}

interface FieldDefinition {
  key: keyof IndependentTTestColumnMapping
  label: string
  description: string
  icon: typeof Users
  filterFn: (col: IndependentTTestColumnInfo) => boolean
}

export function IndependentTTestColumnMapperDialog({
  open,
  columns,
  testName,
  onConfirm,
  onCancel,
}: IndependentTTestColumnMapperDialogProps) {
  const [mapping, setMapping] = useState<Partial<IndependentTTestColumnMapping>>({})
      ...
      return [
        {
          ...
          icon: BarChart3,
          filterFn: (col: IndependentTTestColumnInfo) =>
            col.dataType === ColumnDataType.Numeric || col.dataType === ColumnDataType.Ordinal,
        },
        {
    ...
    return [
      {
        ...
        description:
          'The categorical variable defining the two groups (e.g., Treatment: Control/Drug)',
        icon: Users,
        filterFn: (col: IndependentTTestColumnInfo) =>
          col.dataType === ColumnDataType.Categorical || col.dataType === ColumnDataType.Binary,
      },
  ...
  const getAvailableColumns = useCallback(
    (fieldKey: keyof IndependentTTestColumnMapping) => {
  ...
  const handleFieldChange = useCallback(
    (fieldKey: keyof IndependentTTestColumnMapping, value: string) => {
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm text-gray-600">
            {isWideFormat
              ? 'Independent T-Test in wide format: choose the two numeric columns representing the groups.'
              : 'Map your selected columns to the Independent T-Test inputs. Specify which column is the grouping variable and which is the outcome (numeric) variable.'}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="rounded-md bg-blue-50 border border-blue-200 p-3 text-sm text-blue-900">
          <p>
            ...
            {isWideFormat
              ...
              : 'Independent T-Test compares the means of two groups. The group variable must have exactly 2 categories (e.g., Control vs Treatment).'}
          </p>
        </div>
