...
export function SimpleEffectsDialog({
  ...
  const [adjustmentMethod, setAdjustmentMethod] = useState<PostHocAdjustmentMethod>('tukey')
  ...
  const handleConfirm = useCallback(() => {
    ...
    onConfirm({
      ...
      controlLevels: Object.keys(controlLevels).length > 0 ? controlLevels : undefined,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
        ...
        <DialogHeader className="px-6 pt-6 pb-3 border-b border-gray-200 shrink-0">
          <DialogTitle className="text-xl font-semibold">Simple Effects (Two-Way ANOVA)</DialogTitle>
          <DialogDescription className="text-sm text-gray-600">
            Configure optional follow-up tests when your two-factor design shows a significant interaction.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-md bg-blue-50 border border-blue-200 p-4 text-sm text-blue-900">
            ...
            <p className="text-xs leading-relaxed">
              When a significant interaction is found, simple effects help interpret WHERE the
              interaction occurs. Instead of testing the overall effect of each factor, simple
              effects test one factor at each level of the other factor.
            </p>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-start space-x-3 p-4 rounded-lg border-2 border-gray-200 hover:border-blue-300 transition-colors">
              <Checkbox
                id="factorAWithinB"
                checked={factorAWithinB}
                onCheckedChange={(checked) => setFactorAWithinB(checked === true)}
              />
              <div className="grid gap-1.5 leading-none flex-1">
                ...
                >
                  Analyze <span className="text-blue-700 font-semibold">{factor1Name}</span> within
                  each level of <span className="text-purple-700 font-semibold">{factor2Name}</span>
                </Label>
                <p className="text-xs text-gray-600 leading-relaxed">
                  Tests whether the effect of {factor1Name} is different at each level of{' '}
                  {factor2Name}. Uses the selected adjustment method for pairwise comparisons.
                </p>
                <p className="text-xs text-gray-500 italic mt-1">
                  Example: "Is {factor1Name} effect significant when {factor2Name} = Level1? When{' '}
                  {factor2Name} = Level2?"
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start space-x-3 p-4 rounded-lg border-2 border-gray-200 hover:border-blue-300 transition-colors">
              <Checkbox
                ...
                onCheckedChange={(checked) => setFactorBWithinA(checked === true)}
              />
              <div className="grid gap-1.5 leading-none flex-1">
                <Label
                  ...
                  className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 cursor-pointer"
                >
                  Analyze <span className="text-purple-700 font-semibold">{factor2Name}</span>{' '}
                  within each level of{' '}
                  <span className="text-blue-700 font-semibold">{factor1Name}</span>
                </Label>
                <p className="text-xs text-gray-600 leading-relaxed">
                  Tests whether the effect of {factor2Name} is different at each level of{' '}
                  {factor1Name}. Uses the selected adjustment method for pairwise comparisons.
                </p>
                <p className="text-xs text-gray-500 italic mt-1">
                  Example: "Is {factor2Name} effect significant when {factor1Name} = Level1? When{' '}
                  {factor1Name} = Level2?"
                </p>
              </div>
            </div>
          ...
          <div className="space-y-2 rounded-md border border-gray-200 p-4">
            ...
            >
              <SelectTrigger className="w-full" data-testid="two-way-adjustment-select">
            ...
            <p className="text-xs text-gray-600">
              Tukey HSD is the default for ANOVA. Dunnett requires a control level for each factor.
            </p>
            ...
            {isDunnett && (
              <div className="space-y-3 pt-2">
                <div className="space-y-1.5">
                  ...
                  <Select value={controlFactor1} onValueChange={setControlFactor1}>
                    <SelectTrigger className="w-full" data-testid="two-way-control-factor1-select">
                      <SelectValue placeholder={`Select control for ${factor1Name}`} />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={controlFactor2} onValueChange={setControlFactor2}>
                    <SelectTrigger className="w-full" data-testid="two-way-control-factor2-select">
                      <SelectValue placeholder={`Select control for ${factor2Name}`} />
                    </SelectTrigger>
                    <SelectContent>
            ...
            {dunnettValidationError && (
              <p className="text-xs text-red-600">{dunnettValidationError}</p>
            )}
          </div>
          ...
          <div className="rounded-md bg-amber-50 border border-amber-200 p-3 text-sm text-amber-900">
            ...
            <p className="text-xs">
              Request simple effects when interaction terms are significant or when decision-makers expect detailed results.
              If no interaction is detected, most analysts skip this step and rely on main effects. Choose zero, one, or both options - simple effects are optional diagnostics.
            </p>
          </div>
        </div>
        </div>
        ...
        <DialogFooter className="px-6 py-4 border-t border-gray-200 shrink-0">
          ...
          <Button onClick={handleConfirm} disabled={Boolean(dunnettValidationError || posthocQError)}>
            {factorAWithinB || factorBWithinA ? 'Continue with Simple Effects' : 'Skip Simple Effects'}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
