...
const FIELD_DEFINITIONS: FieldDefinition[] = [
  {
    ...
    description: 'The categorical variable measured before the intervention (rows in contingency table)',
    icon: Clock,
  },
  {
    key: 'after',
    label: 'After (Post-Treatment)',
    description: 'The categorical variable measured after the intervention (columns in contingency table)',
    icon: ArrowRight,
  },
]
...
export function McNemarColumnMapperDialog({
  ...
  const isComplete = useMemo(() => {
    return !!mapping.before && !!mapping.after && mapping.before !== mapping.after
  }, [mapping])

  // Check for duplicates
  const hasDuplicates = useMemo(() => {
    return Boolean(mapping.before && mapping.after && mapping.before === mapping.after)
  }, [mapping])
  ...
  const handleFieldChange = useCallback((fieldKey: keyof McNemarColumnMapping, value: string) => {
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm text-gray-600">
            Map your selected columns to the McNemar's Test inputs. Specify which column
            represents the before (pre-treatment) and which represents the after (post-treatment) measurement.
          </DialogDescription>
        </DialogHeader>

        <div className="rounded-md bg-blue-50 border border-blue-200 p-3 text-sm text-blue-900">
          <p>
            <strong>Note:</strong> McNemar's Test requires paired binary data. Each column must have
            exactly 2 categories with identical labels (e.g., Yes/No measured before and after treatment).
          </p>
        </div>
