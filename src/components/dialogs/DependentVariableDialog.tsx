 ...
 *
 * Reference: easyCris.Avalonia/Views/StatisticalAnalysis/DependentVariableDialog.axaml.cs
...
function getColumnSuitability(
  ...
  const isCategorical =
    column.dataType === 'Categorical' || column.dataType === 'Binary'
    ...
    case DependentVariableDialogMode.RegressionCategoricalOutcome:
      if (isCategorical) {
        ...
        return {
          ...
          message:
            '⚠ Numeric value (logistic regression expects a categorical/binary outcome)',
        }
      }
...
export function DependentVariableDialog({
  ...
  const [selectedVariable, setSelectedVariable] = useState<string>(() => {
    ...
    return (
      sortedColumns.find(c => getColumnSuitability(c, mode).level === 'ideal')
        ?.columnName ??
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[80dvh] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden overscroll-contain space-y-6">
          ...
          <div
            className={`rounded-md p-3 text-sm ${secondaryInstruction.color}`}
          >
                    ...
                    <div
                      ...
                      className={`rounded-lg border-2 p-4 transition-colors ${
                        isIdeal
                          ...
                          : isUnsuitable
                            ? 'bg-red-50 dark:bg-red-950/30 border-red-200 dark:border-red-800 opacity-60'
                      ...
                      <div className="flex items-start space-x-3">
                        {isUnsuitable ? (
                          <Tooltip>
                            ...
                            <TooltipContent>
                              <p className="text-sm">
                                This column type is unsuitable for this analysis
                                type.
                              </p>
                            </TooltipContent>
                          </Tooltip>
                        ...
                        <div className="flex-1 space-y-2">
                          <Label
                            ...
                            className={`font-semibold text-base ${isUnsuitable ? 'cursor-not-allowed text-muted-foreground' : 'cursor-pointer'}`}
                          >
                            {column.columnName}
                          </Label>
                          ...
                          {/* Type info with icon */}
                          <div
                            className={`flex items-center space-x-2 text-sm ${suitability.color}`}
                          >
          ...
          <div className="rounded-md bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800 p-3 text-sm text-blue-900 dark:text-blue-200">
            <p className="italic">
              ...
              {mode ===
                DependentVariableDialogMode.RegressionCategoricalOutcome &&
