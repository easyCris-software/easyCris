/**
 * Paired T-Test Column Mapper Dialog
 *
 ...
 */
export interface PairedTTestColumnInfo {
  columnName: string
  columnId: string
...
interface PairedTTestColumnMapperDialogProps {
  open: boolean
  columns: PairedTTestColumnInfo[]
  testName: string
  onConfirm: (mapping: PairedTTestColumnMapping) => void
  onCancel: () => void
}

interface FieldDefinition {
  key: keyof PairedTTestColumnMapping
  label: string
  description: string
  icon: typeof Clock
  filterFn: (col: PairedTTestColumnInfo) => boolean
}

export function PairedTTestColumnMapperDialog({
  open,
  columns,
  testName,
  onConfirm,
  onCancel,
}: PairedTTestColumnMapperDialogProps) {
  const [mapping, setMapping] = useState<Partial<PairedTTestColumnMapping>>({})
  ...
  const FIELD_DEFINITIONS: FieldDefinition[] = useMemo(() => {
      ...
      return [
        {
          ...
          icon: BarChart3,
          filterFn: (col: PairedTTestColumnInfo) =>
            col.dataType === ColumnDataType.Numeric || col.dataType === ColumnDataType.Ordinal,
        },
        {
    ...
    return [
      ...
      },
      {
        ...
        description: 'Identifier that links the paired observations (e.g., Subject ID)',
        icon: Clock,
        filterFn: () => true,
      },
    ]
  ...
  const getAvailableColumns = useCallback(
    (fieldKey: keyof PairedTTestColumnMapping) => {
  ...
  const isComplete = useMemo(() => {
    return (
      ...
      (isWideFormat || (mapping.group !== mapping.pair_id && mapping.outcome !== mapping.pair_id))
    )
  ...
  const handleFieldChange = useCallback(
    (fieldKey: keyof PairedTTestColumnMapping, value: string) => {
      ...
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm text-gray-600">
            {isWideFormat
              ? 'Paired T-Test in wide format: choose the two numeric columns that form each pair.'
              : 'Map your selected columns to the Paired T-Test inputs. Specify which column is the time/condition variable and which is the outcome (numeric) variable.'}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="rounded-md bg-blue-50 border border-blue-200 p-3 text-sm text-blue-900">
          <p>
            ...
            {isWideFormat
              ...
              : 'Paired T-Test compares matched pairs (e.g., before/after measurements). The time/condition variable must have exactly 2 values. Pair/Subject ID is optional; if omitted, pairing uses row order within each time point.'}
          </p>
        </div>
