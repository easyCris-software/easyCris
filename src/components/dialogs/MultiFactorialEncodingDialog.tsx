 ...
 *
 * Reference: C:\Users\RajLord_new\Desktop\Bmad_project\easyCris.Avalonia\Views\StatisticalAnalysis\MultiFactorialEncodingDialog.axaml.cs
 * Lines: 285 lines
 *
 * Features:
 ...
 */
function calculateTermCount(factorCount: number, maxInteractionDepth: number): number {
 ...
 */
function getInteractionDepthDescription(depth: number, factorCount: number): string {
...
export function MultiFactorialEncodingDialog({
  ...
  const [maxInteractionDepth, setMaxInteractionDepth] = useState<number>(smartDefaultDepth)
  ...
  const [simpleEffectsMap, setSimpleEffectsMap] = useState<Map<string, Set<string>>>(() => {
  ...
  const interactionDepthDescription = useMemo(() => {
    return getInteractionDepthDescription(maxInteractionDepth, selectedFactorsList.length)
  }, [maxInteractionDepth, selectedFactorsList.length])

  const handleFactorToggle = useCallback((factorName: string, checked: boolean) => {
  ...
  const handleConfirm = useCallback(() => {
    ...
    onOpenChange(false)
  }, [maxInteractionDepth, selectedFactors, buildSimpleEffects, onConfirm, onOpenChange])
      ...
      <DialogContent className="max-w-4xl max-h-[90dvh] flex flex-col overflow-hidden">
        <DialogHeader>
          ...
          <DialogDescription className="text-sm text-gray-600">
            Configure interaction depth and simple effects for {factors.length}-factor ANOVA
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden overscroll-contain space-y-6">
          ...
          <div className="space-y-3">
            <div className="text-sm font-medium text-gray-700">Factors (uncheck to exclude):</div>
            <div className="space-y-2">
              {factors.map((factor) => (
                <div
                  key={factor.columnName}
                  className="flex items-center space-x-3 rounded-md border border-gray-200 bg-gray-50 p-3"
                >
                  <Checkbox
          ...
          <div className="rounded-lg border-2 border-green-200 bg-green-50 p-4 space-y-3">
            <div className="font-semibold text-base text-green-900">Interaction Depth</div>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="0">Full model (all interactions)</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-1 text-sm">
              <div className="text-green-800">{interactionDepthDescription}</div>
          ...
          {selectedFactorsList.length >= 2 && (
            <div className="rounded-lg border-2 border-purple-200 bg-purple-50 p-4 space-y-3">
              <div>
                <div className="font-semibold text-base text-purple-900">Simple Effects</div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 max-h-64 overflow-y-auto">
                {simpleEffectsCombinations.map(({ main, within }) => {
                  const isChecked = simpleEffectsMap.get(main)?.has(within) ?? false

                  return (
                    <div key={`${main}-within-${within}`} className="flex items-start space-x-2">
                      <Checkbox
                        id={`se-${main}-${within}`}
                        checked={isChecked}
                        onCheckedChange={(checked) =>
                          handleSimpleEffectToggle(main, within, checked === true)
                        }
                      />
                      <label
                      ...
                      >
                        <span className="font-medium">{main}</span> within each level of{' '}
          ...
          <div className="rounded-md bg-blue-50 border border-blue-200 p-3 text-sm text-blue-900">
            ...
            <p className="text-xs">
              ANOVA uses effect coding (no explicit baseline selection). The interaction depth
              controls model complexity: main effects only is simplest, full model tests all
              possible interactions. Simple effects help interpret significant interactions by
              testing one factor at each level of another.
            </p>
          </div>
        </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleConfirm} disabled={selectedFactorsList.length < 2}>
            OK
          </Button>
        </DialogFooter>
      </DialogContent>
