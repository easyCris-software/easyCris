...
export function BottomLeftTip() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      const activeTipIndex = TIPS.findIndex((tip) => {
  ...
  useEffect(() => {
    ...
    const unsubscribeResults = useResultsStore.subscribe(() => {
      const activeTipIndex = TIPS.findIndex((tip) => {
        const dismissed = localStorage.getItem(`tip-dismissed-${tip.id}`) === 'true'
        const conditionMet = tip.condition()
        return !dismissed && conditionMet
    ...
    const unsubscribePlots = usePlotsStore.subscribe(() => {
      const activeTipIndex = TIPS.findIndex((tip) => {
        const dismissed = localStorage.getItem(`tip-dismissed-${tip.id}`) === 'true'
        const conditionMet = tip.condition()
        return !dismissed && conditionMet
  ...
  const handleNextTip = () => {
    ...
    // Find next tip
    const nextTipIndex = TIPS.findIndex((tip, idx) => {
      if (idx <= currentTipIndex) return false
      const dismissed = localStorage.getItem(`tip-dismissed-${tip.id}`) === 'true'
      const conditionMet = tip.condition()
      return !dismissed && conditionMet
    })
