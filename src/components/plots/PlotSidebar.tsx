...
import { applyLmmTrajectoryOverride } from '@/services/plotResult/lmm/applyLmmTrajectoryOverride'
import type { LmmTraceRoleOverride, TraceRoleMapping } from '@/services/plotResult/lmm/resolveTraceRoles'
...
import { resolveBracketForShape } from '@/utils/plotBuilders/resolveBracketForShape'
import { cascadeMasterToggle, getMasterToggleState } from '@/utils/plotBuilders/cascadeBracketToggle'
import { rebuildBracketShapes, BRACKET_THIN_PARAMS, BRACKET_FAT_PARAMS } from '@/utils/plotBuilders/rebuildBracketShapes'
...
import { getActiveViewLabel, getLmmSiblingViews } from './lmmViewToggle'
...
export function PlotSidebar({ className }: PlotSidebarProps) {
  ...
  // Store state
  const { activePlot, updatePlot, getPlotStats, setPlotStats, setLmmStyleOverride, lmmStyleOverrides } = usePlotsStore(
    useShallow((state) => ({
      activePlot: state.getActivePlot(),
      updatePlot: state.updatePlot,
      getPlotStats: state.getPlotStats,
      setPlotStats: state.setPlotStats,
      setLmmStyleOverride: state.setLmmStyleOverride,
      lmmStyleOverrides: state.lmmStyleOverrides,
    }))
  )
  const setActivePlot = usePlotsStore((state) => state.setActivePlot)
  const siblingPlots = usePlotsStore(
    useShallow((state) => {
      if (!activePlot || activePlot.sourceType !== 'test_result' || !activePlot.resultId) return []
      return state.getPlotsByResult(activePlot.resultId)
    })
  )
  const lmmSiblings = getLmmSiblingViews(activePlot, siblingPlots)
  ...
  const lmmEditEligible = isLmmEditSignificanceEligible(activePlot, layoutMeta)

  // LMM trajectory style override state (Phase 2)
  // lmmRoleMapping first — isLmmTrajectory depends on it (finding 3: guard by resolved)
  const lmmRoleMapping = layoutMeta.traceRoleMapping as TraceRoleMapping | undefined
  const lmmCompoundGuardReason = layoutMeta.compoundGuardReason
  const isLmmTrajectory =
    activePlot?.sourceType === 'test_result' &&
    activePlot?.lmmMode === 'trajectory' &&
    lmmRoleMapping?.resolved === true &&
    lmmRoleMapping?.dashMap !== undefined &&
    layoutMeta.trajectoryLayout !== 'compound'
  const lmmOverrideKey =
    isLmmTrajectory && activePlot?.resultId
      ? `${activePlot.resultId}|${activePlot.facetKey ?? 'pooled'}|trajectory`
      : null
  const currentLmmOverride: LmmTraceRoleOverride | undefined =
    lmmOverrideKey ? lmmStyleOverrides[lmmOverrideKey] : undefined
  const lmmGroupLevels = lmmRoleMapping?.resolved ? Object.keys(lmmRoleMapping.dashMap) : []
  const currentBaselineLevel = lmmRoleMapping?.resolved
    ? (Object.entries(lmmRoleMapping.dashMap).find(([, v]) => v === 'dot')?.[0] ?? '')
    : ''
  const meansType =
  ...
  const bracketEffects = useMemo(() => {
    ...
    const entries = Object.entries(effectMap).map(([effectId, meta]) => ({
      effectId,
      label: meta.label,
      group: meta.group as 'main' | 'simple' | 'comparison',
    ...
    const filtered =
      activePlot?.type === 'grouped_bar'
        ...
        : entries
    const order: Record<'comparison' | 'main' | 'simple', number> = { comparison: 0, main: 1, simple: 2 }
  ...
  // LMM trajectory style override — apply new override and trigger rebuild
  const applyLmmStyleOverride = useCallback((nextOverride: LmmTraceRoleOverride) => {
    if (!isLmmTrajectory || !lmmOverrideKey || !activePlot?.resultId) return
    const result = getResult(activePlot.resultId)
    if (!result) return

    // Persist override — store prunes swapStyles:false and clears entry when nothing meaningful remains
    setLmmStyleOverride(lmmOverrideKey, nextOverride)

    // Rebuild plot with effective overrides
    const effectiveOverrides: Record<string, LmmTraceRoleOverride> = {
      ...lmmStyleOverrides,
      [lmmOverrideKey]: nextOverride,
    }
    applyLmmTrajectoryOverride({
      result,
      effectiveOverrides,
      facetKey: activePlot.sourceType === 'test_result' ? activePlot.facetKey : null,
  ...
  const mergeRebuiltLayout = (
    ...
    nextShapes.forEach((shape) => {
      ...
      if (typeof name === 'string') {
        ...
        if (name.startsWith('sig_bracket_')) {
          // C1 fix: fall back to the rebuilt shape when current layout has no entry for this name
    ...
    return {
      ...rebuiltLayout,
      meta: nextMeta,
      // H1: lock hovermode:false during LMM Edit Significance mode so rebuilds cannot
      // inadvertently re-enable hover tooltips. Gated to eligible LMM context only.
      ...(nextMeta.editSignificanceMode === true && isLmmEditSignificanceEligible(activePlot, nextMeta)
  ...
  const runDoseInterpolation = (override?: {
    mode?: DoseResponseInterpolationMode
    input?: string | number
  }) => {
    ...
    const computeMode = override?.mode ?? interpolationMode
    const computeInput = override?.input !== undefined ? String(override.input) : interpolationInput
    const parsedInput = Number(computeInput)
    setBatchInterpolationResults([])
    setSelectedInterpolationIndex(null)
    const interpolationResult = interpolateDoseResponse(
      doseInterpolationContext,
    ...
    const xValue = computeMode === 'forward' ? parsedInput : interpolationResult.value
    const yValue = computeMode === 'forward' ? interpolationResult.value : parsedInput
    const observedRange = doseInterpolationContext.observedDoseRange
    if (
      shouldRejectDoseInterpolationByStabilityCap({
        xValue,
  ...
  useEffect(() => {
    ...
    const bridge = {
      runDoseInterpolation: ({
        ...
        setInterpolationInput(String(input))
        runDoseInterpolation({ mode, input })
      },
    }

    ;(window as typeof window & {
      __E2E_PLOT_SIDEBAR_BRIDGE__?: typeof bridge
    }).__E2E_PLOT_SIDEBAR_BRIDGE__ = bridge

    return () => {
      const e2eWindow = window as typeof window & {
        __E2E_PLOT_SIDEBAR_BRIDGE__?: typeof bridge
      }
      if (e2eWindow.__E2E_PLOT_SIDEBAR_BRIDGE__ === bridge) {
        delete e2eWindow.__E2E_PLOT_SIDEBAR_BRIDGE__
      }
    }
  }, [runDoseInterpolation])
  ...
  const updateAnnotationFont = (fontFamily: string) => {
    if (!activePlot) return
    const resolvedFont = resolvePlotFontFamily(fontFamily)
    updateLayout({
      meta: {
        ...layoutMeta,
  ...
  const toggleEffectVisibility = (effectId: string, visible: boolean) => {
    if (!activePlot) return
    const effectMap = layoutMeta.bracketEffectMap ?? derivedBracketMeta?.effectMap ?? {}
    const isMaster = effectMap[effectId]?.group === 'comparison'

    // For master toggles, cascade visibility to all children; otherwise just this effectId.
    const nextVisibility = isMaster
      ? cascadeMasterToggle(effectId, visible, effectMap, bracketVisibility)
      : { ...bracketVisibility, [effectId]: visible }

    // Collect all shape names affected by this change
    const effectShapesMap = layoutMeta.bracketEffectShapes ?? derivedBracketMeta?.effectShapes ?? {}
    const affectedShapeNames = new Set<string>()
    for (const [id, isVis] of Object.entries(nextVisibility)) {
      if (isVis !== (bracketVisibility[id] ?? true)) {
        for (const name of effectShapesMap[id] ?? []) affectedShapeNames.add(name)
      }
    }
    ...
    const updatedShapes = shapes.map((shape) => {
      ...
      if (!affectedShapeNames.has(shape.name)) return shape
      return { ...shape, visible }
    })

    updateLayout({
      shapes: updatedShapes,
      meta: { ...layoutMeta, bracketVisibility: nextVisibility },
    })
  }
  ...
  const updateBracketLabelMode = (mode: 'stars' | 'pvalue') => {
    ...
    if (catalog?.brackets?.length && nextShapes.length > 0) {
      ...
      for (let i = 0; i < nextShapes.length; i++) {
        ...
        const bracket = resolveBracketForShape(
          name,
          catalog.brackets,
          shapeToEffectId,
          bracketByEffectId,
  ...
  const updateErrorBarType = (nextType: 'se' | 'sd' | 'ci' | 'iqr' | 'none') => {
    ...
    if (activePlot.resultId) {
      ...
      const rebuilt = rebuildTestResultPlot(result, activePlot.type, {
        ...
        lmmMode: activePlot.sourceType === 'test_result' ? activePlot.lmmMode : null,
        facetKey: activePlot.sourceType === 'test_result' ? activePlot.facetKey : null,
        testType: activePlot.sourceType === 'test_result' ? activePlot.testType : null,
        lmmStyleOverrides,
      })
      if (!rebuilt) return
  ...
  const normalizedFont = useMemo(() => {
    return resolvePlotFontFamily(currentFont)
  }, [currentFont])
  ...
  const normalizedAnnotationFont = useMemo(() => {
    return resolvePlotFontFamily(currentAnnotationFont)
  ...
  const normalizedTitleFont = useMemo(() => {
    return resolvePlotFontFamily(currentTitleFont)
  }, [currentTitleFont])
  ...
  useEffect(() => {
    ...
    const resolvedFont = resolvePlotFontFamily(font)
    const titleFont =
      typeof layout.title === 'object'
        ? (layout.title as Partial<Layout['title']>).font?.family
        : undefined
    const resolvedTitleFont = resolvePlotFontFamily(titleFont)
  ...
  // Check if plot supports trendlines (numeric scatter with markers only; never LMM)
  const supportsTrendline = useMemo(() => {
    if (isLmmPlot) return false
  ...
  }, [isLmmPlot, getTrendlineSourceTrace, isNumericSeries])
  ...
  const applyTrendline = useCallback(
    async (options: { replaceExisting: boolean; silent?: boolean }) => {
      if (!activePlot || !supportsTrendline || isLmmPlot) return
  ...
  return (
    <div className={cn('flex flex-col h-full bg-white dark:bg-zinc-900', className)}>
      ...
      <div className="px-4 py-3 border-b border-zinc-200 dark:border-zinc-800">
        ...
        {/* LMM Trajectory / Contrast view toggle — only shown when both sibling plots exist */}
        {lmmSiblings && (
          <div
            className="flex items-center gap-0.5 mt-2 p-0.5 rounded-md bg-zinc-100 dark:bg-zinc-800"
            data-testid="lmm-view-toggle"
          >
            {(() => {
              const isActiveTrajectory =
                activePlot.sourceType === 'test_result' &&
              ...
              const isActiveContrast =
                activePlot.sourceType === 'test_result' &&
                activePlot.type === 'line' &&
                activePlot.lmmMode === 'contrast'
              const trajectoryLabel = getActiveViewLabel('trajectory') ?? 'Trajectory'
              const contrastLabel = getActiveViewLabel('contrast') ?? 'Contrast'
              return (
                <>
            <button
              type="button"
              onClick={() => setActivePlot(lmmSiblings.trajectoryPlotId)}
              className={cn(
              ...
              aria-pressed={isActiveTrajectory}
            >
              {trajectoryLabel}
            </button>
            <button
              type="button"
              onClick={() => setActivePlot(lmmSiblings.contrastPlotId)}
              className={cn(
                'flex-1 text-[11px] font-medium px-2 py-1 rounded transition-colors',
                isActiveContrast
                  ? 'bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 shadow-sm'
      ...
      {/* Compound disabled banner — shown on legacy LMM trajectory cards when compound guard failed */}
      {lmmCompoundGuardReason !== undefined && activePlot?.sourceType === 'test_result' && activePlot.lmmMode === 'trajectory' && (
        <div
          className="mx-3 mb-2 px-2 py-1.5 rounded text-[11px] bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 text-amber-800 dark:text-amber-300"
          data-testid="compound-guard-reason"
        >
          <span className="font-medium">Compound disabled:</span> {lmmCompoundGuardReason}
        </div>
      )}
        ...
        <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
          ...
          <TabsContent value="colors" className="m-0 flex-1 min-h-0 overflow-hidden">
            <ScrollArea className="h-full" scrollbarSide="left" leftViewportPadding="sm">
              ...
              {/* LMM trajectory trace style overrides */}
              {isLmmTrajectory && lmmRoleMapping?.resolved && lmmGroupLevels.length === 2 && (
                <div className="space-y-2" data-testid="lmm-style-override-panel">
                  <Label className="text-xs font-medium">Trace style</Label>
                  <div className="rounded-md border border-zinc-200 dark:border-zinc-700 p-3 space-y-3">
                    {/* Baseline level select */}
                    <div className="space-y-1.5">
                      <Label className="text-[11px] text-zinc-500">Baseline (dotted line)</Label>
                      <Select
                        value={currentBaselineLevel}
                        onValueChange={(value) => {
                          const other = lmmGroupLevels.find((k) => k !== value) ?? ''
                          // Preserve existing swapStyles — do not reset it on baseline change (finding 5)
                          applyLmmStyleOverride({
                            ...currentLmmOverride,
                            baselineLevel: value,
                            contrastLevel: other,
                          })
                      ...
                      >
                        <SelectTrigger className="h-8" data-testid="lmm-baseline-select">
                          <SelectValue />
                        </SelectTrigger>
                        <SelectContent>
                          {lmmGroupLevels.map((level) => (
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="text-[11px] text-zinc-500">Swap styles</div>
                        <div className="text-[10px] text-zinc-400">Invert solid/dot assignment</div>
                      </div>
                      <Switch
                        data-testid="lmm-swap-styles-switch"
                        checked={currentLmmOverride?.swapStyles === true}
                        onCheckedChange={(checked) => {
                          applyLmmStyleOverride({
                            ...currentLmmOverride,
                            swapStyles: checked || undefined,
                          })
                        }}
                      />
                    </div>

                    {/* Invalid override note */}
                    {lmmRoleMapping.reason && (
                      <div
                        className="text-[10px] text-amber-600 dark:text-amber-400"
                        data-testid="lmm-override-reason"
                      >
                        {lmmRoleMapping.reason}
                      </div>
                    )}
          ...
          <TabsContent value="brackets" className="m-0 flex-1 min-h-0 overflow-hidden">
            ...
            {!hasBrackets ? (
              ...
              <div className="space-y-3">
                ...
                {lmmEditEligible && <div className="flex items-center justify-between rounded border border-zinc-200 dark:border-zinc-700 px-3 py-2">
                  <div>
                    <div className="text-xs font-medium text-zinc-800 dark:text-zinc-200">Edit significance</div>
                    <div className="text-[10px] text-zinc-400">Disables hover to allow dragging markers</div>
                  </div>
                  <Switch
                    checked={layoutMeta.editSignificanceMode === true}
                    onCheckedChange={(checked) => {
                      if (!lmmEditEligible) return  // defense-in-depth: never run for non-LMM plots
                      const priorHovermode = checked
                        ? ((layoutMeta.priorHovermode ?? layout.hovermode ?? 'x unified') as string | false)
                        : undefined
                      const restoredHovermode = checked
                        ? (false as const)
                        : (layoutMeta.priorHovermode ?? 'x unified')

                      // Rebuild sig_bracket_* shapes: fat hit-target in edit mode, thin when off
                      const storedParams = layoutMeta.bracketShapeParams
                      // I1: spread merge — BRACKET_THIN_PARAMS is the base so all fields (lineColor, etc.)
                      // are preserved; storedParams overrides geometry without dropping future fields
                      const thinParams = storedParams != null
                        ? { ...BRACKET_THIN_PARAMS, ...storedParams }
                        : BRACKET_THIN_PARAMS
                      const fatParams = { ...BRACKET_FAT_PARAMS, halfWidth: thinParams.halfWidth }
                      const yAxisRange = (layout as any).yaxis?.range as [number, number] | undefined
                      const yRange = yAxisRange
                        ? Math.abs(yAxisRange[1] - yAxisRange[0])
                        : (storedParams?.ySpan ?? 1)
                      const currentShapes = Array.isArray(layout.shapes) ? layout.shapes : []
                      const nextShapes = rebuildBracketShapes(
                        currentShapes as unknown[],
                        yRange,
                        checked ? fatParams : thinParams,
                      )

                      updateLayout({
                        hovermode: restoredHovermode,
                        shapes: nextShapes as typeof layout.shapes,
                        meta: {
                          ...layoutMeta,
                          editSignificanceMode: checked,
                          priorHovermode,
                        },
                      } as Parameters<typeof updateLayout>[0])
                    }}
                  />
                </div>}
                ...
                <div className="space-y-2 pr-1">
                  {bracketEffects
                    // Render comparison masters inline with their children; skip orphan children (rendered under master)
                    .filter(effect => effect.group !== 'simple' || !effect.parentId || !bracketEffects.some(e => e.effectId === effect.parentId))
                    .map((effect) => {
                      if (effect.group === 'comparison') {
                        const effectMap = layoutMeta.bracketEffectMap ?? derivedBracketMeta?.effectMap ?? {}
                        const masterState = getMasterToggleState(effect.effectId, effectMap, bracketVisibility)
                        const children = bracketEffects.filter(e => e.parentId === effect.effectId)
                        return (
                          <div key={effect.effectId} className="space-y-1">
                            {/* Master comparison toggle */}
                            <div className="flex items-center gap-3 rounded border border-zinc-300 dark:border-zinc-600 bg-zinc-50 dark:bg-zinc-800/50 px-3 py-2">
                              <Checkbox
                                checked={masterState}
                                onCheckedChange={(checked) =>
                                  toggleEffectVisibility(effect.effectId, checked === 'indeterminate' ? true : Boolean(checked))
                                }
                                className={cn(
                                  'rounded',
                                  effect.significant
                                    ? 'data-[state=checked]:bg-red-500 data-[state=checked]:border-red-500 data-[state=indeterminate]:bg-red-500 data-[state=indeterminate]:border-red-500'
                                    : 'data-[state=checked]:bg-blue-500 data-[state=checked]:border-blue-500 data-[state=indeterminate]:bg-blue-500 data-[state=indeterminate]:border-blue-500'
                                )}
                              />
                              <div>
                                ...
                                <div className="text-[10px] uppercase tracking-wide text-zinc-400">Comparison</div>
                              </div>
                            </div>
                            {/* Child per-timepoint toggles */}
                            {children.map(child => (
                              <div
                                key={child.effectId}
                                className="flex items-center gap-3 rounded border border-zinc-200 dark:border-zinc-700 px-3 py-2 ml-4"
                              >
                                <Switch
                                  checked={isBracketEffectVisible(child)}
                                  onCheckedChange={(checked) =>
                                    toggleEffectVisibility(child.effectId, Boolean(checked))
                                  }
                                  className={cn(
                                    child.significant
                                ...
                                <div>
                                  <div className="text-xs font-medium text-zinc-800 dark:text-zinc-200">
                                    {child.label.split(' | ')[1] ?? child.label}
                                  </div>
                                  <div className="text-[10px] uppercase tracking-wide text-zinc-400">Simple effect</div>
                                </div>
                              </div>
                            ))}
                          </div>
                      ...
                      // Non-grouped simple/main effects (no parent master)
                      return (
                        <div
                          key={effect.effectId}
