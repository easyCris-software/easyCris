...
import { isTestResultPlot, type PlotSpec } from '@/store/plots-store'
...
export function PlotThumbnail({
  ...
  // Narrower column → shorter thumbnail (more landscape) to reduce per-card vertical height.
  // 'narrow' columns are the smallest width so get the most compact height ratio.
  // wide = 2-column grid, compact/narrow = 1-column (different panel widths, same layout).
  // Single-column modes use 3:2 to minimise per-card vertical height and reduce total scroll.
  const DENSITY_ASPECT_CLASS = {
    wide: 'aspect-square',   // 2-col — square thumbnail at half panel width
    compact: 'aspect-[3/2]', // 1-col — landscape to cut scroll height
    narrow: 'aspect-[3/2]',  // 1-col, narrower panel — same policy, same ratio
  } as const satisfies Record<'wide' | 'compact' | 'narrow', string>
  ...
  const thumbnailNeedsWebGL = useMemo(
    () => thumbnailData.some((trace) => (trace as { type?: unknown })?.type === 'scattergl'),
    [thumbnailData]
  )
  const thumbnailWebGLUnavailable = thumbnailNeedsWebGL && !isWebGLSupported()
  ...
  return (
    <div
      ...
      {...(isTestResultPlot(plot) && {
        'data-test-type': plot.testType,
        ...(plot.lmmMode != null && { 'data-lmm-mode': plot.lmmMode }),
        ...(plot.facetKey != null && { 'data-facet-key': plot.facetKey }),
      })}
      onClick={onClick}
      role="button"
      tabIndex={0}
    ...
    >
      ...
      <div
        className={cn(
          'overflow-hidden rounded-t-lg bg-muted/20 w-full',
          DENSITY_ASPECT_CLASS[density ?? 'wide']
        )}
      >
        {thumbnailWebGLUnavailable ? (
          <div
            className="flex h-full w-full items-center justify-center p-4 text-center text-xs font-medium text-amber-700 dark:text-amber-300"
            data-testid="scattergl-thumbnail-webgl-fallback"
          >
            WebGL unavailable
          </div>
      ...
      {/* Title Bar */}
      <div
        data-testid="plot-thumbnail-title-bar"
        className={cn(
          'border-t border-border/50',
          density === 'wide' ? 'px-2 py-1.5' : 'px-2 py-1'
        )}
      >
