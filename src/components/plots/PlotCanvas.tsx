...
export function PlotCanvas({ scale, onScaleChange, className }: PlotCanvasProps) {
  ...
  // Layout with all critical settings
  const finalLayout = useMemo<Partial<Layout>>(() => {
    ...
    if (supportsTrendlineStats && trendlineMeta?.equation) {
      // Format polynomial powers with superscripts (x^2 → x<sup>2</sup>, x^3 → x<sup>3</sup>, etc.)
    ...
    const storedTitleAnnotation = normalizedBaseAnnotations.find(
      (a) => typeof a === 'object' && (a as { name?: string }).name === '_title_'
    ) as SystemAnnotationPosition | undefined
    const storedXAxisAnnotation = normalizedBaseAnnotations.find(
      (a) => typeof a === 'object' && (a as { name?: string }).name === '_xaxis_title_'
    ) as SystemAnnotationPosition | undefined
    const storedYAxisAnnotation = normalizedBaseAnnotations.find(
      (a) => typeof a === 'object' && (a as { name?: string }).name === '_yaxis_title_'
    ) as SystemAnnotationPosition | undefined
    const titlePosition = storedTitleAnnotation ?? systemAnnotationPositions._title_
    const xAxisTitlePosition = storedXAxisAnnotation ?? systemAnnotationPositions._xaxis_title_
    const yAxisTitlePosition = storedYAxisAnnotation ?? systemAnnotationPositions._yaxis_title_
    ...
    const buildLegendLine = (
      ...
      legendLinesArr.push(
        `<span style="color:${resolvedColor}">${resolvedSymbol}</span> ${name}`
      )
    }
    if (showLegend) {
    ...
    const defaultYAxisX = plotAreaLeft - axisOffset - yAxisStandoffOffset
    ...
    // =========================================================================
    const storedLegendAnnotation = normalizedBaseAnnotations.find(
      (a) => typeof a === 'object' && (a as { name?: string }).name === '_legend_'
    ) as SystemAnnotationPosition | undefined
    const legendPosition = storedLegendAnnotation ?? systemAnnotationPositions._legend_
    ...
    return {
      ...
      paper_bgcolor: 'white',
      plot_bgcolor: 'white',
      // Enable spike lines for toggleSpikelines button
      hovermode: 'x unified',
      xaxis: {
        ...baseXAxis,
  ...
  }, [plot, canvasWidth, canvasHeight, isCompact, renderedData])
    ...
    (event: Partial<Layout>) => {
      ...
      if (shapePositionEdits.length > 0) {
        ...
        if (
          ...
          const yRange = existingRange[1] - existingRange[0]
          const extraRatio =
            plotHeight > 0 ? Math.max(0.08, (canvasHeight - plotHeight) / plotHeight) : 0.08
  ...
  const handleInitialized = useCallback(
    ...
    // Copy Plot handler (OLE Copy/Paste - Phase 3)
    const handleCopyPlot = useCallback(async () => {
      console.log('[PlotCanvas] Copy plot started')

      if (!plot) {
        console.error('[PlotCanvas] No plot available')
        toast.error('No plot available to copy')
        return
      }
      console.log('[PlotCanvas] Plot found:', { id: plot.id, type: plot.type })

      const { projectFilePath, projectDirty, saveProject, saveProjectAs } =
        useAppStore.getState()

      let ecpPath = projectFilePath
      console.log('[PlotCanvas] Initial project state:', {
        projectFilePath,
        projectDirty,
        ecpPath
      })
      ...
      if (!ecpPath) {
        console.log('[PlotCanvas] No project file, prompting save as...')
        await saveProjectAs()
        ecpPath = useAppStore.getState().projectFilePath
        console.log('[PlotCanvas] After save as, ecpPath:', ecpPath)
      } else if (projectDirty) {
        console.log('[PlotCanvas] Project dirty, saving...')
        await saveProject()
        ecpPath = useAppStore.getState().projectFilePath
        console.log('[PlotCanvas] After save, ecpPath:', ecpPath)
      }

      if (!ecpPath) {
        console.error('[PlotCanvas] No project file path after save')
        toast.error('Save the project before copying the plot')
        return
      }

      const layout = (finalLayout ?? plot.plotlyLayout ?? {}) as Layout
      const liveLayout = (graphDivRef.current as { layout?: Layout } | null)?.layout
      const exportBaseLayout = (liveLayout ?? layout) as Layout
      const backgroundIsTransparent = (value: unknown): boolean => {
      ...
      // Clean annotations to remove Plotly.js-specific properties not supported by Python Plotly
      const cleanAnnotations = (annotations: any[] | undefined) => {
        if (!annotations) return undefined
        const cleaned = annotations.map((ann) => {
          const { editable, ...cleanAnn } = ann
          if (editable !== undefined) {
            console.log('[PlotCanvas] Removed editable property from annotation:', ann.text)
          }
          return cleanAnn
        })
        return cleaned
      ...
      const liveAnnotations = (() => {
        ...
        const annotations = graphDiv?.layout?.annotations
        return Array.isArray(annotations) ? annotations : undefined
      })()
      ...
      const exportLayout = {
        ...
        height,
        paper_bgcolor:
          backgroundIsTransparent(exportBaseLayout.paper_bgcolor) || !exportBaseLayout.paper_bgcolor
            ? '#ffffff'
            : exportBaseLayout.paper_bgcolor,
        plot_bgcolor:
          backgroundIsTransparent(exportBaseLayout.plot_bgcolor) || !exportBaseLayout.plot_bgcolor
            ? '#ffffff'
            : exportBaseLayout.plot_bgcolor,
        annotations: cleanAnnotations(
          (liveAnnotations ?? (exportBaseLayout.annotations as any[])) as any[]
        ),
      } as Layout

      if (plot?.testType === 'multifactorial_anova') {
        exportLayout.margin = {
          ...(exportLayout.margin ?? {}),
          autoexpand: false,
        }
        if (exportLayout.xaxis && typeof exportLayout.xaxis === 'object') {
          exportLayout.xaxis = { ...(exportLayout.xaxis as any), automargin: false }
        }
        if (exportLayout.yaxis && typeof exportLayout.yaxis === 'object') {
          exportLayout.yaxis = { ...(exportLayout.yaxis as any), automargin: false }
        }
      }

      console.log('[PlotCanvas] Export settings:', {
        width,
        height,
        dpi: 300,
        annotationsCount: exportLayout.annotations?.length ?? 0,
      })

      const tempDirectory = await tempDir()
      const tempFileName = `easycris_plot_${crypto.randomUUID()}.png`
      const tempPath = await join(tempDirectory, tempFileName)
      console.log('[PlotCanvas] Temp PNG path:', tempPath)

      try {
        console.log('[PlotCanvas] Exporting plot with Kaleido...')
        const result = await exportPlotWithKaleido(
          { ...plot, plotlyLayout: exportLayout },
          tempPath,
          {
            format: 'png',
            width,
        ...
        console.log('[PlotCanvas] Export result:', result)

        if (!result.success) {
          console.error('[PlotCanvas] Export failed:', result.error)
          toast.error(result.error ?? 'Failed to export plot image')
          return
        }

        console.log('[PlotCanvas] Reading PNG bytes from temp file...')
        const pngBytes = await readFile(tempFileName, { baseDir: BaseDirectory.Temp })
        console.log('[PlotCanvas] PNG bytes read:', pngBytes.length, 'bytes')

        console.log('[PlotCanvas] Calling tauriApi.copyPlotOle...')
        console.log('[PlotCanvas] Arguments:', {
          ecpPath,
          pngBytesLength: pngBytes.length
        })

        const resultMessage = await tauriApi.copyPlotOle(ecpPath, pngBytes)

        console.log('[PlotCanvas] OLE copy result:', resultMessage)
        ...
        const message =
          typeof error === 'object' && error !== null && 'message' in error
            ...
            : error instanceof Error
              ? error.message
              : String(error)
        console.error('[PlotCanvas] Copy plot failed:', error)
        console.error('[PlotCanvas] Error details:', {
          message,
          type: typeof error,
        ...
        toast.error(`Copy failed: ${message}`)
          ...
          try {
            console.log('[PlotCanvas] Cleaning up temp file:', tempPath)
            await remove(tempFileName, { baseDir: BaseDirectory.Temp })
          } catch (cleanupError) {
            console.warn('[PlotCanvas] Cleanup failed:', cleanupError)
          }
      }

      console.log('[PlotCanvas] Copy plot completed')
    }, [plot, finalLayout])
      ...
      >
        <ContextMenu>
          ...
          <ContextMenuContent className="w-56">
            <ContextMenuItem onClick={handleCopyPlot}>
