...
import { runDoseResponse5PL, waitForResults } from './utils/ui-workflow.mjs'
...
async function runTest() {
  ...
  try {
    ...
    await loadFixture(driver, 'dose_response_5pl')
    await runDoseResponse5PL(driver, {})
    await waitForResults(driver)

    // Load baseline and extract from UI
    const baseline = await loadRBaseline('dose_response_5pl')
    const actual = await extractStatsFromUI(driver, 'dose_response_5pl')

    console.log('\n=== 5PL VALUE BY VALUE COMPARISON ===')
