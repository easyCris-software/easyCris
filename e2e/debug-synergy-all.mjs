...
async function runTest() {
  ...
  try {
    ...
    await loadFixture(driver, 'synergy_all')
    await runSynergyAll(driver, {})
    await waitForResults(driver)

    // Load baseline and extract from UI
    const baseline = await loadRBaseline('synergy_all')
    const actual = await extractStatsFromUI(driver, 'synergy_all')

    console.log('\n=== SYNERGY ALL (COMBINED) VALUE BY VALUE ===')
