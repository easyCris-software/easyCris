...
import { runSynergyBliss, waitForResults } from './utils/ui-workflow.mjs'
...
async function runTest() {
  ...
  try {
    ...
    await loadFixture(driver, 'synergy_bliss')
    await runSynergyBliss(driver, {})
    await waitForResults(driver)

    // Load baseline and extract from UI
    const baseline = await loadRBaseline('synergy_bliss')
    const actual = await extractStatsFromUI(driver, 'synergy_bliss')

    console.log('\n=== SYNERGY BLISS VALUE BY VALUE ===')
