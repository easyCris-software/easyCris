...
import { runSynergyLoewe, waitForResults } from './utils/ui-workflow.mjs'
...
async function runTest() {
  ...
  try {
    ...
    await loadFixture(driver, 'synergy_loewe')
    await runSynergyLoewe(driver, {})
    await waitForResults(driver)

    // Load baseline and extract from UI
    const baseline = await loadRBaseline('synergy_loewe')
    const actual = await extractStatsFromUI(driver, 'synergy_loewe')

    console.log('\n=== SYNERGY LOEWE VALUE BY VALUE ===')
