...
import { runDoseResponseCompare, waitForResults } from './utils/ui-workflow.mjs'
...
async function runTest() {
  ...
  try {
    ...
    await loadFixture(driver, 'dose_response_compare')
    await runDoseResponseCompare(driver, {})
    await waitForResults(driver)

    // Load baseline and extract from UI
    const baseline = await loadRBaseline('dose_response_compare')
    const actual = await extractStatsFromUI(driver, 'dose_response_compare')

    console.log('\n=== DOSE-RESPONSE COMPARE VALUE BY VALUE ===')
