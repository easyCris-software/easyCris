...
import { runSynergyZIP, waitForResults } from './utils/ui-workflow.mjs'
...
async function runTest() {
  ...
  try {
    ...
    await loadFixture(driver, 'synergy_zip')
    await runSynergyZIP(driver, {})
    await waitForResults(driver)

    // Load baseline and extract from UI
    const baseline = await loadRBaseline('synergy_zip')
    const actual = await extractStatsFromUI(driver, 'synergy_zip')

    console.log('\n=== SYNERGY ZIP VALUE BY VALUE ===')
