/**
 * E2E Test: Violin Plot Validation
 *
 * Validates that violin plots render correct distributions matching R baseline.
 */
...
async function runTest() {
  ...
  try {
    logStep('Starting Violin Plot E2E validation...')
    ...
    // 1. Load fixture
    await loadPlotFixture(driver, 'violin')
    logStep('Loaded violin plot fixture')

    // 2. Load R baseline
    const rBaseline = loadPlotBaseline('violin_r_baseline.json')
    ...
    await savePlotScreenshot(driver, 'violin')
    ...
    if (plotStats.plotType !== 'violin') {
      throw new Error(`Expected plot type 'violin', got '${plotStats.plotType}'`)
    }
    ...
    // Note: Allow higher tolerance (5%) for density estimation differences
    ...
    logSuccess('Violin plot validation passed')
