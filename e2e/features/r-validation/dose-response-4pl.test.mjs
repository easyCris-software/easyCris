/**
 * R Baseline Validation: Dose-Response 4PL
 * Validates 39 metrics (35 statistical + 4 plot) against R baseline (7% tolerance)
 */
...
async function runTest() {
  ...
  try {
    logStep('Starting Dose-Response 4PL R validation...')
    ...
    const fixture = await loadFixture(driver, 'dose_response_4pl')
    logStep(`Loaded fixture with ${fixture.validatedMetrics} expected metrics`)

    logStep('Running Dose-Response 4PL Test via UI...')
    ...
    const comparison = await validateAgainstRBaseline(driver, 'dose_response_4pl', PHARMACOLOGY_TOLERANCE)
    ...
    await exportPlotScreenshot(driver, 'dose_response_4pl', 300, 'doseresponse')

    // Validate plot metrics
    logStep('Validating plot metrics...')
    const plotComparison = await validatePlotAgainstRBaseline(driver, 'dose_response_4pl', null, PHARMACOLOGY_TOLERANCE)
