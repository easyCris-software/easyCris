/**
 * R Baseline Validation: Mann-Whitney U Test
 * Validates 48 metrics (statistical + plot) against R baseline
 *
...
import { runMannWhitneyTest, waitForResults, switchToPlotsTab } from '../../utils/ui-workflow.mjs'
...
async function runTest() {
  ...
  try {
    logStep('Starting Mann-Whitney U Test R validation...')
    ...
    const fixture = await loadFixture(driver, 'mann_whitney')
    logStep(`Loaded fixture with ${fixture.validatedMetrics} expected metrics`)

    logStep('Running Mann-Whitney U Test via UI...')
    await runMannWhitneyTest(driver, {
      valueColumn: 'value',
      groupColumn: 'group'
    })
    ...
    const comparison = await validateAgainstRBaseline(driver, 'mann_whitney', 0.0001)
    logSuccess(`All ${comparison.totalMetrics} statistical metrics validated`)

    // Switch to Plots tab to validate box/violin plots
    ...
    await exportPlotScreenshot(driver, 'mann_whitney', 300, 'box')
    logStep('Validating box plot metrics...')
    const boxPlotComparison = await validatePlotAgainstRBaseline(driver, 'mann_whitney', 'box', 0.01)
    ...
    await exportPlotScreenshot(driver, 'mann_whitney', 300, 'violin')
    logStep('Validating violin plot metrics...')
    const violinPlotComparison = await validatePlotAgainstRBaseline(driver, 'mann_whitney', 'violin', 0.01)
