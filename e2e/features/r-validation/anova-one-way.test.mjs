/**
 * R Baseline Validation: One-Way ANOVA
 * Validates 103 metrics (statistical + bar/box/violin plots) against R baseline
 *
 * FIX: Use importFromValidation to load CSV directly instead of .ecp fixture
 *
...
async function runTest() {
  ...
  try {
    logStep('Starting One-Way ANOVA R validation...')
    ...
    // Load .ecp fixture
    const fixture = await loadFixture(driver, 'anova_one_way')
    logStep(`Loaded fixture with ${fixture.validatedMetrics} expected metrics`)

    logStep('Running One-Way ANOVA via UI...')
    ...
    const comparison = await validateAgainstRBaseline(driver, 'anova_one_way', 0.0001)
    ...
    await exportPlotScreenshot(driver, 'anova_one_way', 300, 'bar')
    logStep('Validating bar plot metrics...')
    const barPlotComparison = await validatePlotAgainstRBaseline(driver, 'anova_one_way', 'bar', 0.01)
    ...
    await exportPlotScreenshot(driver, 'anova_one_way', 300, 'box')
    logStep('Validating box plot metrics...')
    const boxPlotComparison = await validatePlotAgainstRBaseline(driver, 'anova_one_way', 'box', 0.01)
    ...
    await exportPlotScreenshot(driver, 'anova_one_way', 300, 'violin')
    logStep('Validating violin plot metrics...')
    const violinPlotComparison = await validatePlotAgainstRBaseline(driver, 'anova_one_way', 'violin', 0.01)
