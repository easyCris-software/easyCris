/**
 * R Baseline Validation: Dose-Response 5PL
 * Validates 43 metrics (39 statistical + 4 plot) against R baseline (7% tolerance)
 */
...
import { runDoseResponse5PL, waitForResults, switchToPlotsTab } from '../../utils/ui-workflow.mjs'
...
async function runTest() {
  ...
  try {
    logStep('Starting Dose-Response 5PL R validation...')
    ...
    const fixture = await loadFixture(driver, 'dose_response_5pl')
    logStep(`Loaded fixture with ${fixture.validatedMetrics} expected metrics`)

    logStep('Running Dose-Response 5PL Test via UI...')
    await runDoseResponse5PL(driver, {})

    await waitForResults(driver)

    // Validate statistical metrics against R baseline
    logStep('Validating statistical metrics...')
    const comparison = await validateAgainstRBaseline(driver, 'dose_response_5pl', PHARMACOLOGY_TOLERANCE)
    ...
    await exportPlotScreenshot(driver, 'dose_response_5pl', 300, 'doseresponse')

    // Validate plot metrics
    logStep('Validating plot metrics...')
    const plotComparison = await validatePlotAgainstRBaseline(driver, 'dose_response_5pl', null, PHARMACOLOGY_TOLERANCE)
