/**
 * R Baseline Validation: Independent Samples T-Test
 * Validates 71 metrics (statistical + plot) against R baseline
 *
...
import { runIndependentTTest, waitForResults, switchToPlotsTab } from '../../utils/ui-workflow.mjs'
...
async function runTest() {
  ...
  try {
    logStep('Starting Independent T-Test R validation...')
    ...
    logStep('Running Independent T-Test via UI...')
    ...
    const comparison = await validateAgainstRBaseline(driver, 't_test_two_sample', 0.0001)
    ...
    await exportPlotScreenshot(driver, 't_test_two_sample', 300, 'box')
    logStep('Validating box plot metrics...')
    const boxPlotComparison = await validatePlotAgainstRBaseline(driver, 't_test_two_sample', 'box', 0.01)
    ...
    await exportPlotScreenshot(driver, 't_test_two_sample', 300, 'violin')
    logStep('Validating violin plot metrics...')
    const violinPlotComparison = await validatePlotAgainstRBaseline(driver, 't_test_two_sample', 'violin', 0.01)
