/**
 * R Baseline Validation: Dose-Response 4PL Interpolation
...
import { runDoseResponse4PL, waitForResults, switchToPlotsTab } from '../../utils/ui-workflow.mjs'
...
async function readInterpolationStats(driver) {
  return driver.executeScript(() => {
    ...
    const byActiveId = activePlotId
      ...
      : null
    const byTestType = document.querySelector('[data-plot-stats][data-test-type="dose_response_4pl"]')
...
async function runTest() {
  ...
  try {
    logStep('Starting Dose-Response 4PL interpolation validation...')
    ...
    const baseline = loadPlotBaseline('dose_response_4pl_interpolation_r_baseline.json')
    ...
    // Forward mode via Enter key
    logStep('Validating forward interpolation via Enter key...')
    await setInterpolationMode(driver, 'Conc -> Response')
    const forwardInput = await setInterpolationInput(driver, baseline.interpolation_forward_x_input)
    await forwardInput.sendKeys(Key.ENTER)
    ...
    await exportPlotScreenshot(driver, 'dose_response_4pl', 300, 'interpolation_forward')

    // Inverse mode via Compute click
    logStep('Validating inverse interpolation via Compute click...')
    await setInterpolationMode(driver, 'Response -> Conc')
    await setInterpolationInput(driver, baseline.interpolation_inverse_y_input)
    ...
    await exportPlotScreenshot(driver, 'dose_response_4pl', 300, 'interpolation_inverse')

    logSuccess('Dose-Response 4PL interpolation validation passed')
  } finally {
    if (driver && webdriver) {
