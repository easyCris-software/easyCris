...
def _build_residual_normality_tests(residuals: np.ndarray, alpha: float) -> List[Dict[str, Any]]:
    ...
    The two-way ANOVA normality assumption is model-residual normality, not
    per-cell normality, so this intentionally operates on one residual vector.
    """
    test_specs = [
...
def anova_two_way(
    ...
    try:
        ...
        # ASSUMPTION CHECK 2: Test for normality of model residuals.
