...
def _run_stratified_lmm(
    ...
    if invalid_df_method:
        return _error(invalid_df_method)
    ...
    if not normalized_stratify_by:
        return _error("Stratified lmm_anova requires at least one stratification factor.")

    for name in normalized_stratify_by:
        if name not in predictors:
            return _error(f"Stratification factor '{name}' was not found in predictors.")
        if _normalize_predictor_type(predictor_types.get(name), predictors[name]) != "categorical":
            return _error(f"Stratification factor '{name}' must be categorical.")
    ...
    if not remaining_predictors:
        return _error("Stratified lmm_anova requires remaining model predictors after removing stratification factors.")

    row_count = len(dependent)
    if len(subject) != row_count:
        return _error("dependent and subject must have the same length.")
...
def _fit_model(df: pd.DataFrame, fixed_formula: str, re_formula: str, reml: bool, groups: pd.Series):
    ...
    if best_result is not None:
        best_warnings.extend(errors)
        return best_result, best_warnings, optimizer_used
...
def _display_df(df_approx: float) -> int | None:
    if not np.isfinite(df_approx):
        return None
    return int(df_approx)
...
def _extract_fixed_effects(
    ...
    rows = []
    for term_name, row in table.iterrows():
        if str(term_name).strip().lower() == "intercept":
            continue
        statistic = float(row["statistic"])
        p_value = float(row["pvalue"])
        rows.append(
            {
                "source": _term_label(str(term_name), predictor_metas),
                "term": str(term_name),
                "statistic_type": "Chi-Square",
                "statistic": format_number(statistic),
                "chi_square": format_number(statistic),
...
def _build_continuous_time_followups(
    ...
    return pairwise_rows, continuous_records, []
...
def lmm_anova(
    ...
    try:
        ...
        if not isinstance(predictors, dict) or not predictors:
            return _error("lmm_anova requires at least one predictor.")
        ...
        for name, values in predictors.items():
            if len(values) != row_count:
                return _error(f"Predictor '{name}' length does not match dependent length.")
        ...
        if group_values is not None:
            if len(group_values) != row_count:
                return _error("random_effects_config.group_values must match dependent length.")
        ...
        if len(df) < 6:
            return _error("Insufficient valid rows for lmm_anova after removing missing values.")

        group_sizes = df.groupby("group_id", sort=False).size()
        if group_sizes.shape[0] < 3:
            return _error("lmm_anova requires at least 3 subjects.")
        if int(group_sizes.min()) < 2:
            return _error("Each subject requires at least 2 observations for repeated-measures LMM.")

        for meta in predictor_metas:
            if meta.is_categorical and df[meta.internal_name].nunique() < 2:
                return _error(f"Categorical predictor '{meta.original_name}' requires at least 2 levels.")
        ...
        continuous_pairwise_rows, continuous_records, continuous_warnings = _build_continuous_time_followups(
            fit,
            df,
            predictor_metas,
            alpha,
        ...
        return _error(f"lmm_anova failed: {exc}")
