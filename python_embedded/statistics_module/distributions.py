...
from scipy.stats import shapiro, cramervonmises, norm  # Keep these from scipy (statsmodels doesn't have them)
...
def cramer_von_mises_test(data, alpha=0.05):
    ...
    try:
        ...
        # Perform Cramér-von Mises test (scipy)
        result = cramervonmises(standardized, norm.cdf)

        return {
            'success': True,
            'test_type': 'cramer_von_mises',
            'cvm_statistic': format_number(result.statistic),
            'p_value': format_number(result.pvalue),
            'is_normal': bool(result.pvalue >= alpha),
            'is_significant': bool(result.pvalue < alpha),
            'alpha': format_number(alpha),
            'n': int(len(arr)),
            'interpretation': 'Tests if data comes from a normal distribution',
            'method': 'Cramér-von Mises goodness-of-fit test',
            'null_hypothesis': 'Data follows a normal distribution',
            'decision': 'Fail to reject H0 (data appears normal)' if result.pvalue >= alpha else 'Reject H0 (data deviates from normality)'
        }
    except Exception as e:
        return {'success': False, 'error': str(e)}
