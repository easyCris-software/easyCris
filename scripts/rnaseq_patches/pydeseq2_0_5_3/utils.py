...
def _get_pyplot():
    """Import matplotlib lazily so non-plotting DESeq2 paths do not require pyplot at module import time."""
...
def locfit_weighted(
    ...
    """Approximate locfit with weighted local polynomial regression.

    This mirrors DESeq2's localDispersionFit which uses locfit on
    log(dispersion) ~ log(mean) with weights=means.
    """
    n = len(features)
    if n == 0:
        return np.array([])
    ...
    for i in range(n):
        weights_i = w[:, i] * weights
        w_sqrt = np.sqrt(weights_i)
        if degree == 1:
            A = np.column_stack([w_sqrt, w_sqrt * features])
            b = w_sqrt * targets
            beta = np.linalg.lstsq(A, b, rcond=None)[0]
            yest[i] = beta[0] + beta[1] * features[i]
        else:
            A = np.column_stack(
                [w_sqrt, w_sqrt * features, w_sqrt * features * features]
            )
            b = w_sqrt * targets
            beta = np.linalg.lstsq(A, b, rcond=None)[0]
            yest[i] = beta[0] + beta[1] * features[i] + beta[2] * features[i] ** 2
...
def locfit_predict(
    ...
    """Predict values at query points using locfit-style local regression."""
    features = np.asarray(features, dtype=float)
    targets = np.asarray(targets, dtype=float)
    weights = np.asarray(weights, dtype=float)
    ...
    preds = np.zeros_like(query, dtype=float)
    flat_query = query.ravel()

    for i, x0 in enumerate(flat_query):
        dist = np.abs(features - x0)
        h = np.sort(dist)[r]
        if not np.isfinite(h) or h <= 0:
            preds[i] = np.average(targets, weights=weights)
            continue
        w = np.clip(dist / h, 0.0, 1.0)
        w = (1 - w**3) ** 3
        weights_i = w * weights
        if np.all(weights_i == 0):
            preds[i] = np.average(targets, weights=weights)
            continue
        ...
        if degree == 1:
            ...
            b = w_sqrt * targets
            beta = np.linalg.lstsq(A, b, rcond=None)[0]
            preds[i] = beta[0] + beta[1] * x0 + beta[2] * x0**2

    return preds.reshape(query.shape)
...
def lowess_weighted(
    ...
    """Run lowess smoothing with optional external weights.

    The weights array is used to up/down-weight observations during each local fit.
    This approximates locfit's weighted local regression used by DESeq2 for
    fitType="local".
    """
    n = len(features)
    if n == 0:
    ...
    for _ in range(iter):
        for i in range(n):
            weights_i = delta * w[:, i] * weights
            b = np.array(
                [np.sum(weights_i * targets), np.sum(weights_i * targets * features)]
            )
            A = np.array(
                [
                    [np.sum(weights_i), np.sum(weights_i * features)],
                    [np.sum(weights_i * features), np.sum(weights_i * features * features)],
                ]
            )
            beta = np.linalg.lstsq(A, b, rcond=None)[0]
            yest[i] = beta[0] + beta[1] * features[i]
