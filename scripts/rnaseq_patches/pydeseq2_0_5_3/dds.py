...
from pydeseq2.utils import locfit_predict, locfit_weighted, lowess_weighted
...
class DeseqDataSet(ad.AnnData):
    ...
    fit_type: str
        ...
        relation via a robust gamma-family GLM. ``"local"``: fit a locally weighted
        regression on log dispersions vs log means. ``"mean"``: use the mean of
        gene-wise dispersion estimates. Will set the fit type for the DEA and the vst
        transformation. If needed, it can be set separately for each method.
        (default: ``"parametric"``).
    ...
    def vst(
        ...
        fit_type: Literal["parametric", "local", "mean"] | None = None,
    ) -> None:
        ...
        fit_type: str
            ...
            * ``"parametric"``: fit a dispersion-mean relation via a robust
              gamma-family GLM.
            * ``"local"``: fit a locally weighted regression on log dispersions
              vs log means.
        ...
        if fit_type is not None:
            ...
            # Mirror DESeq2: use the actual dispersion fit type when available.
            self.vst_fit_type = self.uns.get("disp_function_type", self.fit_type)
    ...
    def vst_fit(
        ...
        if use_design:
            ...
            elif self.vst_fit_type == "local":
                self._fit_local_dispersion_trend(vst=True)
            else:
                warnings.warn(
                    "use_design=True is only useful when fit_type='parametric' or 'local'. ",
                    UserWarning,
                    stacklevel=2,
                )
    ...
    def vst_transform(self, counts: np.ndarray | None = None) -> np.ndarray:
        ...
        if self.vst_fit_type == "parametric":
            ...
            if "vst_local_smooth" not in self.uns:
                raise RuntimeError("Fit the dispersion curve prior to applying VST.")
            smooth = self.uns.get("vst_local_smooth")
            local_fit = self.uns.get("vst_local_fit")
            ...
            def disp_func(x):
                ...
                y = np.interp(logx, log_means, log_disps, left=log_disps[0], right=log_disps[-1])
                return np.exp(y)

            ncounts = normed_counts
            max_count = np.max(ncounts)
            if not np.isfinite(max_count) or max_count <= 0:
                return np.log2(ncounts + 1)

            xg = np.sinh(np.linspace(np.arcsinh(0), np.arcsinh(max_count), num=1000))[1:]
            if "size_factors" in self.obs:
                sf = self.obs["size_factors"].values
            else:
                sf = np.ones(ncounts.shape[1])
            xim = np.mean(1.0 / sf)
            ...
            spl_y = np.cumsum((xg[1:] - xg[:-1]) * (integrand[1:] + integrand[:-1]) / 2)
            ...
            row_means = np.mean(ncounts, axis=1)
            h1 = np.quantile(row_means, 0.95)
            h2 = np.quantile(row_means, 0.999)
            if not np.isfinite(h1) or not np.isfinite(h2) or h1 <= 0 or h2 <= 0:
                return np.log2(ncounts + 1)
            ...
            return eta * splf(np.arcsinh(ncounts)) + xi
            ...
            raise NotImplementedError(
                f"Found fit_type '{self.vst_fit_type}'. Expected 'parametric', 'local' or 'mean'."
            )

    def deseq2(self, fit_type: Literal["parametric", "local", "mean"] | None = None) -> None:
        ...
        ----------
        fit_type : str
            Either None, ``"parametric"``, ``"local"`` or ``"mean"`` for the type of
            fitting of dispersions to the mean intensity. ``"parametric"``: fit a
            dispersion-mean relation via a robust gamma-family GLM. ``"local"``: fit
            a locally weighted regression on log dispersions vs log means. ``"mean"``:
            use the mean of gene-wise dispersion estimates.
    ...
    def fit_dispersion_trend(self, vst: bool = False) -> None:
        ...
        if fit_type == "parametric":
            ...
            self._fit_local_dispersion_trend(vst)
        elif fit_type == "mean":
            self._fit_mean_dispersion_trend(vst)
        else:
            raise NotImplementedError(
                f"Expected 'parametric', 'local' or 'mean' trend curve fit "
    ...
    def disp_function(self, x):
        ...
        if self.uns["disp_function_type"] == "parametric":
            ...
            smooth = self.uns.get("local_smooth")
            x = np.asarray(x, dtype=float)
            x = np.clip(x, 1e-8, None)
            logx = np.log(x)
            if smooth is None:
                return np.full_like(x, self.uns.get("mean_disp", np.nan))
            log_means = np.asarray(smooth["x"], dtype=float)
            log_disps = np.asarray(smooth["y"], dtype=float)
            y = np.interp(logx, log_means, log_disps, left=log_disps[0], right=log_disps[-1])
            return np.exp(y)
        ...
        while (coeffs > 1e-10).all() and (
            ...
            if not converged or (coeffs <= 1e-10).any():
                warnings.warn(
                    ...
                    "Switching to a local dispersion trend.",
                    UserWarning,
                    stacklevel=2,
                )
    ...
    def _fit_local_dispersion_trend(self, vst: bool = False):
        """Fit the dispersion curve using a local regression on log scale."""
        disp_param_name = "vst_genewise_dispersions" if vst else "genewise_dispersions"

        if disp_param_name not in self.var:
            self.fit_genewise_dispersions(vst)

        disps = self.var.loc[self.non_zero_genes, disp_param_name].copy()
        means = self.var.loc[self.non_zero_genes, "_normed_means"].copy()

        mask = (
            (disps >= 10 * self.min_disp)
            & np.isfinite(disps)
            & np.isfinite(means)
            & (means > 0)
        )
        ...
        if mask.sum() < 5:
            warnings.warn(
                "Insufficient points for local dispersion fit. "
        ...
        log_means = np.log(means[mask].values)
        log_disps = np.log(disps[mask].values)
        weights = means[mask].values

        smooth = locfit_weighted(log_means, log_disps, weights, frac=0.7, degree=2)
        order = np.argsort(log_means)
        log_means = log_means[order]
        smooth = smooth[order]

        log_means_unique, unique_idx = np.unique(log_means, return_index=True)
        smooth_unique = smooth[unique_idx]

        if log_means_unique.size == 0:
            warnings.warn(
                "Local dispersion fit produced no valid points. "
                "Switching to a mean-based dispersion trend.",
                UserWarning,
                stacklevel=2,
            )
        ...
        if vst:
            self.uns["vst_local_smooth"] = {"x": log_means_unique, "y": smooth_unique}
            self.uns["vst_local_fit"] = {
                "x": log_means,
                "y": log_disps,
                "w": weights,
                "frac": 0.7,
            ...
            self.vst_fit_type = "local"
        else:
            self.uns["disp_function_type"] = "local"
            self.uns["local_smooth"] = {"x": log_means_unique, "y": smooth_unique}
            self.var["fitted_dispersions"] = np.full(self.n_vars, np.nan)
            fitted = np.exp(
                np.interp(
                    np.log(self.var.loc[self.var["non_zero"], "_normed_means"].values),
                    log_means_unique,
                    smooth_unique,
                    left=smooth_unique[0],
                    right=smooth_unique[-1],
                )
            )
            self.var.loc[self.var["non_zero"], "fitted_dispersions"] = fitted
    ...
    def _refit_without_outliers(
        ...
        elif sub_dds.uns["disp_function_type"] == "local":
            sub_dds.uns["local_smooth"] = self.uns["local_smooth"]
