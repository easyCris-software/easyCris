...
// ============================================================================
// Singular dataset baseline generator
// Reads test_singular_asymptotic_r_results.csv and emits per-method JSON
// baselines for lmm-anova-singular.test.mjs.
//
// Output key schema mirrors the stratified baseline schema but uses a single
// stratification dimension (Trait) instead of strain|sex:
//
//   Omnibus (per trait × effect):
//     st${n}_fe${m}_source          — effect label ("Condition", "Day", "Condition x Day")
//     st${n}_fe${m}_stratum_trait   — trait value ("Trait_A", "Trait_B")
//     st${n}_fe${m}_f_value         — F statistic
//     st${n}_fe${m}_num_df          — numerator df
//     st${n}_fe${m}_den_df          — denominator df
//     st${n}_fe${m}_p               — omnibus p-value
//
//   Simple effects (per trait × day × comparison):
//     st${n}_se${m}_label           — comparison label string
//     st${n}_se${m}_stratum_trait   — trait value
//     st${n}_se${m}_within_factor   — "Day"
//     st${n}_se${m}_within_level    — day value ("-1", "0", ...)
//     st${n}_se${m}_estimate        — emmeans estimate
//     st${n}_se${m}_se              — standard error
//     st${n}_se${m}_t_ratio         — t-ratio
//     st${n}_se${m}_p_raw           — unadjusted p
//     st${n}_se${m}_p               — adjusted p (method-specific)
// ============================================================================

const SINGULAR_OMNIBUS_EFFECTS = ['Condition', 'Day', 'Condition x Day']
const DAY_LEVEL_ORDER = ['D-1', 'D0', 'D1', 'D2', 'D3', 'D4']

function buildSingularBaseline(rows, adjustment) {
  // Collect trait order as encountered in the file (preserves R's sort order)
  const traitsSeen = []
  for (const row of rows) {
    if (!traitsSeen.includes(row.Trait)) traitsSeen.push(row.Trait)
  }

  const baseline = {}

  traitsSeen.forEach((trait, traitOffset) => {
    const stratumIndex = traitOffset + 1

    // ── Omnibus ─────────────────────────────────────────────────────────────
    const omnibusRows = rows.filter(
      (r) => r.Trait === trait && r.section === 'omnibus'
    )

    SINGULAR_OMNIBUS_EFFECTS.forEach((effect, effectOffset) => {
      const effectIndex = effectOffset + 1
      const row = omnibusRows.find((r) => r.effect === effect)
      if (!row) {
        throw new Error(
          `Missing omnibus row for Trait=${trait}, effect="${effect}"`
        )
      }
      baseline[`st${stratumIndex}_fe${effectIndex}_source`]        = effect
      baseline[`st${stratumIndex}_fe${effectIndex}_stratum_trait`]  = trait
      baseline[`st${stratumIndex}_fe${effectIndex}_f_value`]        = asNumber(row.f_value, `${trait}/${effect}/f_value`)
      baseline[`st${stratumIndex}_fe${effectIndex}_num_df`]         = asNumber(row.num_df,  `${trait}/${effect}/num_df`)
      baseline[`st${stratumIndex}_fe${effectIndex}_den_df`]         = asNumber(row.den_df,  `${trait}/${effect}/den_df`)
      baseline[`st${stratumIndex}_fe${effectIndex}_p`]              = asNumber(row.p_value, `${trait}/${effect}/p_value`)
    })

    // ── Simple effects ───────────────────────────────────────────────────────
    const seRows = rows.filter(
      (r) => r.Trait === trait && r.section === 'simple_effect' && r.adjustment === adjustment
    )

    // Order by day level (DAY_LEVEL_ORDER), then by comparison label within day
    const orderedSe = []
    for (const dayLevel of DAY_LEVEL_ORDER) {
      const forDay = seRows
        .filter((r) => String(r.within_level) === dayLevel)
        .sort((a, b) => String(a.comparison).localeCompare(String(b.comparison)))
      orderedSe.push(...forDay)
    }

    if (orderedSe.length === 0) {
      throw new Error(
        `No simple-effect rows for Trait=${trait}, adjustment=${adjustment}`
      )
    }

    orderedSe.forEach((row, seOffset) => {
      const seIndex = seOffset + 1
      baseline[`st${stratumIndex}_se${seIndex}_label`]         = String(row.comparison)
      baseline[`st${stratumIndex}_se${seIndex}_stratum_trait`] = trait
      baseline[`st${stratumIndex}_se${seIndex}_within_factor`] = String(row.within_factor)
      baseline[`st${stratumIndex}_se${seIndex}_within_level`]  = String(row.within_level)
      baseline[`st${stratumIndex}_se${seIndex}_estimate`]      = asNumber(row.estimate, `${trait}/${row.comparison}/estimate`)
      baseline[`st${stratumIndex}_se${seIndex}_se`]            = asNumber(row.se,       `${trait}/${row.comparison}/se`)
      baseline[`st${stratumIndex}_se${seIndex}_t_ratio`]       = asNumber(row.t_ratio,  `${trait}/${row.comparison}/t_ratio`)
      baseline[`st${stratumIndex}_se${seIndex}_p_raw`]         = asNumber(row.p_raw,    `${trait}/${row.comparison}/p_raw`)
      baseline[`st${stratumIndex}_se${seIndex}_p`]             = asNumber(row.p_adjusted, `${trait}/${row.comparison}/p_adjusted`)
    })
  })
...
function generateSingularBaselines(resultsCsv, outDir) {
  if (!fs.existsSync(resultsCsv)) {
    console.warn(
      `Skipping singular baselines — R results CSV not found: ${path.relative(projectRoot, resultsCsv)}\n` +
      `  Run _test_validation/.../run_r_lmm.R with R to regenerate it.`
    )
    return
  }
  const rows = parseTableCsv(resultsCsv)

  const adjustments = ['tukey', 'dunnett']
  for (const adj of adjustments) {
    const baseline = buildSingularBaseline(rows, adj)
    const outPath  = path.join(outDir, `lmm_anova_singular_${adj}_r_baseline.json`)
    writeJson(outPath, baseline)
    console.log(
      `Generated ${path.relative(projectRoot, outPath)} (${Object.keys(baseline).length} metrics)`
    )
  }
}
...
function main() {
  ...
  // Singular dataset baselines (tukey + dunnett)
  generateSingularBaselines(
    path.join(
      projectRoot,
      '_test_validation',
      'Group1_Hypothesis_Testing',
      'linear_mixed_models',
      'test_singular_asymptotic_r_results.csv'
    ),
    baselinesRoot
  )
}
