...
Usage:
    ...
    python compile_python_nuitka.py stats_backend     # compile only stats_backend
    python compile_python_nuitka.py rnaseq_backend plot_backend  # compile specific backends
"""
...
BACKENDS = (
    ...
    {
        "entrypoint": "rnaseq_backend.py",
        "name": "rnaseq_backend",
        "extra_args": (),
        "allow_unittest": True,
    },
    {
        "entrypoint": "plot_backend.py",
        "name": "plot_backend",
        "extra_args": (
    ...
    try:
        ...
        for backend in backends_to_build:
            try:
                ...
                print(
                    f"[compile-python] FAILED: {backend['name']} — {type(exc).__name__}: {exc}",
                    file=sys.stderr,
                )
