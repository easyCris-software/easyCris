...
//
// Handles:
// - Spawning python.exe stats_backend.py (or compiled .exe)
...
#[cfg(not(feature = "obfuscate"))]
const PYTHON_BACKEND_ARG: &str = "python_embedded/stats_backend.py";
#[cfg(not(feature = "obfuscate"))]
const PYTHON_BACKEND_EXE: &str = "python_embedded/dist/stats_backend.dist/stats_backend.exe";

// Plot backend paths
#[cfg(not(feature = "obfuscate"))]
const PLOT_BACKEND_ARG: &str = "python_embedded/plot_backend.py";
#[cfg(not(feature = "obfuscate"))]
const PLOT_BACKEND_EXE: &str = "python_embedded/dist/plot_backend.dist/plot_backend.exe";

// RNA-seq backend paths
#[cfg(not(feature = "obfuscate"))]
const RNASEQ_BACKEND_ARG: &str = "python_embedded/rnaseq_backend.py";
#[cfg(not(feature = "obfuscate"))]
const RNASEQ_BACKEND_EXE: &str = "python_embedded/dist/rnaseq_backend.dist/rnaseq_backend.exe";

#[inline(always)]
fn python_backend_script_rel() -> String {
    #[cfg(feature = "obfuscate")]
    {
        obfstr::obfstr!("python_embedded/python.exe").to_string()
    }
    #[cfg(not(feature = "obfuscate"))]
    {
...
#[inline(always)]
fn stats_backend_script_rel() -> String {
    #[cfg(feature = "obfuscate")]
    {
        obfstr::obfstr!("python_embedded/stats_backend.py").to_string()
    }
    #[cfg(not(feature = "obfuscate"))]
    {
...
#[inline(always)]
fn stats_backend_exe_rel() -> String {
    #[cfg(feature = "obfuscate")]
    {
        obfstr::obfstr!("python_embedded/dist/stats_backend.dist/stats_backend.exe").to_string()
    }
    #[cfg(not(feature = "obfuscate"))]
    {
...
#[inline(always)]
fn plot_backend_script_rel() -> String {
    #[cfg(feature = "obfuscate")]
    {
        obfstr::obfstr!("python_embedded/plot_backend.py").to_string()
    }
    #[cfg(not(feature = "obfuscate"))]
    {
...
#[inline(always)]
fn plot_backend_exe_rel() -> String {
    #[cfg(feature = "obfuscate")]
    {
        obfstr::obfstr!("python_embedded/dist/plot_backend.dist/plot_backend.exe").to_string()
    }
    #[cfg(not(feature = "obfuscate"))]
    {
...
#[inline(always)]
fn rnaseq_backend_script_rel() -> String {
    #[cfg(feature = "obfuscate")]
    {
        obfstr::obfstr!("python_embedded/rnaseq_backend.py").to_string()
    }
    #[cfg(not(feature = "obfuscate"))]
    {
...
#[inline(always)]
fn rnaseq_backend_exe_rel() -> String {
    #[cfg(feature = "obfuscate")]
    {
        obfstr::obfstr!("python_embedded/dist/rnaseq_backend.dist/rnaseq_backend.exe").to_string()
    }
    #[cfg(not(feature = "obfuscate"))]
    {
...
fn hardened_required_prefix_stats() -> String {
    #[cfg(feature = "obfuscate")]
    {
        lc!("Hardened release requires compiled stats backend at:")
    }
    #[cfg(not(feature = "obfuscate"))]
    {
...
fn hardened_required_prefix_rnaseq() -> String {
    #[cfg(feature = "obfuscate")]
    {
        lc!("Hardened release requires compiled RNA-seq backend at:")
    }
    #[cfg(not(feature = "obfuscate"))]
    {
...
fn hardened_required_prefix_plot() -> String {
    #[cfg(feature = "obfuscate")]
    {
        lc!("Hardened release requires compiled plot backend at:")
    }
    #[cfg(not(feature = "obfuscate"))]
    {
...
fn hardened_required_suffix_profile() -> String {
    #[cfg(feature = "obfuscate")]
    {
        lc!("Script fallback is disabled for build profile")
    }
    #[cfg(not(feature = "obfuscate"))]
    {
...
fn get_python_base_dir() -> PathBuf {
    ...
    if let Some(local_data_dir) = dirs::data_local_dir() {
        ...
        for app_dir in app_dirs {
            let bundle_resources_dir = local_data_dir
                .join(app_dir)
                .join("_up_")
...
pub enum BackendMode {
    /// Development: python.exe stats_backend.py
    Script,
    /// Production: compiled stats_backend.exe
    Compiled,
...
pub async fn spawn_python_backend(
    ...
    let (cmd, args): (PathBuf, Vec<PathBuf>) = match mode {
        ...
        BackendMode::Compiled | BackendMode::CompiledRequired => {
            let exe_path = base_dir.join(stats_backend_exe_rel());
...
pub async fn spawn_rnaseq_backend(
    payload: Value,
    mode: BackendMode,
    app: &tauri::AppHandle,
) -> CommandResult<Value> {
    ...
    let (cmd, args): (PathBuf, Vec<PathBuf>) = match mode {
        ...
        BackendMode::Compiled | BackendMode::CompiledRequired => {
            let exe_path = base_dir.join(rnaseq_backend_exe_rel());
...
///
/// Similar to spawn_python_backend but uses plot_backend.py
///
/// # Arguments
/// * `payload` - JSON payload to send to Python backend
/// * `mode` - Backend mode (Script or Compiled)
...
fn resolve_plot_backend_command(
    mode: BackendMode,
    ...
    let (cmd, args): (PathBuf, Vec<PathBuf>) = match mode {
        ...
        BackendMode::Compiled | BackendMode::CompiledRequired => {
            let exe_path = base_dir.join(plot_backend_exe_rel());
...
async fn spawn_persistent_plot_export_worker(
    ...
    let (cmd, args, process_cwd, _) = resolve_plot_backend_command(mode)?;
...
pub async fn spawn_plot_backend(payload: Value, mode: BackendMode) -> CommandResult<Value> {
    let action = payload
        .get("action")
    ...
    // Get base directory for Python paths
    let (cmd, args, process_cwd, python_cwd) = resolve_plot_backend_command(mode)?;
...
pub fn detect_backend_mode() -> BackendMode {
    detect_backend_mode_for(stats_backend_exe_rel(), "stats", "stats", "stats")
}

/// Determine plot backend mode based on file existence
pub fn detect_plot_backend_mode() -> BackendMode {
    detect_backend_mode_for(plot_backend_exe_rel(), "plot", "plot", "plot")
}

/// Determine RNA-seq backend mode based on file existence
pub fn detect_rnaseq_backend_mode() -> BackendMode {
    detect_backend_mode_for(rnaseq_backend_exe_rel(), "RNA-seq", "RNA-seq", "RNA-seq")
}
