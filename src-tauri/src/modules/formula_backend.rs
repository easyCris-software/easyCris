...
impl DuckDbEvaluationContext {
    ...
    fn fetch_cell(&self, model_row: usize, col_id: &str) -> Result<LiteralValue, ExcelError> {
        ...
        let quoted_col = format!("\"{}\"", col_id.replace('"', "\"\""));
        let logical_sql = format!(
            "SELECT d.{col} \
             FROM main.row_order ro \
             JOIN data d ON d._row_index = ro.physical_row \
             ORDER BY ro.sort_key \
             LIMIT 1 OFFSET ?",
            col = quoted_col
        );

        let mut stmt = conn.prepare(&logical_sql).map_err(|e| {
            log::error!("DuckDB logical row prepare error: {:?}", e);
    ...
    fn fetch_range(
        ...
        let is_contiguous = model_rows
            .windows(2)
            .all(|window| window[1] == window[0] + 1);
        if is_contiguous {
            let start_row = model_rows[0] as i64;
            let row_count = model_rows.len() as i64;
            let logical_sql = format!(
                "SELECT (? + row_number() OVER (ORDER BY __easycris_ro_sort_key) - 1)::BIGINT AS logical_index, {cols} \
                 FROM ( \
                   SELECT ro.sort_key AS __easycris_ro_sort_key, d.* \
                   FROM main.row_order ro \
                   JOIN data d ON d._row_index = ro.physical_row \
                   ORDER BY ro.sort_key \
                   LIMIT ? OFFSET ? \
            ...
            let mut stmt = conn.prepare(&logical_sql).map_err(|e| {
                log::error!("DuckDB logical row prepare error for range: {:?}", e);
                ExcelError::new(ExcelErrorKind::Value)
            })?;
            let mut rows = stmt.query([start_row, row_count, start_row]).map_err(|e| {
                log::error!("DuckDB query error: {:?}", e);
                ExcelError::new(ExcelErrorKind::Value)
            })?;
            ...
            let rows_sql = model_rows
                ...
                .join(", ");
            let logical_sql = format!(
                "WITH requested(logical_index) AS (VALUES {rows}), \
                      ordered AS ( \
                        SELECT physical_row, row_number() OVER (ORDER BY sort_key) - 1 AS logical_index \
                        FROM main.row_order \
                      ) \
                 SELECT ordered.logical_index, {cols} \
                 FROM requested \
                 JOIN ordered ON ordered.logical_index = requested.logical_index \
                 JOIN data d ON d._row_index = ordered.physical_row \
                 ORDER BY requested.logical_index",
                rows = rows_sql,
                cols = cols_sql
            );
...
mod tests {
    ...
    // ========================================================================
    // Wave 1 backend value tests
    //
    // These tests call evaluate_formula with a real in-memory DuckDB connection
    // to verify that Formualizer produces correct scalar results for Wave 1
    // candidate functions.
    //
    // Test dataset (matches wave1Registry.ts comment and Vitest test data):
    //   row 0: col-a=10, col-b=1
    //   row 1: col-a=20, col-b=2
    //   row 2: col-a=30, col-b=3
    //   Excel letter mapping: A=col-a, B=col-b, C=col-c
    //
    // Status: RED until each function is promoted in formulaCatalog.ts.
    // Once promoted and Formualizer support is confirmed, these turn GREEN
    // and serve as a permanent regression guard.
    //
    // Parity with wave1Registry.ts WAVE1_MANIFEST:
    //   MAXIFS        ✓ test_wave1_maxifs
    //   MINIFS        ✓ test_wave1_minifs
    //   XMATCH        ✓ test_wave1_xmatch
    //   FORECAST.LINEAR ✓ test_wave1_forecast_linear
    //   T.TEST        ✓ test_wave1_ttest (approx p-value)
    //   TEXTAFTER     ✓ test_wave1_textafter  (literal-only, no DuckDB data needed)
    //   TEXTBEFORE    ✓ test_wave1_textbefore (literal-only)
    //   VALUETOTEXT   ✓ test_wave1_valuetotext (literal-only)
    //   ARRAYTOTEXT   ✓ test_wave1_arraytotext
    // ========================================================================

    /// Create an in-memory DuckDB connection populated with the Wave 1 test dataset.
    ///
    /// Table schema:
    ///   data (_row_index INTEGER, "col-a" DOUBLE, "col-b" DOUBLE, "col-c" VARCHAR)
    fn make_wave1_conn() -> Arc<Mutex<Connection>> {
        ...
        conn.execute_batch(
            ...
            INSERT INTO data VALUES (0, 10.0, 1.0, 'foo');
            INSERT INTO data VALUES (1, 20.0, 2.0, 'bar');
            INSERT INTO data VALUES (2, 30.0, 3.0, 'baz');
        ...
        .expect("Wave 1 test table setup failed");
        Arc::new(Mutex::new(conn))
    }

    /// Column-letter → column-ID map for the Wave 1 test dataset.
    fn wave1_col_map() -> HashMap<String, String> {
        [
            ("A".to_string(), "col-a".to_string()),
            ("B".to_string(), "col-b".to_string()),
            ("C".to_string(), "col-c".to_string()),
        ]
        .into_iter()
        .collect()
    }

    /// Evaluate a formula against the Wave 1 test dataset.
    /// `view_position` is 0-based (row, col) of the formula cell.
    fn wave1_eval(formula: &str) -> Result<Value, String> {
        evaluate_formula(
            make_wave1_conn(),
            "wave1-test",
            formula,
            (0, 0), // formula cell at A1 (0-based view row 0, col 0)
            wave1_col_map(),
            None, // identity row mapping
            3,    // 3 rows total
        )
    }

    // -----------------------------------------------------------------------
    // Builtin init guard — SUM must work; catches regressions where builtins
    // are not registered before evaluate_formula() runs.
    // -----------------------------------------------------------------------
    #[test]
    fn test_builtin_sum_guard() {
        let result = wave1_eval("=SUM(1,2)").expect("SUM eval failed");
        assert_eq!(result, Value::from(3.0_f64), "SUM(1,2) should be 3");
    }

    #[test]
    fn test_duckdb_formula_uses_logical_row_order_with_sparse_physical_rows() {
        let conn = Connection::open_in_memory().expect("in-memory DuckDB open failed");
        conn.execute_batch(
            r#"
            CREATE TABLE data (
            ...
            INSERT INTO data VALUES (0, 10.0);
            INSERT INTO data VALUES (2, 30.0);
            CREATE TABLE main.row_order(physical_row BIGINT, sort_key BIGINT);
            INSERT INTO main.row_order VALUES (0, 0), (2, 1000000);
            "#,
        )
        .expect("sparse logical formula setup failed");
        ...
        .expect("formula eval should use logical row_order");

        assert_eq!(result, Value::from(30.0_f64));
    }

    #[test]
    fn test_duckdb_formula_maps_view_rows_to_logical_order_slice() {
        let conn = Connection::open_in_memory().expect("in-memory DuckDB open failed");
        conn.execute_batch(
            r#"
            CREATE TABLE data (
        ...
        .expect("formula eval should map view rows before logical row lookup");

        assert_eq!(result, Value::from(30.0_f64));
    }

    // -----------------------------------------------------------------------
    // MAXIFS — max of col-a where col-b > 1
    //   col-b > 1 → rows 1,2 (col-b=2,3)
    //   col-a at those rows: 20, 30 → max = 30
    // -----------------------------------------------------------------------
    #[test]
    fn test_wave1_maxifs() {
        let result = wave1_eval(r#"=MAXIFS(A1:A3,B1:B3,">1")"#).expect("MAXIFS eval failed");
        assert_eq!(
            result,
            Value::from(30.0_f64),
            "MAXIFS(A1:A3,B1:B3,\">1\") should be 30"
        );
    }

    // -----------------------------------------------------------------------
    // MINIFS — min of col-a where col-b > 1
    //   col-b > 1 → rows 1,2 (col-a=20,30) → min = 20
    // -----------------------------------------------------------------------
    #[test]
    fn test_wave1_minifs() {
        let result = wave1_eval(r#"=MINIFS(A1:A3,B1:B3,">1")"#).expect("MINIFS eval failed");
        assert_eq!(
            result,
            Value::from(20.0_f64),
            "MINIFS(A1:A3,B1:B3,\">1\") should be 20"
        );
    }

    // -----------------------------------------------------------------------
    // XMATCH — position of 20 in col-a
    //   col-a = [10, 20, 30], value 20 is at 1-based position 2
    // -----------------------------------------------------------------------
    #[test]
    fn test_wave1_xmatch() {
        let result = wave1_eval("=XMATCH(20,A1:A3)").expect("XMATCH eval failed");
        // Formualizer returns position as a number; accept both integer and float
        let pos = match &result {
            Value::Number(n) => n.as_f64().unwrap_or(0.0),
            _ => panic!("XMATCH: expected Number result, got {:?}", result),
        };
        assert!(
            (pos - 2.0).abs() < 1e-9,
            "XMATCH(20, A1:A3) should be 2, got {}",
            pos
        );
    }

    // -----------------------------------------------------------------------
    // FORECAST.LINEAR — linear regression
    //   known_y = A1:A3 = [10,20,30], known_x = B1:B3 = [1,2,3]
    //   slope = (30-10)/(3-1) = 10, intercept = 10 - 10*1 = 0
    //   at x=4: y = 10*4 + 0 = 40
    // -----------------------------------------------------------------------
    #[test]
    fn test_wave1_forecast_linear() {
        let result =
            wave1_eval("=FORECAST.LINEAR(4,A1:A3,B1:B3)").expect("FORECAST.LINEAR eval failed");
        let v = match &result {
            Value::Number(n) => n.as_f64().unwrap_or(f64::NAN),
            _ => panic!("FORECAST.LINEAR: expected Number, got {:?}", result),
        };
        assert!(
            (v - 40.0).abs() < 1e-6,
            "FORECAST.LINEAR(4,A1:A3,B1:B3) should be ~40, got {}",
            v
        );
    }

    // -----------------------------------------------------------------------
    // T.TEST — paired two-tailed t-test (tails=2, type=1)
    //   A=[10,20,30], B=[1,2,3]; differences=[9,18,27]
    //   mean_diff=18, sd=9, t=18/(9/sqrt(3))≈3.464, df=2
    //   expected p ≈ 0.074 (two-tailed); allow ±0.01 tolerance
    // -----------------------------------------------------------------------
    #[test]
    fn test_wave1_ttest() {
        let result = wave1_eval("=T.TEST(A1:A3,B1:B3,2,1)").expect("T.TEST eval failed");
        let p = match &result {
            Value::Number(n) => n.as_f64().unwrap_or(f64::NAN),
            _ => panic!("T.TEST: expected Number, got {:?}", result),
        };
        assert!(
            p > 0.0 && p <= 1.0,
            "T.TEST p-value must be in (0,1], got {}",
            p
        );
        assert!(
            (p - 0.074).abs() < 0.01,
            "T.TEST(A1:A3,B1:B3,2,1) expected ~0.074, got {}",
            p
        );
    }

    // -----------------------------------------------------------------------
    // TEXTAFTER — literal-only (no cell refs needed)
    // -----------------------------------------------------------------------
    #[test]
    fn test_wave1_textafter() {
        let result = wave1_eval(r#"=TEXTAFTER("hello-world","-")"#).expect("TEXTAFTER eval failed");
        assert_eq!(
            result,
            Value::String("world".to_string()),
            "TEXTAFTER(\"hello-world\",\"-\") should be \"world\""
        );
    }

    // -----------------------------------------------------------------------
    // TEXTBEFORE — literal-only
    // -----------------------------------------------------------------------
    #[test]
    fn test_wave1_textbefore() {
        let result =
            wave1_eval(r#"=TEXTBEFORE("hello-world","-")"#).expect("TEXTBEFORE eval failed");
        assert_eq!(
            result,
            Value::String("hello".to_string()),
            "TEXTBEFORE(\"hello-world\",\"-\") should be \"hello\""
        );
    }

    // -----------------------------------------------------------------------
    // VALUETOTEXT — literal-only; converts 42 → "42"
    // -----------------------------------------------------------------------
    #[test]
    fn test_wave1_valuetotext() {
        let result = wave1_eval("=VALUETOTEXT(42)").expect("VALUETOTEXT eval failed");
        // Result is a string representation of the number
        match &result {
            Value::String(s) => assert!(
                s.contains("42"),
                "VALUETOTEXT(42) should contain \"42\", got {:?}",
                s
            ),
            other => panic!("VALUETOTEXT: expected String, got {:?}", other),
        }
    }

    // -----------------------------------------------------------------------
    // ARRAYTOTEXT — A1:A2 = [10, 20]; in single-cell context returns scalar string
    //   Expected: "{10,20}" or "10, 20" depending on Formualizer format.
    //   Must be a non-empty string that does not start with '#' (error sigil).
    //   Numeric result is NOT acceptable — ARRAYTOTEXT always returns text.
    // -----------------------------------------------------------------------
    #[test]
    fn test_wave1_arraytotext() {
        // A1:A2 = [10, 20]. Both values must appear in the output string.
        let result = wave1_eval("=ARRAYTOTEXT(A1:A2)").expect("ARRAYTOTEXT eval failed");
        match &result {
            Value::String(s) => {
                assert!(
                    !s.is_empty() && !s.starts_with('#'),
                    "ARRAYTOTEXT(A1:A2) should return non-empty, non-error string, got {:?}",
                    s
                );
                assert!(
                    s.contains("10"),
                    "ARRAYTOTEXT(A1:A2) result must contain '10', got {:?}",
                    s
                );
                assert!(
                    s.contains("20"),
                    "ARRAYTOTEXT(A1:A2) result must contain '20', got {:?}",
                    s
                );
            }
            other => panic!(
                "ARRAYTOTEXT: expected String result (the function always returns text), got {:?}",
                other
            ),
        }
    }
    ...
    // =======================================================================
    // Wave 2 — distribution / descriptive / math value tests
    //
    // Uses the same wave1_eval() helper (same 3-row dataset):
    //   col-a = {10, 20, 30}   (mapped to A column)
    //   col-b = {1,  2,  3}    (mapped to B column)
    //
    // Floating-point comparisons use assert_approx() with 1e-6 tolerance.
    // Exact values (0.0, 0.5, 1.0, etc.) use assert_eq! directly.
    // =======================================================================

    fn assert_approx(val: &Value, expected: f64, tol: f64, msg: &str) {
        let v = val
            .as_f64()
            .unwrap_or_else(|| panic!("{}: expected f64, got {:?}", msg, val));
        assert!(
            (v - expected).abs() <= tol,
            "{}: expected {} ± {}, got {}",
            msg,
    ...
    #[test]
    fn test_wave2_norm_dist() {
        // NORM.DIST(0, 0, 1, TRUE) = Φ(0) = 0.5 exactly
        let r = wave1_eval("=NORM.DIST(0, 0, 1, TRUE)").expect("NORM.DIST eval failed");
        assert_approx(&r, 0.5, 1e-8, "NORM.DIST(0,0,1,TRUE)");
    }

    #[test]
    fn test_wave2_norm_inv() {
        // NORM.INV(0.5, 0, 1) = 0.0 (median of standard normal)
        let r = wave1_eval("=NORM.INV(0.5, 0, 1)").expect("NORM.INV eval failed");
        assert_approx(&r, 0.0, 1e-10, "NORM.INV(0.5,0,1)");
    }

    #[test]
    fn test_wave2_norm_s_dist() {
        // NORM.S.DIST(0, TRUE) = 0.5
        let r = wave1_eval("=NORM.S.DIST(0, TRUE)").expect("NORM.S.DIST eval failed");
        assert_approx(&r, 0.5, 1e-8, "NORM.S.DIST(0,TRUE)");
    }

    #[test]
    fn test_wave2_norm_s_inv() {
        // NORM.S.INV(0.5) = 0.0
        let r = wave1_eval("=NORM.S.INV(0.5)").expect("NORM.S.INV eval failed");
        assert_approx(&r, 0.0, 1e-10, "NORM.S.INV(0.5)");
    }

    // ── t-distribution ────────────────────────────────────────────────────

    #[test]
    fn test_wave2_t_dist() {
        // T.DIST(0, 5, TRUE) = 0.5 (symmetric around 0)
        let r = wave1_eval("=T.DIST(0, 5, TRUE)").expect("T.DIST eval failed");
        assert_approx(&r, 0.5, 1e-10, "T.DIST(0,5,TRUE)");
    }

    #[test]
    fn test_wave2_t_inv() {
        // T.INV(0.5, 5) = 0.0 (median of t distribution)
        let r = wave1_eval("=T.INV(0.5, 5)").expect("T.INV eval failed");
        assert_approx(&r, 0.0, 1e-10, "T.INV(0.5,5)");
    }

    // ── Chi-squared distribution ──────────────────────────────────────────

    #[test]
    fn test_wave2_chisq_dist() {
        // CHISQ.DIST(0, 2, TRUE) = 0.0 (CDF at 0 is always 0 for chi-sq)
        let r = wave1_eval("=CHISQ.DIST(0, 2, TRUE)").expect("CHISQ.DIST eval failed");
        assert_approx(&r, 0.0, 1e-10, "CHISQ.DIST(0,2,TRUE)");
    }

    #[test]
    fn test_wave2_chisq_inv() {
        // CHISQ.INV(0.5, 2) = 2*ln(2) ≈ 1.3862943611
        // Chi-sq(2) CDF: F(x) = 1 - e^(-x/2), so F^{-1}(0.5) = -2*ln(0.5) = 2*ln(2)
        let r = wave1_eval("=CHISQ.INV(0.5, 2)").expect("CHISQ.INV eval failed");
        assert_approx(
            &r,
            2.0_f64 * std::f64::consts::LN_2,
            1e-6,
            "CHISQ.INV(0.5,2)",
        );
    }
    ...
    #[test]
    fn test_wave2_f_dist() {
        // F.DIST(1, 5, 5, TRUE) = 0.5 (F(d1,d2) is symmetric around 1 when d1=d2)
        let r = wave1_eval("=F.DIST(1, 5, 5, TRUE)").expect("F.DIST eval failed");
        assert_approx(&r, 0.5, 1e-6, "F.DIST(1,5,5,TRUE)");
    }

    #[test]
    fn test_wave2_f_inv() {
        // F.INV(0.5, 5, 5) = 1.0 (median of F(5,5) = 1.0)
        let r = wave1_eval("=F.INV(0.5, 5, 5)").expect("F.INV eval failed");
        assert_approx(&r, 1.0, 1e-6, "F.INV(0.5,5,5)");
    }

    // ── Discrete distributions ────────────────────────────────────────────

    #[test]
    fn test_wave2_binom_dist() {
        // BINOM.DIST(3, 5, 0.5, TRUE) = P(X≤3) for B(5,0.5)
        // = (C(5,0)+C(5,1)+C(5,2)+C(5,3)) / 2^5 = (1+5+10+10)/32 = 26/32 = 0.8125
        let r = wave1_eval("=BINOM.DIST(3, 5, 0.5, TRUE)").expect("BINOM.DIST eval failed");
        assert_approx(&r, 0.8125, 1e-10, "BINOM.DIST(3,5,0.5,TRUE)");
    }

    #[test]
    fn test_wave2_poisson_dist() {
        // POISSON.DIST(2, 2, FALSE) = λ^k * e^(-λ) / k! = 4 * e^(-2) / 2 = 2 * e^(-2)
        let expected = 2.0_f64 * std::f64::consts::E.powf(-2.0);
        let r = wave1_eval("=POISSON.DIST(2, 2, FALSE)").expect("POISSON.DIST eval failed");
        assert_approx(&r, expected, 1e-8, "POISSON.DIST(2,2,FALSE)");
    }

    // ── Continuous distributions ──────────────────────────────────────────

    #[test]
    fn test_wave2_expon_dist() {
        // EXPON.DIST(1, 1, FALSE) = λ*e^(-λx) = 1*e^(-1) = e^(-1)
        let expected = std::f64::consts::E.powf(-1.0);
        let r = wave1_eval("=EXPON.DIST(1, 1, FALSE)").expect("EXPON.DIST eval failed");
        assert_approx(&r, expected, 1e-8, "EXPON.DIST(1,1,FALSE)");
    }

    #[test]
    fn test_wave2_gamma_dist() {
        // GAMMA.DIST(1, 1, 1, FALSE) = PDF with α=1 (exponential special case) = e^(-1)
        let expected = std::f64::consts::E.powf(-1.0);
        let r = wave1_eval("=GAMMA.DIST(1, 1, 1, FALSE)").expect("GAMMA.DIST eval failed");
        assert_approx(&r, expected, 1e-8, "GAMMA.DIST(1,1,1,FALSE)");
    }

    #[test]
    fn test_wave2_weibull_dist() {
        // WEIBULL.DIST(1, 1, 1, FALSE) = shape=1, scale=1 → (1/1)*(1/1)^0*e^(-1) = e^(-1)
        let expected = std::f64::consts::E.powf(-1.0);
        let r = wave1_eval("=WEIBULL.DIST(1, 1, 1, FALSE)").expect("WEIBULL.DIST eval failed");
        assert_approx(&r, expected, 1e-8, "WEIBULL.DIST(1,1,1,FALSE)");
    }

    #[test]
    fn test_wave2_beta_dist() {
        // BETA.DIST(0.5, 2, 2, TRUE) = 0.5 (Beta(2,2) CDF is symmetric around 0.5)
        let r = wave1_eval("=BETA.DIST(0.5, 2, 2, TRUE)").expect("BETA.DIST eval failed");
        assert_approx(&r, 0.5, 1e-8, "BETA.DIST(0.5,2,2,TRUE)");
    }

    #[test]
    fn test_wave2_negbinom_dist() {
        // NEGBINOM.DIST(0, 1, 0.5, FALSE) = C(0,0)*0.5^1*0.5^0 = 0.5
        let r = wave1_eval("=NEGBINOM.DIST(0, 1, 0.5, FALSE)").expect("NEGBINOM.DIST eval failed");
        assert_approx(&r, 0.5, 1e-10, "NEGBINOM.DIST(0,1,0.5,FALSE)");
    }

    #[test]
    fn test_wave2_hypgeom_dist() {
        // HYPGEOM.DIST(1, 2, 2, 4, FALSE) = C(2,1)*C(2,1)/C(4,2) = 2*2/6 = 2/3
        let r = wave1_eval("=HYPGEOM.DIST(1, 2, 2, 4, FALSE)").expect("HYPGEOM.DIST eval failed");
        assert_approx(&r, 2.0 / 3.0, 1e-8, "HYPGEOM.DIST(1,2,2,4,FALSE)");
    }

    // ── Inference helpers ─────────────────────────────────────────────────

    #[test]
    fn test_wave2_covariance_p() {
        // COVARIANCE.P(A1:A3, B1:B3): col-a={10,20,30}, col-b={1,2,3}
        // mean_a=20, mean_b=2
        // cov = ((10-20)(1-2)+(20-20)(2-2)+(30-20)(3-2))/3 = (10+0+10)/3 = 20/3
        let r = wave1_eval("=COVARIANCE.P(A1:A3, B1:B3)").expect("COVARIANCE.P eval failed");
        assert_approx(&r, 20.0 / 3.0, 1e-8, "COVARIANCE.P(A1:A3,B1:B3)");
    }

    #[test]
    fn test_wave2_covariance_s() {
        // COVARIANCE.S(A1:A3, B1:B3): same data, divide by n-1=2 → 20/2 = 10.0
        let r = wave1_eval("=COVARIANCE.S(A1:A3, B1:B3)").expect("COVARIANCE.S eval failed");
        assert_approx(&r, 10.0, 1e-8, "COVARIANCE.S(A1:A3,B1:B3)");
    }

    #[test]
    fn test_wave2_confidence_norm() {
        // CONFIDENCE.NORM(0.05, 1, 100) = z_{0.975} / 10 ≈ 1.959964 / 10 ≈ 0.195996
        let r = wave1_eval("=CONFIDENCE.NORM(0.05, 1, 100)").expect("CONFIDENCE.NORM eval failed");
        assert_approx(&r, 0.195996, 1e-4, "CONFIDENCE.NORM(0.05,1,100)");
    }

    #[test]
    fn test_wave2_confidence_t() {
        // CONFIDENCE.T(0.05, 1, 100) = t_{0.025,99} / 10 ≈ 1.984217 / 10 ≈ 0.198422
        let r = wave1_eval("=CONFIDENCE.T(0.05, 1, 100)").expect("CONFIDENCE.T eval failed");
        assert_approx(&r, 0.198422, 1e-4, "CONFIDENCE.T(0.05,1,100)");
    }

    #[test]
    fn test_wave2_standardize() {
        // STANDARDIZE(10, 20, 10) = (10 - 20) / 10 = -1.0
        let r = wave1_eval("=STANDARDIZE(10, 20, 10)").expect("STANDARDIZE eval failed");
        assert_approx(&r, -1.0, 1e-10, "STANDARDIZE(10,20,10)");
    }

    // ── Descriptive stats ────────────────────────────────────────────────

    #[test]
    fn test_wave2_avedev() {
        // AVEDEV(A1:A3): col-a={10,20,30}, mean=20
        // = (|10-20|+|20-20|+|30-20|)/3 = 20/3 ≈ 6.6667
        let r = wave1_eval("=AVEDEV(A1:A3)").expect("AVEDEV eval failed");
        assert_approx(&r, 20.0 / 3.0, 1e-8, "AVEDEV(A1:A3)");
    }

    #[test]
    fn test_wave2_geomean() {
        // GEOMEAN(A1:A3): (10*20*30)^(1/3) = 6000^(1/3) ≈ 18.1712
        let expected = 6000.0_f64.powf(1.0 / 3.0);
        let r = wave1_eval("=GEOMEAN(A1:A3)").expect("GEOMEAN eval failed");
        assert_approx(&r, expected, 1e-6, "GEOMEAN(A1:A3)");
    }

    #[test]
    fn test_wave2_harmean() {
        // HARMEAN(A1:A3): 3/(1/10+1/20+1/30) = 3/(11/60) = 180/11 ≈ 16.3636
        let expected = 180.0_f64 / 11.0;
        let r = wave1_eval("=HARMEAN(A1:A3)").expect("HARMEAN eval failed");
        assert_approx(&r, expected, 1e-6, "HARMEAN(A1:A3)");
    }

    // ── Math ─────────────────────────────────────────────────────────────

    #[test]
    fn test_wave2_product() {
        // PRODUCT(A1:A3) = 10 * 20 * 30 = 6000
        let r = wave1_eval("=PRODUCT(A1:A3)").expect("PRODUCT eval failed");
        assert_eq!(r, Value::from(6000.0_f64), "PRODUCT(A1:A3) should be 6000");
    }

    // GAMMALN: not in formualizer-eval 0.4.3 builtins — handled by FFP sync path.
    // No Rust value test; covered by FFP sync evaluation.

    // ── Wave 3 — LET + LAMBDA ────────────────────────────────────────────────

    #[test]
    fn test_wave3_let_literal() {
        // LET(x, 5, x*2) → 10
        let r = wave1_eval("=LET(x,5,x*2)").expect("LET eval failed");
        assert_eq!(r, Value::from(10.0_f64), "LET(x,5,x*2) should be 10");
    }

    #[test]
    fn test_wave3_lambda_via_let() {
        // formualizer-eval parser does not support immediate-invocation syntax LAMBDA(...)(arg).
        // Standard Excel form binds via LET first: LET(f, LAMBDA(x, x*2), f(5)) → 10
        let r = wave1_eval("=LET(f,LAMBDA(x,x*2),f(5))").expect("LAMBDA via LET eval failed");
        assert_eq!(
            r,
            Value::from(10.0_f64),
            "LET(f,LAMBDA(x,x*2),f(5)) should be 10"
        );
    }

    #[test]
    fn test_wave3_lambda_self_invoke_unsupported() {
        // Documents a known parser limitation: =LAMBDA(x,x*2)(5) immediate self-invocation
        // is NOT supported by the formualizer-eval parser. The trailing '(' after the closing
        // ')' of the LAMBDA body is treated as an unexpected token.
        // This test guards against silent regression if parser support is added in future —
        // when it starts returning Ok, promote routingFormula in wave3Registry.ts.
        let result = wave1_eval("=LAMBDA(x,x*2)(5)");
        assert!(
            result.is_err(),
            "Expected parse error for self-invoke LAMBDA syntax, but got: {:?}",
            result
        );
    }
}
