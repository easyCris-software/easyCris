...
use crate::modules::python_backend::{detect_rnaseq_mode, spawn_rnaseq, BackendMode};
...
pub async fn run_rnaseq_analysis(
    ...
    let mode = detect_rnaseq_mode();
    match spawn_rnaseq(payload.clone(), mode, &app).await {
        Ok(result) => Ok(result),
        Err(primary_error) => {
            ...
            match spawn_rnaseq(payload, BackendMode::Script, &app).await {
                Ok(result) => Ok(result),
                Err(fallback_error) => {
