...
use crate::modules::hybrid_cache_manager::{
    ...
    PasteBlockPayload, PasteBlockResult, HYBRID_CACHE,
};
...
async fn run_cache_blocking<T, F>(label: &'static str, work: F) -> Result<T, String>
where
    T: Send + 'static,
...
pub async fn update_cell(
    ...
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("update_cell", move || {
...
pub async fn update_cells_batch(
    ...
    // Try HYBRID_CACHE first (handles large DuckDB datasets + in-memory via HYBRID_CACHE)
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("update_cells_batch", move || {
            HYBRID_CACHE.update_cells_batch(&dataset_id, updates)
        })
        .await;
    }
    // Fall back to legacy CACHE for small datasets that haven't been migrated
...
#[command]
pub async fn apply_paste_block(
    dataset_id: String,
    payload: PasteBlockPayload,
) -> Result<PasteBlockResult, String> {
    run_cache_blocking("apply_paste_block", move || {
        HYBRID_CACHE.apply_paste_block(&dataset_id, payload)
    })
    .await
}
...
pub async fn get_column_data(dataset_id: String, column_id: String) -> Result<Vec<Value>, String> {
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("get_column_data", move || {
            HYBRID_CACHE.get_column_data(&dataset_id, &column_id).ok_or_else(|| {
                format!(
...
pub async fn get_columns_data(
    ...
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("get_columns_data", move || {
...
pub async fn get_columns_sampled_data(
    ...
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("get_columns_sampled_data", move || {
            HYBRID_CACHE.get_columns_sampled_data(&dataset_id, &column_ids, sample_size, seed)
        })
        .await;
    }
...
pub async fn get_columns_aggregated_data(
    ...
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("get_columns_aggregated_data", move || {
            HYBRID_CACHE.get_columns_aggregated_data(&dataset_id, &group_by, &aggregations)
        })
        .await;
    }
...
pub async fn search_columns_values(
    ...
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("search_columns_values", move || {
            HYBRID_CACHE.search_columns_values(
                &dataset_id,
                &column_ids,
                &search_text,
                case_sensitive,
...
pub async fn flush_dataset_to_arrow(dataset_id: String) -> Result<String, String> {
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("flush_dataset_to_arrow", move || {
...
pub async fn get_rows(
    ...
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("get_rows", move || {
            HYBRID_CACHE
...
pub async fn add_column(
    ...
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("add_column", move || {
            HYBRID_CACHE.add_column(&dataset_id, &column_id, default_value)
        })
        .await;
    }
...
pub async fn insert_row_at(dataset_id: String, row_index: usize) -> Result<usize, String> {
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("insert_row_at", move || {
...
/// Existing rows at/after the index are shifted down by `count`.
#[command]
pub async fn insert_rows_at(
    dataset_id: String,
    row_index: usize,
    count: usize,
) -> Result<usize, String> {
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("insert_rows_at", move || {
            HYBRID_CACHE.insert_rows_at(&dataset_id, row_index, count)
        })
        .await;
    }
    CACHE.insert_rows_at(&dataset_id, row_index, count)
}

#[command]
pub async fn append_rows(dataset_id: String, count: usize) -> Result<usize, String> {
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("append_rows", move || {
            HYBRID_CACHE.append_rows(&dataset_id, count)
        })
        .await;
    }
    CACHE.append_rows(&dataset_id, count)
}

#[command]
pub async fn remove_rows_from_end(dataset_id: String, count: usize) -> Result<usize, String> {
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("remove_rows_from_end", move || {
            HYBRID_CACHE.remove_rows_from_end(&dataset_id, count)
        })
        .await;
    }
    CACHE.remove_rows_from_end(&dataset_id, count)
}
...
pub async fn remove_row_at(dataset_id: String, row_index: usize) -> Result<usize, String> {
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("remove_row_at", move || {
...
pub async fn remove_column(dataset_id: String, column_id: String) -> Result<usize, String> {
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("remove_column", move || {
...
pub async fn create_empty_duckdb(
    ...
    run_cache_blocking("create_empty_duckdb", move || {
...
/// For DuckDB-backed datasets, reads physical schema.
/// For in-memory datasets, returns union of keys across rows.
#[command]
pub async fn get_persisted_column_ids(dataset_id: String) -> Result<Vec<String>, String> {
    if HYBRID_CACHE.has_dataset(&dataset_id) {
        return run_cache_blocking("get_persisted_column_ids", move || {
            HYBRID_CACHE
                .get_persisted_column_ids(&dataset_id)
                .ok_or_else(|| format!("Dataset '{}' not found", dataset_id))
        })
        .await;
    }
...
pub async fn get_aggregates_for_test(
    ...
    let test_name_for_result = test_name.clone();
    let aggregates = run_cache_blocking("get_aggregates_for_test", move || {
        HYBRID_CACHE.get_aggregates_for_test(
            &dataset_id,
            &test_name,
    ...
    Ok(TestAggregatesResult {
        test_name: test_name_for_result,
        aggregates,
    })
}
...
pub async fn ensure_duckdb_dataset(
    ...
    let (is_large, duckdb_path) = run_cache_blocking("ensure_duckdb_dataset", move || {
        HYBRID_CACHE.ensure_duckdb_dataset(&dataset_id, column_info)
    })
    .await?;
...
/// Warm DuckDB connection and structural metadata tables for a dataset.
#[command]
pub async fn prewarm_dataset_infrastructure(dataset_id: String) -> Result<(), String> {
    tokio::task::spawn_blocking(move || HYBRID_CACHE.prewarm_dataset_infrastructure(&dataset_id))
        .await
        .map_err(|error| format!("Dataset infrastructure warmup worker failed: {error}"))?
}
...
pub async fn flush_overlay(dataset_id: String) -> Result<(), String> {
    run_cache_blocking("flush_overlay", move || {
...
pub async fn get_sorted_row_indices(
    ...
    run_cache_blocking("get_sorted_row_indices", move || {
...
pub async fn get_grouped_row_order(
    ...
    let result = run_cache_blocking("get_grouped_row_order", move || {
        HYBRID_CACHE.get_grouped_row_order(
            &dataset_id,
            &group_column,
...
pub async fn get_lazy_group_metadata(
    ...
    let result = run_cache_blocking("get_lazy_group_metadata", move || {
        HYBRID_CACHE.get_lazy_group_metadata(
            &dataset_id,
            &group_column,
            sort_column.as_deref(),
            descending,
...
pub async fn get_group_rows(
    ...
    run_cache_blocking("get_group_rows", move || {
...
pub async fn import_large_csv(
    ...
    run_cache_blocking("import_large_csv", move || {
...
pub async fn import_large_parquet(
    ...
    run_cache_blocking("import_large_parquet", move || {
...
pub async fn import_large_csv_with_project(
    ...
    run_cache_blocking("import_large_csv_with_project", move || {
...
pub async fn import_large_parquet_with_project(
    ...
    run_cache_blocking("import_large_parquet_with_project", move || {
...
pub async fn set_dataset_hybrid(
    ...
    run_cache_blocking("set_dataset_hybrid", move || {
...
pub async fn get_rows_hybrid(
    ...
    run_cache_blocking("get_rows_hybrid", move || {
        HYBRID_CACHE
            .get_rows_range(&dataset_id, start_row, end_row)
            .ok_or_else(|| format!("Dataset '{}' not found", dataset_id))
    })
...
/// Get selected columns from hybrid cache rows.
#[command]
pub async fn get_rows_hybrid_columns(
    dataset_id: String,
    start_row: usize,
    end_row: usize,
    column_ids: Vec<String>,
) -> Result<Vec<HashMap<String, Value>>, String> {
    run_cache_blocking("get_rows_hybrid_columns", move || {
        HYBRID_CACHE.get_rows_range_columns(&dataset_id, start_row, end_row, &column_ids)
    })
    .await
}
...
pub async fn get_column_data_hybrid(
    ...
    run_cache_blocking("get_column_data_hybrid", move || {
...
pub async fn update_cell_hybrid(
    ...
    run_cache_blocking("update_cell_hybrid", move || {
        HYBRID_CACHE.update_cell(&dataset_id, row, &column, value)
    })
    .await
}
...
pub async fn flush_to_arrow_hybrid(dataset_id: String) -> Result<String, String> {
    run_cache_blocking("flush_to_arrow_hybrid", move || {
        HYBRID_CACHE.flush_to_arrow(&dataset_id)
    })
    .await
}
...
pub async fn export_columns_to_arrow_hybrid(
    ...
    run_cache_blocking("export_columns_to_arrow_hybrid", move || {
...
pub async fn get_column_aggregate(
    ...
    let log_dataset_id = dataset_id.clone();
    let log_column_id = column_id.clone();
    let log_func = func.clone();
    let result = run_cache_blocking("get_column_aggregate", move || {
        HYBRID_CACHE.get_column_aggregate(&dataset_id, &column_id, &func)
    })
    .await;
...
pub async fn get_column_aggregate_range(
    ...
    let log_column_id = column_id.clone();
    let log_func = func.clone();
    let result = run_cache_blocking("get_column_aggregate_range", move || {
...
pub async fn get_column_aggregate_rows(
    ...
    let log_column_id = column_id.clone();
    let log_func = func.clone();
    let result = run_cache_blocking("get_column_aggregate_rows", move || {
...
pub async fn get_all_column_stats(
    ...
    run_cache_blocking("get_all_column_stats", move || {
...
pub async fn get_column_duplicate_summary(
    ...
    run_cache_blocking("get_column_duplicate_summary", move || {
        HYBRID_CACHE.get_column_duplicate_summary(
            &dataset_id,
            &column_id,
            max_examples.unwrap_or(5),
        )
    })
    .await
}
...
pub async fn register_existing_duckdb(
    ...
    run_cache_blocking("register_existing_duckdb", move || {
...
mod tests {
    ...
    fn test_release_like_directory_validation_allows_absolute_dir_without_extension() {
        let dir_path = std::env::temp_dir().join("easycris_project_data_dir");
        assert!(
