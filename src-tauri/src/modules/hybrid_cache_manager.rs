...
/// Phase 5: Reduced from 10,000 to 1,000 (lower data loss risk window)
const OVERLAY_FLUSH_THRESHOLD: usize = 1_000;
...
impl HybridCacheManager {
    ...
    pub fn get_rows_range(
        ...
        match datasets.get(&namespaced_key)? {
            ...
            DatasetStorage::DuckDB { columns, .. } => {
                ...
                let sql = format!(
                    "SELECT _row_index, {} FROM data WHERE _row_index >= ? AND _row_index < ? ORDER BY _row_index",
                    col_sql
                );
                ...
                // Capture namespaced_key for closure
                let ns_key = namespaced_key.clone();
                let rows: Vec<HashMap<String, Value>> = stmt
                    .query_map(params![start_row as i64, end_row as i64], |row| {
                        let row_idx: i64 = row.get(0)?;
                        let mut map = HashMap::new();

                        for (i, col_name) in col_names.iter().enumerate() {
                            // Check overlay first (pending edits) - Phase B: Use namespaced key
                            let key = (ns_key.clone(), row_idx, col_name.clone());
                            let value = if let Some(overlay_val) = overlay.get(&key) {
                                overlay_val.clone()
                            } else {
                                // Get from DuckDB with proper type handling
    ...
    pub fn update_cell(
        ...
        match datasets.get(&namespaced_key) {
            ...
            Some(DatasetStorage::DuckDB { .. }) => {
                ...
                // row is 0-based, matching _row_index
    ...
    pub fn insert_row_at(&self, dataset_id: &str, row_index: usize) -> Result<usize, String> {
        ...
        match datasets.get(&namespaced_key) {
            ...
            Some(DatasetStorage::DuckDB { row_count, .. }) => {
                ...
                let row_idx = clamped_row_index as i64;
                let shift_guard: i64 = 1_000_000_000;

                let transaction_result: Result<(), String> = (|| {
                    conn.execute("BEGIN TRANSACTION", [])
                        .map_err(|e| e.to_string())?;

                    // Two-phase shift prevents transient PK collisions on _row_index.
                    conn.execute(
                        "UPDATE data SET _row_index = _row_index + ? WHERE _row_index >= ?",
                        params![shift_guard, row_idx],
                    )
                    .map_err(|e| e.to_string())?;

                    conn.execute(
                        "UPDATE data SET _row_index = _row_index - ? WHERE _row_index >= ?",
                        params![shift_guard - 1, row_idx + shift_guard],
                    )
                    .map_err(|e| e.to_string())?;

                    conn.execute("INSERT INTO data (_row_index) VALUES (?)", params![row_idx])
                        .map_err(|e| e.to_string())?;
        ...
        match datasets.get(&namespaced_key) {
            ...
            Some(DatasetStorage::DuckDB { row_count, .. }) => {
                ...
                let transaction_result: Result<(), String> = (|| {
                    ...
                    conn.execute("DELETE FROM data WHERE _row_index = ?", params![row_idx])
                        .map_err(|e| e.to_string())?;
                    ...
                    conn.execute(
                        "UPDATE data SET _row_index = _row_index - ? WHERE _row_index >= ?",
                        params![shift_guard + 1, row_idx + 1 + shift_guard],
                    )
                    .map_err(|e| e.to_string())?;
    ...
    pub fn flush_overlay(&self, dataset_id: &str) -> Result<(), String> {
        ...
        // Collect edits while holding overlay lock
        let edits: Vec<(i64, String, Value)>;
        {
            let overlay = self.overlay.read().unwrap();
            edits = overlay
                .iter()
                .filter(|((ds_id, _, _), val)| ds_id == &namespaced_key && !is_calc_pending(val))
                .map(|((_, row, col), val)| (*row, col.clone(), val.clone()))
                .collect();
        }
        ...
        // Ensure all columns exist in DuckDB before updates (protects auto-flush)
        let mut added_columns: Vec<ColumnInfo> = Vec::new();
        ...
        // Group edits by column for efficient prepared statements
        let mut by_column: HashMap<String, Vec<(i64, Value)>> = HashMap::new();
        for (row_idx, col, val) in edits.iter() {
            by_column
                .entry(col.clone())
                .or_default()
                .push((*row_idx, val.clone()));
        }

        for (col, updates) in by_column.iter() {
            if col != "_row_index" && !table_columns.contains(col.as_str()) {
                // Column missing in DuckDB: add it based on the first non-null value.
                let sample = updates
                    .iter()
                ...
                added_columns.push(ColumnInfo {
                    name: col.clone(),
                    display_name: col.clone(),
                    dtype: sql_type.to_string(),
                });
            }
        }

        // Clear any aborted transaction state from a previous failure
        // (DuckDB will reject new statements until a ROLLBACK is issued).
        let _ = conn.execute("ROLLBACK", []);
        ...
        let transaction_result: Result<(), String> = (|| {
            ...
            // Ensure rows exist before applying updates (handles blank datasets and buffer rows)
            let mut unique_rows: HashSet<i64> = HashSet::new();
            for (row_idx, _, _) in edits.iter() {
                unique_rows.insert(*row_idx);
            }

            if !unique_rows.is_empty() {
                let insert_sql = "INSERT INTO data (_row_index) SELECT ? WHERE NOT EXISTS (SELECT 1 FROM data WHERE _row_index = ?)";
                let mut insert_stmt = conn.prepare(insert_sql).map_err(|e| e.to_string())?;
                for row_idx in unique_rows {
                    insert_stmt
                        .execute(params![row_idx, row_idx])
                        .map_err(|e| e.to_string())?;
                }
            }

            for (col, updates) in by_column {
                // Use proper SQL identifier quoting (not character replacement)
                let quoted_col = Self::quote_identifier(&col);

                // Prepared statement with parameter binding (safe from injection)
                let sql = format!("UPDATE data SET {} = ? WHERE _row_index = ?", quoted_col);
                let mut stmt = conn.prepare(&sql).map_err(|e| e.to_string())?;

                for (row_idx, val) in updates {
                    // Convert Value to DuckDB-compatible parameter
                    // CRITICAL: Empty/whitespace strings must become NULL to avoid type conversion errors
                    // (e.g., DuckDB can't convert '' to DOUBLE for numeric columns)
                    match val {
                        Value::Null => stmt.execute(params![Option::<String>::None, row_idx]),
                        Value::Bool(b) => stmt.execute(params![b, row_idx]),
                        Value::Number(n) => {
                            if let Some(i) = n.as_i64() {
                                stmt.execute(params![i, row_idx])
                            } else if let Some(f) = n.as_f64() {
                                stmt.execute(params![f, row_idx])
                            } else {
                                stmt.execute(params![n.to_string(), row_idx])
                            }
                        }
                        Value::String(s) => {
                            // Treat empty/whitespace strings as NULL to prevent type conversion errors
                            if s.trim().is_empty() {
                                stmt.execute(params![Option::<String>::None, row_idx])
                            } else {
                                stmt.execute(params![s, row_idx])
                            }
                        }
                        _ => stmt.execute(params![val.to_string(), row_idx]),
                    }
                    .map_err(|e| e.to_string())?;
                }
            }
        ...
        if let Some(max_row_idx) = edits.iter().map(|(row, _, _)| *row).max() {
            if max_row_idx >= 0 {
        ...
        // Phase 3: Invalidate row-range cache for this dataset
        // Data in DuckDB has changed, cached ranges are now stale
        self.invalidate_row_range_cache(&namespaced_key);
        self.invalidate_column_stats_cache(&namespaced_key);
    ...
    fn get_table_columns(&self, conn: &Connection) -> Result<Vec<ColumnInfo>, String> {
