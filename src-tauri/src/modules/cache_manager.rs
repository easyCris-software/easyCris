...
fn validate_user_column_id(column_id: &str) -> Result<(), String> {
    if column_id == RESERVED_VIRTUAL_COLUMN_ID {
...
impl CacheManager {
    ...
    /// Insert `count` empty rows at `row_index`.
    ...
    pub fn insert_rows_at(
        ...
        if let Some(dataset) = cache.get_mut(dataset_id) {
            ...
            for offset in 0..count {
                dataset.insert(insert_at + offset, HashMap::new());
            }
    ...
    /// Append `count` empty rows without shifting existing rows.
    ...
    pub fn remove_rows_from_end(&self, dataset_id: &str, count: usize) -> Result<usize, String> {
        ...
        if let Some(dataset) = cache.get_mut(dataset_id) {
            ...
            dataset.truncate(dataset.len() - count);
            Ok(dataset.len())
        } else {
            Err(format!("Dataset '{}' not found in cache", dataset_id))
        }
    ...
    /// For in-memory datasets, this is the union of keys across all rows.
    pub fn get_persisted_column_ids(&self, dataset_id: &str) -> Option<Vec<String>> {
        let cache = self.datasets.lock().unwrap();
        let dataset = cache.get(dataset_id)?;
...
mod tests {
    ...
    #[test]
    fn test_insert_rows_at_inserts_count_rows_and_shifts_data() {
        let cache = CacheManager::new();
    ...
    fn test_insert_rows_at_zero_count_is_noop() {
        ...
        cache.set_dataset("ds", vec![row]);

        let new_count = cache
            .insert_rows_at("ds", 0, 0)
            .expect("insert_rows_at should succeed");
    ...
    #[test]
    fn test_insert_rows_at_clamps_index_beyond_length() {
        let cache = CacheManager::new();
    ...
    #[test]
    fn test_append_rows_does_not_shift_existing_data() {
        let cache = CacheManager::new();
    ...
    #[test]
    fn test_append_rows_zero_count_is_noop() {
        let cache = CacheManager::new();
    ...
    #[test]
    fn test_remove_rows_from_end_removes_only_tail_rows() {
        let cache = CacheManager::new();
        ...
        cache.set_dataset("ds", vec![row1, row2, row3]);

        let new_count = cache
            .remove_rows_from_end("ds", 2)
    ...
    #[test]
    fn test_remove_rows_from_end_zero_count_is_noop() {
        let cache = CacheManager::new();
    ...
    #[test]
    fn test_update_cells_batch_rejects_reserved_virtual_column_id() {
        let cache = CacheManager::new();
        cache.set_dataset("ds", vec![HashMap::new()]);
    ...
    #[test]
    fn test_update_cells_batch_rejects_reserved_virtual_column_id_atomically() {
        let cache = CacheManager::new();
        let mut row = HashMap::new();
        ...
        let dataset = cache.get_dataset("ds").expect("dataset should exist");
        assert_eq!(dataset[0].get("col-0"), Some(&Value::from("original")));
    }

    #[test]
    fn test_update_cell_rejects_reserved_virtual_column_id() {
        let cache = CacheManager::new();
        cache.set_dataset("ds", vec![HashMap::new()]);

        let err = cache
            .update_cell("ds", 0, "__add_column__", Value::from("x"))
            .expect_err("reserved virtual column id should be rejected");
        assert_eq!(
            err,
            "Reserved virtual column id '__add_column__' is not writable"
    ...
    #[test]
    fn test_add_column_rejects_reserved_virtual_column_id() {
        let cache = CacheManager::new();
        cache.set_dataset("ds", vec![HashMap::new()]);

        let err = cache
            .add_column("ds", "__add_column__", Value::Null)
            .expect_err("reserved virtual column id should be rejected");
        assert_eq!(
            err,
            "Reserved virtual column id '__add_column__' is not writable"
