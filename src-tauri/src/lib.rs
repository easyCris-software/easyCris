...
#[cfg(windows)]
fn disable_default_webview_context_menus(window: &tauri::WebviewWindow) {
    let label = window.label().to_string();
    if let Err(dispatch_error) = window.with_webview(move |webview| {
        // WebView2 default editable-field context menus bypass the DOM-level
        // contextmenu guard, so disable them at the host boundary.
        unsafe {
            let result = webview
                .controller()
                .CoreWebView2()
            ...
            if let Err(com_error) = result {
                log::warn!(
                    "Failed to disable default WebView2 context menus for '{}': {com_error}",
                    label
                );
            }
        }
    }) {
        log::warn!("Failed to access webview for context menu policy: {dispatch_error}");
    }
}
...
static PENDING_REMOTE_JOIN_URL: std::sync::Mutex<Option<String>> = std::sync::Mutex::new(None);
const REMOTE_JOIN_SCHEME: &str = "easycris-remote://";
...
fn should_disable_single_instance(
    build_profile: Option<&str>,
    disable_single_instance: Option<&str>,
) -> bool {
    build_profile == Some("e2e")
        || disable_single_instance
            .map(parse_truthy_flag)
            .unwrap_or(false)
}
...
#[cfg(windows)]
fn maybe_yield_for_windows_webview_resize(event: &tauri::WindowEvent) {
    if !matches!(event, tauri::WindowEvent::Resized(_)) {
        return;
    }
    // Tauri/WebView2 can present a black/white resize flash before Chromium
    // repaints. Yield the current timeslice without sleeping for a full
    // scheduler quantum on the Win32 message-pump thread.
    // https://github.com/tauri-apps/tauri/issues/6322#issuecomment-1448141495
    // https://github.com/tauri-apps/tauri/issues/6322#issuecomment-1437047847
    std::thread::yield_now();
}

#[cfg(not(windows))]
fn maybe_yield_for_windows_webview_resize(_event: &tauri::WindowEvent) {}
...
fn remote_join_url_from_args(args: &[String]) -> Option<String> {
    args.iter()
        .find_map(|arg| remote_join_url_from_raw_command_line(arg.trim_matches('"')))
}

fn remote_join_url_from_raw_command_line(raw_command_line: &str) -> Option<String> {
    let start = raw_command_line.char_indices().find_map(|(index, _)| {
        raw_command_line
            .get(index..index + REMOTE_JOIN_SCHEME.len())
            .filter(|candidate| candidate.eq_ignore_ascii_case(REMOTE_JOIN_SCHEME))
            .map(|_| index)
    })?;
    let candidate = raw_command_line[start..]
...
#[cfg(windows)]
fn remote_join_url_from_current_command_line() -> Option<String> {
    #[link(name = "kernel32")]
    extern "system" {
    ...
    unsafe {
        ...
        let raw = String::from_utf16_lossy(std::slice::from_raw_parts(ptr, len));
        remote_join_url_from_raw_command_line(&raw)
    }
}
...
fn startup_remote_join_url_from_args(args: &[String]) -> Option<String> {
    remote_join_url_from_args(args).or_else(remote_join_url_from_current_command_line)
}

#[tauri::command]
fn take_pending_remote_join_url() -> Option<String> {
    PENDING_REMOTE_JOIN_URL
        .lock()
        .ok()
        .and_then(|mut pending| pending.take())
}
...
#[tauri::command]
fn current_remote_join_url() -> Option<String> {
    remote_join_url_from_current_command_line()
}

#[tauri::command]
fn apply_webview_chrome_policy(app: AppHandle, label: String) -> Result<(), String> {
    let Some(window) = app.get_webview_window(&label) else {
        return Err(format!("Webview window '{label}' not found"));
    };

    #[cfg(windows)]
    disable_default_webview_context_menus(&window);
...
fn deliver_remote_join_url_to_frontend(app: &AppHandle, remote_url: String) {
    // Emit through both channels: Tauri events cover an already-loaded frontend,
    // while the DOM event reaches the WebView eval path used by protocol tests.
    // Frontend storage is idempotent for the same URL.
    let _ = app.emit("remote-join-link", remote_url.clone());

    let Some(main_window) = app.get_webview_window("main") else {
        log::warn!("Main window not ready for remote invite protocol delivery");
        return;
    };
    let Ok(serialized_url) = serde_json::to_string(&remote_url) else {
        log::warn!("Unable to serialize remote invite URL for frontend delivery");
        return;
    };
    let script = format!(
        "window.dispatchEvent(new CustomEvent('easycris-remote-join-link', {{ detail: {url} }}));",
        url = serialized_url
    );
...
#[cfg(test)]
mod remote_join_link_tests {
    use super::{
        remote_join_url_from_args, remote_join_url_from_raw_command_line,
        should_disable_single_instance, should_intercept_window_close,
    };

    #[test]
    fn finds_easycris_remote_url_argument() {
        let args = vec![
            "easycris.exe".to_string(),
            "easycris-remote://join?host=127.0.0.1:49152&session=s&token=t".to_string(),
        ];

        assert_eq!(
            remote_join_url_from_args(&args).as_deref(),
            Some("easycris-remote://join?host=127.0.0.1:49152&session=s&token=t")
        );
    }

    #[test]
    fn finds_windows_shortcut_normalized_remote_url_argument() {
        let args = vec![
            "easycris.exe".to_string(),
            "\"easycris-remote://join/?host=127.0.0.1:49152&session=s&token=t\"".to_string(),
        ];
    ...
    #[test]
    fn finds_remote_url_inside_raw_windows_command_line() {
        let raw = "\"C:\\Program Files\\easyCris\\easycris.exe\" \"easycris-remote://join/?host=127.0.0.1:49152&session=s&token=t\"";

        assert_eq!(
            remote_join_url_from_raw_command_line(raw).as_deref(),
            Some("easycris-remote://join/?host=127.0.0.1:49152&session=s&token=t")
        );
    }

    #[test]
    fn finds_remote_url_after_non_ascii_command_line_prefix() {
        let raw = "\"C:\\Users\\Ä°nci\\easyCris\\easycris.exe\" \"easycris-remote://join/?host=127.0.0.1:49152&session=s&token=t\"";
    ...
    #[test]
    fn finds_remote_url_embedded_in_argument() {
        let args = vec![
            "easycris.exe".to_string(),
            "--protocol=easycris-remote://join?host=127.0.0.1:49152&session=s&token=t".to_string(),
        ];
    ...
    #[test]
    fn ignores_non_remote_url_arguments() {
        let args = vec![
            "easycris.exe".to_string(),
            "https://example.test".to_string(),
        ];

        assert!(remote_join_url_from_args(&args).is_none());
    }

    #[test]
    fn disables_single_instance_for_e2e_profile() {
        assert!(should_disable_single_instance(Some("e2e"), None));
    }

    #[test]
    fn disables_single_instance_for_remote_dev_pair_flag() {
        assert!(should_disable_single_instance(Some("dev"), Some("1")));
        assert!(should_disable_single_instance(Some("dev"), Some("true")));
    }

    #[test]
    fn keeps_single_instance_for_normal_dev_and_release_builds() {
        assert!(!should_disable_single_instance(Some("dev"), None));
        assert!(!should_disable_single_instance(Some("release"), None));
        assert!(!should_disable_single_instance(Some("dev"), Some("0")));
    }

    #[test]
    fn intercepts_close_only_for_main_window() {
        assert!(should_intercept_window_close("main"));
        assert!(!should_intercept_window_close("remote-host-controls"));
        assert!(!should_intercept_window_close("secondary"));
    }
}
...
#[tauri::command]
async fn desktop_auth_poll(device_code: String) -> Result<DesktopAuthPollResponse, String> {
...
async fn enable_ecp_file_association(app: AppHandle) -> Result<String, String> {
    ...
    let mut ext_command = Command::new("reg");
    apply_no_window(&mut ext_command);
    let ext_output = ext_command
        .args(&["query", "HKCU\\Software\\Classes\\.ecp", "/ve"])
        .output()
        .map_err(|e| format!("Failed to query .ecp association: {}", e))?;

    let mut cmd_command = Command::new("reg");
    apply_no_window(&mut cmd_command);
    let cmd_output = cmd_command
        .args(&[
            "query",
...
pub fn run() {
    ...
    let startup_remote_join_url = startup_remote_join_url_from_args(&startup_args);
    if let Some(remote_url) = startup_remote_join_url.clone() {
        if let Ok(mut pending) = PENDING_REMOTE_JOIN_URL.lock() {
            *pending = Some(remote_url);
        }
    }
    ...
    let builder = tauri::Builder::default()
        ...
        .on_window_event(|window, event| {
            maybe_yield_for_windows_webview_resize(event);
    ...
    let builder = if should_disable_single_instance(
        option_env!("EASYCRIS_BUILD_PROFILE"),
        option_env!("EASYCRIS_DISABLE_SINGLE_INSTANCE"),
    ) {
        builder
    } else {
        builder.plugin(tauri_plugin_single_instance::init(|app, argv, _cwd| {
            if let Some(remote_url) = remote_join_url_from_args(&argv) {
                if let Ok(mut pending) = PENDING_REMOTE_JOIN_URL.lock() {
                    *pending = Some(remote_url.clone());
                }
                deliver_remote_join_url_to_frontend(app, remote_url);
            }
    ...
    builder
        ...
        .manage(modules::remote_session::RemoteSessionState::default())
        .setup(move |app| {
            ...
            if let Some(main_window) = app.get_webview_window("main") {
                #[cfg(windows)]
                disable_default_webview_context_menus(&main_window);
            ...
            if startup_remote_join_url.is_some() {
                log::info!(
                    "Stored remote invite from CLI/protocol activation for frontend startup"
                );
            }
            ...
            let cli_ecp = startup_args.iter().find_map(|arg| normalize_ecp(arg));

            if let Some(ecp_path) = cli_ecp {
                log::info!("Opening project via CLI activation: {}", ecp_path);
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            get_persisted_column_ids,
            get_aggregates_for_test,
            ensure_duckdb_dataset,
            prewarm_dataset_infrastructure,
            flush_overlay,
            ...
            set_remote_session_pending_guest,
            ...
            set_remote_window_capture_exclusion,
        ])
